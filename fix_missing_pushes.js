const fs = require('fs');
let content = fs.readFileSync('C:/AS/AIStory/frontend/src/pages/editor/components/ScriptEditor.jsx', 'utf-8');

let block1_old =             const basePositioning = getInfoValue(['base_positioning']);
...
let block1_new =             const basePositioning = getInfoValue(['base_positioning']);
            ...
            const eraField = getInfoValue(['era', 'era_setting', 'period', 'time_setting']);
            const regionField = getInfoValue(['region_culture', 'region', 'country', 'country_region']);
            if (eraField) metaParts.push(\Era / Period (年代): \\);
            if (regionField) metaParts.push(\Region / Country (国家地域): \\);
...
let block2_old =                             const basePositioning = getInfoValue(['base_positioning']);
...
let block2_new =                             const basePositioning = getInfoValue(['base_positioning']);
                            if (title) metaParts.push(\Title: \\);
                            if (episode) metaParts.push(\Episode: \\);
                            if (type) metaParts.push(\Type: \\);
                            if (basePositioning) metaParts.push(\Base Positioning: \\);
                            ...
                            if (eraField) metaParts.push(\Era / Period (年代): \\);
                            if (regionField) metaParts.push(\Region / Country (国家地域): \\);
                            if (shotPrefField) metaParts.push(\Shot / Lens Preference (镜头偏好): \\);
                            if (broadcastSafetyField) metaParts.push(\Broadcast Security Level (播出安全等级): \\);
                            metaParts.push('Use this project context as first-class constraints before analyzing the script.');

                     if (metaParts.length > 1) {;

content = content.replace(block1_old, block1_new);
content = content.replace(block1_old, block1_new); // phase 2 has two identical blocks
content = content.replace(block2_old, block2_new);

fs.writeFileSync('C:/AS/AIStory/frontend/src/pages/editor/components/ScriptEditor.jsx', content);
console.log('Done using node!');
