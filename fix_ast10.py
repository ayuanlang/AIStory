...
old_block = """                const { adaptedScriptText, userInput: stage2UserInput } = buildStage2UserInputFromStage1(analyzedText || '', selectedReuseSubjectAssets);
...
new_block = """                const { adaptedScriptText, userInput: stage2UserInput } = buildStage2UserInputFromStage1(analyzedText || '', selectedReuseSubjectAssets);
                if (!String(adaptedScriptText || '').trim()) {
                    throw new Error('第一阶段未提取到“修改后的剧本”正文，请确认 返回结果包含第二部分剧本正文后重试。');
                }
                     ...
                     try {
                         ...
                         if (matchObjStr.startsWith('{')) {
                             ...
                             if (parseObj.code || parseObj.error) {
                                 errMsg = `上游模型接口返回了系统异常，未能正常生成剧本：${parseObj.msg || parseObj.error?.message || matchObjStr}`;
                             }
                         }
                     } catch(e) {
