...
pat = r'<label className="block text-xs font-semibold tracking-wide mb-1 text-primary/95">\{t\(\'时代设定\', \'Era\'\)\}</label>\s*<select.*?value=\{newEra\}.*?onChange=\{.*?setNewEra.*?\}>.*?</select>'
repl = r'<InputGroup label={t("时代设定", "Era")} value={newEra} onChange={setNewEra} list={projectCreateOptions.era} />'
