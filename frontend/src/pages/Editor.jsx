...
const Editor = ({
    ...
    useEffect(() => {
        preloadOssActiveUrlSignatures(fetchOssActiveUrlSignatures).catch(() => {});
    }, []);
    ...
    const getProjectVisualBackfillFromJsonText = (jsonText) => {
        ...
        for (const obj of candidateObjects) {
            ...
            if (payload.Global_Style || payload.borrowed_films.length > 0 || payload.borrowed_films_note || payload.tone || payload.lighting || payload.color_palette || payload.color_spectrum || payload.plot_summary || payload.music_recommendation) {
                return payload;
            }
        }
...
// Fallback: generic key-value rows (with or without backticks, with or without numbered list)
    ...
    const handleImport = async (text, importType = 'auto', importOptions = {}) => {
        ...
        if (projectVisualBackfill && id) {
            ...
            if (isBlank(originalGlobalInfo.color_spectrum) && projectVisualBackfill.color_spectrum) {
                backfillPatch.color_spectrum = projectVisualBackfill.color_spectrum;
            }
