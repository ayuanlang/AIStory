...
export const SubjectLibrary = ({ projectId, project, currentEpisode, uiLang = 'zh', userBatchParallelLimit = 3, onImportText = null }) => {
    ...
    const handleBatchGenerateEntities = async () => {
        ...
        if (!await confirmUiMessage(
            t(
                `将为${entityEpisodeScope === 'current' ? '当前分集中' : '整个项目中'} ${toGenerate.length} 个主体批量生图（遵循依赖顺序）。是否继续？`,
                `Batch generate images for ${toGenerate.length} entities in the ${entityEpisodeScope === 'current' ? 'current episode' : 'whole project'} (respects dependency order). Continue?`
            )
        )) return;
                                                        ...
                                                        {parseVisualDependencies(viewingEntity.visual_dependencies).map((dep, idx) => {
                                                            ...
                                                            return (
                                                                <div key={idx} className="flex-shrink-0 w-24 bg-black/40 border border-white/10 rounded-lg p-1.5 flex flex-col gap-1 relative group">
                                                                    ...
                                                                    <div className="absolute inset-0 z-30 bg-black/80 opacity-0 group-hover:opacity-100 flex flex-col gap-1 items-center justify-center transition-opacity rounded-lg p-1 isolate">
                                                                         {depEntity?.image_url ? (
                                                                             <>
                                                                                <button
