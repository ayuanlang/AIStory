    ...
    const pollOnce = async () => {
        ...
        try {
            ...
            for (const unit of sceneUnits || []) {
                ...
                await trySceneReady(sceneId, row, unit);
            }
            for (const row of rows) {
                const sceneId = String(row?.scene_id || '').trim();
                if (!sceneId) continue;
                await trySceneReady(sceneId, row, byId[sceneId] ? null : row);
            }
        } catch (_) {
...
const runAnalysisPipelineIntegrityGate = async ({
    ...
    onLog?.(
        t(
            `[Integrity Gate] 开始检查：期望 ${expected} 场入库，Subject Index ${resolvedSubjectIndex.length} 字符`,
            `[Integrity Gate] Starting checks: expecting ${expected} scene(s) in workspace, Subject Index ${resolvedSubjectIndex.length} chars`
        ),
        'info'
    );
...
export const ScriptEditor = ({ activeEpisode, projectId, project, onUpdateScript, onUpdateEpisodeInfo, onRefreshEpisodes, onLog: parentOnLog, onImportText, onSwitchToScenes, assetRerunRequest = null, onAssetRerunRequestConsumed = null, uiLang = 'zh', tabMediaRefreshSignal = 0, isTabActive = true, onMediaRefreshRequest = null }) => {
    ...
    const appendAnalysisDetailLog = useCallback((message, type = 'info') => {
        ...
        const next = [...analysisDetailLogsRef.current, entry];
        analysisDetailLogsRef.current = next.length > ANALYSIS_DETAIL_LOG_CAP
            ? next.slice(-ANALYSIS_DETAIL_LOG_CAP)
            : next;
    ...
    const onLog = useCallback((message, type = 'info') => {
        if (captureAnalysisLogRef.current()) {
            appendAnalysisDetailLog(message, type);
        }
        parentOnLogRef.current?.(message, type);
    ...
    const isSceneTableDataLine = (line) => {
        const text = String(line || '').trim();
    ...
    const extractScenesTableMarkdownBlock = (text) => {
        ...
        for (const rawLine of raw.split('\n')) {
            ...
            if (!rowLine.startsWith('|')) {
                if (
                    tableLines.length >= 2
                    && !isSceneTableSeparatorLine(tableLines[tableLines.length - 1])
                ) {
                    tableLines[tableLines.length - 1] = `${tableLines[tableLines.length - 1]} ${rowLine}`.trim();
                    continue;
                }
                if (tableLines.length > 0) break;
                continue;
        ...
        if (tableLines.length < 2 || !tableLinesHaveDataRow(tableLines)) return '';
    ...
    const patchSceneTableRowIdentity = useCallback((tableText, { sceneId, sceneOrder } = {}) => {
        ...
        if (resolvedSceneOrder != null && sceneNoIdx >= 0) {
            ...
            const nextRow = [...row];
            headers.push('Scene No');
            nextRow.push(String(resolvedSceneOrder));
            return `### Part 1: Scenes Table\n\n${buildMarkdownTable(headers, [nextRow])}`.trim();
        }
    ...
    const splitSceneMarkdownTableBySceneId = useCallback((tableText) => {
        ...
        parsed.rows.forEach((row, index) => {
            ...
            byScene[stableSceneId] = {
                ...
                scene_order: resolvedOrder,
                scene_name: sceneName,
                scene_no: sceneNo || String(resolvedOrder),
    ...
    const runStage2_1WithValidationRetry = useCallback(async (runAttempt, contextLabel = 'Stage 2.1') => {
        ...
        const subjectIndexLen = String(validation.subjectIndexText || text || '').trim().length;
        onLog?.(
            t(
                `[Stage 2.1] LLM 已返回并通过校验（Subject Index ${subjectIndexLen} 字符）`,
                `[Stage 2.1] LLM returned and passed validation (Subject Index ${subjectIndexLen} chars)`
            ),
            'success'
        );
    ...
    const persistSceneMarkdownPatch = useCallback(async (sceneMarkdownByScene, options = {}) => {
        ...
        try {
            ...
            const persistedBySceneMap = mergeSceneMarkdownBySceneMaps(
    ...
    const ensureOrchestrationScenesInWorkspace = useCallback(async ({
        ...
        for (const [sceneId, entry] of entries) {
            ...
            const report = await importSingleSceneDuringOrchestration({
                sceneId: stableSceneId,
                sceneOrder,
                markdown: importText,
    ...
    const runStage2_2WithValidationRetry = useCallback(async ({
                    ...
                    selectedReuseSubjectAssets,
                    {
                        onTaskCreated: (taskId) => {
                            ...
                            if (stableTaskId) {
                                onLog?.(
                                    t(
                                        `[${attemptLabel}] 异步任务已创建 task_id=${stableTaskId}`,
                                        `[${attemptLabel}] Async task created task_id=${stableTaskId}`
                                    ),
                                    'info'
                                );
                            }
        ...
        if (explicitSceneUnits.length > 0 && explicitSceneUnits.length !== sceneUnits.length) {
            onLog?.(
                t(
                    `[${label}] 使用显式场景列表 ${explicitSceneUnits.length} 场（从 Stage 2.2 输入解析到 ${sceneUnits.length} 场）`,
                    `[${label}] Using explicit scene list (${explicitSceneUnits.length} scenes; parsed ${sceneUnits.length} from Stage 2.2 input)`
                ),
                'info'
            );
        }
            ...
            const runSequentialSceneFallback = async (reason = '') => {
                ...
                for (const unit of unitsToProcess) {
                    ...
                    await importSingleSceneDuringOrchestration({
                        ...
                        unit,
                        replaceExistingScenes: false,
                        source: `${logPhasePrefix}-sequential-import`,
                    });
                }
        ...
        return () => {
            ...
            if (stillRunning && onLog) {
                ...
                if (now - lastScriptLeaveNoticeAtRef.current > 3000) {
                    ...
                    onLog?.(
                        '已离开剧本页，分析任务仍在后台运行。返回剧本页后会自动恢复完整进度与日志。',
                        'info'
                    );
                }
            }
    ...
    const executeAnalysis = async (content, customSystemPrompt = null, skipMetadata = false, retryCount = 0) => {
        ...
        try {
            ...
            phaseMarks.llmReturnedAt = Date.now();
            onLog?.(
                t('[Stage 1] LLM 已返回，正在校验与持久化...', '[Stage 1] LLM returned; validating and persisting...'),
                'success'
            );
        ...
        try {
            ...
            if (splitStage1Flow) {
                ...
                const runSceneOrchestrationBranch = async () => {
                    ...
                    onLog?.(
                        t('[Stage 2.2] 开始场景编排（镜头节拍生成）...', '[Stage 2.2] Starting scene beats orchestration...'),
                        'info'
                    );
                    const beatsResult = await runStage2_2WithValidationRetry({
                        label: 'Stage 2.2',
    ...
    const executeSceneBeatsRerun = useCallback(async ({ mode = 'all', sceneId = '' } = {}) => {
        ...
        try {
            ...
            if (rerunMode === 'all') {
                const patchSceneIds = allScenePatchMap ? Object.keys(allScenePatchMap).filter(Boolean) : [];
                if (patchSceneIds.length >= orchestrationSceneCount) {
                    // complete per-scene patch map
                } else {
                    const importCheck = validateAutoSceneTableImport(String(validatedBeatsText || '').trim());
                    const tableRowCount = importCheck?.ok
                        ? (Array.isArray(parseMarkdownTable(importCheck.tableText || '')?.rows)
                            ? parseMarkdownTable(importCheck.tableText || '').rows.length
                            : 0)
                    ...
                    if (orchestrationSceneCount > 1 && tableRowCount < orchestrationSceneCount) {
                        throw new Error(t(
                            `全部场景重排未获得完整的分场景 LLM 结果（${tableRowCount || patchSceneIds.length}/${orchestrationSceneCount} 场），已中止以避免写入不完整数据。`,
                            `All-scene rerun did not receive complete per-scene LLM outputs (${tableRowCount || patchSceneIds.length}/${orchestrationSceneCount}); aborted to avoid incomplete writeback.`
                        ));
                    }
                }
            }
            ...
            if (rerunMode === 'single') {
                ...
                await waitForEpisodeSceneCount(
                    fetchScenes,
                    activeEpisode?.id,
                    orchestrationSceneCount,
                    { retries: 20, delayMs: 400 },
                );
                let dbScenes = await fetchScenes(activeEpisode?.id).catch(() => []);
                const missingPatch = filterPatchMapToMissingScenes(allScenePatchMap, dbScenes);
                const missingIds = Object.keys(missingPatch).filter(Boolean);
                if (missingIds.length > 0) {
                    importReport = await importScenesFromPerScenePatchMap(missingPatch, {
                        replaceExistingScenes: false,
                        skipSceneIds: [],
                    });
                } else if (await canSkipBatchSceneImport({
                    fetchScenesFn: fetchScenes,
                    episodeId: activeEpisode?.id,
                    patchSceneIds,
                    ...
                    setAnalysisFlowStatus({
                        phase: 'scene_beats',
                        message: t(
                            `✅ 全部 ${patchSceneIds.length} 场已在工作区校验通过。`,
                            `All ${patchSceneIds.length} scene(s) verified in workspace.`
                        ),
                    });
                    importReport = {
                        ok: true,
    ...
    const workflowCompletenessStats = useMemo(() => {
        ...
        // A subtask that did not report status 'ok' means that category's generation genuinely
        // failed and must be surfaced as a hard failure, not silently hidden as "no data".
        const subtaskReports = Array.isArray(scenePostReport?.subtaskReports) ? scenePostReport.subtaskReports : [];
        const subtaskInfoBySubjectCategory = {};
        subtaskReports.forEach((report) => {
            const reportKey = String(report?.key || '').trim();
            if (!reportKey) return;
            const status = String(report?.status || '').trim().toLowerCase();
            subtaskInfoBySubjectCategory[reportKey] = {
                failed: Boolean(status) && status !== 'ok',
                status,
                error: String(report?.error || report?.recommendation || '').trim(),
            };
        });
        const subtaskCategoryByAssetKey = {
            character: 'characters',
            prop: 'props',
            environment: 'environments',
            poster: 'environments',
        };
        // Default rule: once Stage 2.1 has produced a Subject Index, every asset category it
        ...
        const buildAssetRow = (key, labelZh, labelEn, categoryKey) => {
            ...
            const subtaskInfo = subtaskInfoBySubjectCategory[subtaskCategoryByAssetKey[key]];
                    ...
                    <div className="flex-1 w-full flex items-center justify-between relative max-w-3xl px-8 mt-2 md:mt-0">
                        ...
                        <div className="flex flex-col items-center gap-2 relative">
                             <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold z-10 border ${
                                workflowCompletenessStats.hasFailedSubtask
                                ...
                                {workflowCompletenessStats.hasFailedSubtask ? <X className="w-4 h-4" /> : (!!getStageOutputContent('stage3', 'asset_design_json') ? <Check className="w-4 h-4" /> : 5)}
                            </div>
                            <div className="flex flex-col items-center gap-1 text-center">
                    ...
                    <div className="mt-3 pt-3 border-t border-white/10 text-[11px] text-white/70 leading-relaxed">
                        ...
                        {workflowCompletenessStats.summaryItems.map((item, index) => (
                            <span
                                ...
                                title={item.errorMessage || (item.status === 'missing' ? t('该类子资产入库数量为 0，请检查并重跑。', 'This asset category has 0 imported items. Please check and rerun.') : '')}
                            >
                                {t(item.labelZh, item.labelEn)} {item.analysisCount}→{item.importedCount}{item.mark}
                            </span>
                        ))}
                        <span className="ml-3 text-white/30">
                            {t('（角色/道具/环境/封面默认不允许为 0）', '(character/prop/environment/poster must not be 0 by default)')}
                        </span>
                    </div>
                )}
                ...
                {workflowCompletenessStats.failedCategories.length > 0 && (
                    <div className="mt-3 rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-2.5 text-xs text-red-100 space-y-1.5">
                        ...
                        <div className="pt-0.5">
                            ...
                            >
                                {isRetryingPhase2 ? <Loader2 className="w-3 h-3 animate-spin"/> : null}
                                {isRetryingPhase2 ? t('处理中', 'Processing') : t('重跑失败的资产类型', 'Rerun failed asset categories')}
                            </button>
                        </div>
                    </div>
                )}
