        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50" onClick={onClose}>
            <div className="bg-gray-900 border border-gray-700 p-6 rounded-xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <input className="bg-black/30 border border-gray-700 rounded px-3 py-2 text-sm" inputMode="numeric" value={normalizeUserActiveLevel(draft.is_active, 1)} placeholder={t('启用级别(并行=级别+2)', 'Active Level (parallel=level+2)')} title={t('批量编排与图片/视频提交的最大并行数 = 启用级别 + 2（启用级别 1-12）', 'Batch orchestration and image/video submit parallel cap = active level + 2 (level 1-12)')} onChange={(e) => setDraft((prev) => ({ ...prev, is_active: normalizeUserActiveLevel(e.target.value, prev?.is_active ?? 1) }))} />
