...
// 2) Render frontend: default to direct backend first while the existing static site
//    cannot reliably proxy POST /api requests on Render
// 3) Local/dev: same-origin
const RAW_BASE_URL = String(import.meta?.env?.VITE_API_BASE_URL || '').trim();
const PREFER_DIRECT_BACKEND = String(import.meta?.env?.VITE_PREFER_DIRECT_BACKEND || '1') === '1';
