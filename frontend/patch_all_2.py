...
def patch_all():
    ...
    for i, line in enumerate(lines):
        if 'async def analyze_scene(request: AnalyzeSceneRequest, current_user: User = Depends(get_current_user)' in line:
            lines[i] = line.replace(')): # user auth optional', '), is_stream: bool = False): # user auth optional')
            break
