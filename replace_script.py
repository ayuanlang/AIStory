...
with open('current_old_version.txt', 'r', encoding='utf-8') as f:
    old_block = f.read()

new_block = '''                    {analysisUiReport && analysisUiReport.status !== 'running' && (() => {
                        ...
                        return (
                            <div className={ounded-lg border bg-black/20 p-3 text-xs space-y-1.5 mt-2 }>
                                <div className={ont-bold text-base flex items-center gap-2 }>
                                    ...
                                    {isZeroResult ? t('剧本分析结果异常：提取失败！', 'Analysis Result Abnormal: Extraction Failed!') : t('剧本分析与导入完成！', 'Analysis & Import Completed!')}
                                </div>
...
if old_block in content:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Success! UI logic updated.")
else:
    print("Failed to find exact old block from file.")
