...
def export_project_story_generator_global_package(
    ...
    def _extract_story_structured(md: str) -> Dict[str, Any]:
        ...
        suspense_block = _extract_between(raw, r"##\s*5\)\s*悬念系统", r"##\s*6\)\s*伏笔与回�?)
        foreshadowing_block = _extract_between(raw, r"##\s*6\)\s*伏笔与回�?, r"##\s*7\)\s*分集规划")
        background_block = _extract_between(raw, r"##\s*1\)\s*核心设定（背�?世界观）", r"##\s*2\)\s*主角与目�?)
