...
5. **全域光影执行规范（Mandatory · 每镜 Video Content）**
   ...
   - **光源充足硬约束（环境相关）**：`ENV:[...]` 为室内/办公室/厅堂/豪宅/商业/户外日景等时，默认须**照度充足、主舞台与动线可读**——须写 **≥2 动机光源** + **≥1 环境反射/补光**；禁止整镜只有 vague atmosphere、单点昏暗光或无动机黑场（悬疑/惊悚/恐怖低照度例外须 `Shot Logic` 注明依据）。
...
- **全域光影（§四.5）**：`光影继承:`+`光源充足性:` 已写｜Video「动态连续光影/焦点」+「光线连动弧光」**九项覆盖≥7**｜**§四.5-E 柔和侧逆光+正面Fill+柔散过渡无硬边界** 已写｜**光源≥2+每光源作用范围+每光源可见效果+FG/MG/BG 分层+冷暖+投影/反射** 已写｜对话镜面部受光可读（§六.3）｜ENV 相关镜照度充足或低照度例外已注明｜**整体氛围柔和自然真实**。
