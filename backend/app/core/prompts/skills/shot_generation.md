...
4. **摄影机运动**：推/拉/摇/跟；每场至少1个高级运镜；OTS 必写 Left-Shoulder 或 Right-Shoulder；不可越轴。
   ...
   - **对白收束回全景（强制）**：**对白结束判定** = 本组全部 `(Pn)` 语言读完 **且** 各 `(Pn)` 绑定的说话人/听者 **微表情链与微动作均完成落点结景**；组内须逐 `(Pn)` 落点后再切下一 `(Pn)` 或 Pull Back，此前禁 Pull Back。满足判定后，终段/末镜须 Pull Back / Pull Out / Dolly Out / Crane Up 回到对白前 **Master / Two Shot / Wide** 建置落幅，复写 P1 三层空间与对白后实体落位；禁收束仍停中近景/特写。Walk-and-Talk、Scene 末镜切场、上游强制特写/插帧、**已在 Extreme Wide/Wide/Full 宏观主拍的对白组**除外；`Shot Logic (CN)` 须标注。
...
9. **对话景别与反应覆盖（强制）**：拆镜时对白镜、反应镜、微表情/微动作镜默认 **中景或特写**（含 OTS、Medium Close-up、Close-up）；`(Pn)` 听者段内的微表情与微动作须在同级景别内可读，不得写在全景/远景中一笔带过。景别切换遵守 §三.1 逐级递进，避免越级跳切；Wide/Full 仅用于 Scene 建置、走位展示或特殊调度，不得替代常规对白覆盖。
   - **对白须配说话人运镜（强制）**：每条口型可读对白拆镜时，`Shot Logic (CN)` 须标注对白覆盖方式；`Video Content (CN)` 对白 P 段须写清运镜与说话人景别；**微表情/微动作落点完成后**再回全景建置（§三.4）；特效/宏观场面例外见 §三.4。
...
2. **运镜与动作流**（须符合 §三.4 高级运镜、§三.7 三段式、§五、§六、§七）：
   ...
   - P1 写起始状态 + **当前主场 `ENV:[环境名]`** + **先**前景/中景/背景三层框架、**再**各层内逐个主体建置（§七.1）；**含口型可读对白的 Shot，末句 `(Pn)` 微表情/微动作落点完成后，终段 Pull Back 回 P1 同级全景建置**（§三.4）；环境切换段须再次写明目标 `ENV:[...]`。
...
- 运镜：已建轴线 → 起镜/过渡/落镜 → 无无理由越轴/急变焦 → 焦点闭环 → **已判定对白类型（含特效/宏观例外）**，口型可读对白 P 段已写运镜+说话人主拍（§三.4） → **末句微表情/微动作落点完成后已 Pull Back 回全景建置（§三.4，宏观主拍除外）** → 景别无越级跳切（§三.1、§六.9）。
- 空间：基准唯一 → **先 FG/MG/BG 三层框架齐全** → **各层内每个可见主体是否均已逐个写清** → **当前视角下是否全局可见** → **朝向/视线是否与对白主听众/被点名对象咬合** → 关键道具有坐标 → 动态起落无冲突。
