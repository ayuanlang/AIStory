...
3. **时长策略**：单镜 [4, 15] 秒；默认紧凑节奏，单镜目标 **6s-9s**；含连续动作链、特效相位或同框因果闭环时可合并至 **9s-12s**；长镜头偏好 -> 上限 12s-15s，不得为凑时长堆叠无因停顿。
...
4. **时长推演公式 (强制 4s-15s，紧凑优先)**：
   - **基础计时单位**：所有时间预估必须拆成可核对类型，写入 `Shot Logic (CN)`：`时间预估: 建置Xs + 语言Xs + 动作Xs + 微表情Xs + 特效Xs + 反馈Xs + 转场/停顿Xs = 串行Ys；并行核=Max(...)Xs；题材系数×K；Duration=Zs。` 无该类型写 `0s`，不得只写总秒数。
   - **计算顺序（强制，防重复计时）**：① 先分别估算各类型耗时；② 对白/动作/微表情/特效/运镜**同相位并行**时，只取 `Max(语言, 动作, 微表情, 特效, 运镜)` 作为并行核，**禁止**把已并行的项再次全额串行相加；③ 仅对**必须串行**的段落追加：建置（首段或建置更新=是）、转场桥接、结果落位定格、强悬念插帧；④ 听者反馈若与对白同相位但 Beat 明确要求可视反应，可在并行核外另加 **0.3-0.5s**；⑤ 套用题材节奏系数 K 后四舍五入。
   - **语言耗时**：中文对白/旁白/OS/V.O./自白基准 `中文字数 / 5` 秒；按 `speed` 修正：`快`×0.9、`中速`×1.0、`慢`×1.1、`极慢`×1.25；≤8字短句保底 **1.2s**；>20字长对话拆短句，句间呼吸 **0.2-0.5s**；口型可见的对白不得低于语言耗时；OS/V.O./画外旁白可与画面动作并行取 Max。
   - **建置/运镜耗时**：`建置更新=是` 或 Scene 首镜：空间建置 **1.5-2.5s**；同 Scene 后续镜且 `建置更新=否`：**0.3-0.8s** 仅写变更项；关键角色/道具首次落位每组 **0.4-0.7s**；Rack Focus **0.4-1s**；短程推拉摇移 **0.8-1.5s**；OTS 反打建轴 **1.5-2s**。运镜与对白/动作同相位时并入并行核取 Max，不得全额另计。
   - **动作耗时**：常态短发力 **0.8-1.5s**；递交/转身/落座/起身/后退等单步 **1.2-2.2s**；复杂交互、拉扯、攻击、防御 **2.5-4s**；长距离或多障碍 >5s 趋势必须拆 Shot。
   - **微表情耗时标准化**：仍按 `前置 -> 中段 -> 落点` 推演；与对白/动作同相位时并入并行核取 Max；**独占画面相位**才单独计时：单点 **0.4-0.8s**；完整三段链 **1-2s**；强渐变链（落泪/怒意升级）**1.5-3s**。
   - **听者反馈耗时**：与对白同相位且反应已写入 `(Pn)` 听者段，默认并入并行核；Beat 强制要求独立反应镜或插帧时：单人 **0.5-0.8s**；双人 **0.8-1.2s**；群演统一/随机 **0.8-1.5s**。
   - **特效耗时标准化**：按 `触发源 -> 显形/扩散 -> 命中/作用 -> 维持 -> 余波` 分相；轻量反馈 **1-1.8s**；单段法术/能量/技术 **2.5-4s**；对抗型 **4-6.5s**；大范围环境 **6-10s**（>10s 必须拆镜）；与动作咬合取 `Max(动作, 特效)+余波0.5-1s`。
   - **情绪停顿/插帧耗时**：道具/局部特写 **0.8-1.2s**；强悬念或信息落点 **1-2s**；间歇插帧可嵌入主节拍间隙，独占相位才全额计入。
   - **题材节奏系数 K（乘算，写在时间预估末尾）**：短剧/投流/动作/悬疑惊悚/犯罪 **×0.92**；喜剧/日常 **×0.95**；标准叙事 **×1.0**；情感/爱情/治愈/现实主义细腻 **×1.05**；长镜头偏好输入存在时 K 下限 **0.97**，仍不得突破 15s。
   - **总耗时计算**：`T = (建置串行 + Max(语言, 动作, 微表情, 特效, 运镜) + 独占反馈 + 独占插帧/停顿 + 转场桥接 + Beat 强制反应补差) × K`；多 P 段同镜内，各段并行核**相加**（段间切换即节奏，不每段重复全额建置）。
   - **调平硬规则**：T 四舍五入为整数秒；低于 4s 仅内容极简时允许，否则补足必要落点停顿；高于 15s 必须拆 Shot；高于 11s 须写明不可拆分理由；不得通过删除上游对白、特效相位、微表情链或结果落位来降时长。
...
1. **对白/旁白/自白逐字保留（严重强调）**：上游 Beat 的 Dialogue、OS、V.O.、旁白、内心自白、独白必须以原始完整文本进入 `Video Content (CN)`，格式：`(Pn) {说话动作/闭口聆听/内心独白状态} — Dialogue/OS/V.O./旁白/自白 (CHAR:[@Name] 或 NARRATOR) (voice_type: xx, tone: xx, speed: xx, volume: xx): "完整全句" — {听者视觉反应}`。段内动作与环境描写仍用自然语言，不得退回 `结构=...｜` 字段体。引号内必须逐字等同 Beat 原文：不得省略任何字词、标点、称谓、语气词、重复词、停顿词；不得改成摘要、意译、旁述或“继续说完”。听者反馈覆盖本镜其他在画角色，含群演则补统一/随机反馈。
