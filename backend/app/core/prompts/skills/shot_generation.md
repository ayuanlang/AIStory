...
2. **运镜与动作流**（须符合 §三.4 高级运镜、§三.7 三段式、§五、§六、§七）：
   ...
   - P1 写起始状态 + **当前主场 `ENV:[环境名]`** + **先**前景/中景/背景三层框架、**再**各层内逐个主体建置（§七.1）；含口型可读对白的 Shot，P 段须按 §三.1 在中景/特写主拍；若本镜末含 Pull Back/回全景，只写**可见运镜与三层落位**，收束判定与下一镜依据只写 `Shot Logic (CN)`。
...
- `Video Content`：自然叙述 + P1/Pn 分段 → **已显式写出 `ENV:[...]`（P1 必检）** → **无收束判定/下一镜推演等 Shot Logic 解释句** → 无“上镜/同上一镜”代指 → 有 Global_Style 则首句原文 → Beat 语言逐字可检索（§六.1）。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (自动生成) | (核心动作简述) | (当前场景ID) | (切换判定+收束落幅判定+主节拍规划继承+景深层次继承+环境切换声明+防穿帮自检+时间预估+运镜优化自检+空间结构自检) |  |  | (整数秒数) |  |  |  | (连贯中文叙述体；运镜与动作流按 P1/P2/P3… 分段；**必须显式写出当前主场 ENV:[环境名]，P1 建置段必含**；对白用 (Pn) 格式，含对白 P 段须写运镜+说话人主拍（中景/特写）；**收束落幅判定只写 Shot Logic，Video Content 禁写下一镜/本组对白/路径等解释句，只写可见运镜与落位**；前景/中景/背景、锚点、运镜、动作等信息融入自然句子，禁止字段体；已登记实体保留 CHAR/PROP/ENV 标签，群演用自然语言禁 EXTRA；禁写上镜/承接上一镜等代指；若有 Global_Style 须在“全局动态风格”首句原文写入) |  |  | (该镜头涉及的 `CHAR`, `PROP`, `ENV` 标签列表) |
