...
3. **环境锚点定桩**：角色落位/朝向/动作先锚定环境实体；正反打重建变体锚点。
   - **锚点先行定义（硬约束）**：凡在 `Shot Logic (CN)` 或 `Video Content (CN)` 中出现任何锚点引用（含主锚点、辅助锚点或其他锚点表述，如“距锚点X步/围绕锚点/相对锚点/挂靠锚点”），必须先用一句话交代该锚点具体指代（`锚点：ENV:[...] 的[固定结构/固定物]`）后再引用；禁止未定义锚点即直接使用该说法。
   - **术语外显非强制**：`主锚点/辅助锚点` 属于内部推演标签，最终输出可不出现这些术语；但必须先交代“固定参照物是什么”，再基于该参照物写距离、方位、朝向与动作。
...
> 说明：示例里出现“主锚点/辅助锚点”是为了展示推演方法；生产输出不强制出现术语名词，只要先交代固定参照物，再写相对位置即可。

#### 示例场景设定
- 主锚点：`ENV:[Office]` 的门内侧铰链。
- 辅助锚点：`ENV:[Office]` 的会议桌右前角、靠窗文件柜上沿外侧角点。
- 关系轴线：`CHAR:[@Lin]` 与 `CHAR:[@Chen]` 的对视线。
- 障碍物：两人之间隔着 `PROP:[Desk]`。
- 示例化改写（不显式说“主/辅锚点”也可）：先说明“以门内侧铰链、会议桌右前角、文件柜外侧角点为固定参照物”，再写“谁在参照物左前/右后几步、朝向哪里、动作如何变化”。
...
- **逻辑推演 (Shot Logic)**：纯中文；逐镜必填：`切换判定: 时空关系=...；桥接依据=...；轴线状态=...；跨幅级别=...。` `主节拍规划继承: 来源Beat=...；核心动作=...；承接点=...；落点功能=...；本镜承担=起势/间歇插帧/结果落位。` 非首镜加 `前接说明: 前一镜可见落点=...；本镜过渡手法=...；本镜画面提示词仅复述当前可见实体状态,不写承接上一镜。` 首镜加“开场转场技巧说明”（不可无过渡/None）。环境切换加 `环境切换声明: 切换到 ENV:[...]；桥接依据=...；切换后重建=机位/主锚点/角色坐标/道具坐标。` 无切换写 `环境切换声明: None。` 必含“防穿帮自检”与标准化时间预估：`时间预估: 建置Xs + 语言Xs + 动作Xs + 微表情Xs + 特效Xs + 反馈Xs + 转场/停顿Xs = Ys，Duration=Zs。`
