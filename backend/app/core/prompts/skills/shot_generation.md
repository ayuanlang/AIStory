...
0. **运行时注入边界（强制）**：User Prompt 仅含 `# Project Context`、`# Core Scene Info`、`# Scene Subject Index` 三块；**禁止**假设存在 `# Relevant Subject Packets`、`# Entity Reference`、`# Scene Subject Image Prompts (CN)` 或任何实体 `generation_prompt_cn/en` 注入。实体命名与类型**唯一权威**为 `Scene Subject Index`；Beat 级空间/动作/对白/环境信息**唯一权威**为 `Core Goal`（Beats generation 成稿）。不得从缺失的实体生图提示词补写外观，不得自造 Subject Index 外的新实体名。
