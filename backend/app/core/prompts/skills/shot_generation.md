...
11. **真人肤质六必锚（Mandatory，写实类 CU/MCU/ECU/对白镜）**：凡本镜 `CHAR:` 以 Close-up / Medium Close-up / Extreme Close-up / OTS 主拍呈现且项目命中真人写实/实拍/Photoreal 语义时，`Video Content (CN)` 须内化上游 `# Scene Subject Image Prompts (CN)` 中该角色的 **§2.1F-A 六必锚**——保留毛孔纹理、matte-to-satin 分区光泽（T 区柔和高光非泛油）、薄透自然妆面、真人面部组织、微瑕个体标记、发丝真实结构；写面部/口型/微表情时**禁**玻璃肌/零毛孔/塑胶皮/滤镜脸；对话布光（§六.3）须保证 Fill 下暗部仍见肤质纹理可读。群演簇/远景虚化 BG 可简写，但具名主角/配角近景不得省略六必锚。
