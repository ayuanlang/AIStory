...
1. **对照范围 (强制全检)**：拆镜前逐 Beat 按**前置+六环节**核对：  
   ...
   - 另检：轴线与进出画路径、肢体接触、**Beat 动作/道具/ENV 逐句继承（§一.2，每个 PROP/修饰/层位节点是否在 Video Content 可检索）**、道具连续、口型/OS/V.O.、光色连续、特效相位、**环境实体微动态（§七.7，树枝/灯光/车流等，BG 是否完全静止）**、**含人群时是否双层并列（§七.7 环境实体 + §七.7-b 人群，禁只写人群漏环境）**、**背景人群是否完全随机且跨镜无重复描述（§七.7-b，≥2 种异质微动作，禁整簇同步木偶）**。
...
- **背景人群随机动势（§七.7-b，强制，与 §7.7 并列追加）**：BG 可见群演/匿名人群簇时，**须在 §7.7 环境实体微动已写前提下另写**；**每镜完全随机生成**，同一簇内 **≥2 种随机差异化微动作**，全镜持续、幅度 subtle，禁整簇如同木偶同步。**跨镜禁重复（强制）**：同 Scene 内各镜 BG 人群簇的**区段锚点+人数+微动作组合须重新随机**，**禁止**复用上镜或同场他镜已写过的相同描述（含整段句式、固定动作 trio 如连续多镜皆「停笔抬头/侧身低语/敲键盘」）；可保留环境锚点类型（如仍处文件柜前/百叶侧），但人数分布与微动作须换用不同随机组合。**写法**：先写簇位（FG/MG/BG+区段+人数+虚化）→ 再从动作池随机择 2–3 项组合分列（停笔抬头/侧身低语/端杯让位/翻页/倚墙抱臂/缓步经过/停步张望/整理衣领/接打电话/端托盘穿行/倚栏观望/低头看手机/系鞋带/推轮椅缓行等，**禁止**每镜套用同一套固定 trio）。**统一反馈时刻**（主配角关键台词/动作触发）：可短暂写簇内**部分人**同步反应 + **其余人**仍保持各自随机手头动作，或全簇统一反应一次后立即恢复差异化动势；**禁止**无触发时整簇全程同一动作。**与宏观场景区分**：§三.4 宏观军团/大军/机群等主节拍允许整齐同步动势；稀疏背景小簇（2–10 人）一律按本条完全随机写法。
- **反例（强制禁止）**：❌ 背景文件柜、百叶窗、窗外景物全程无任何动势，画面如静态照片｜❌ **只写办公人员簇动作，漏写百叶光斑/窗外树/顶灯/车流等 ENV 微动**｜❌ `后景 5 名路人齐步同向走过、动作完全一致`｜❌ `办公人员簇全员停谈转头定格至镜末`（无对白触发且无异质动作）｜❌ **同 Scene SH01–SH04 四镜皆写「文件柜前：停笔抬头/侧身低语/敲键盘」**（跨镜重复、非完全随机）｜❌ 为凑微动写与 Beat 矛盾的「狂风大作」干扰对白｜✅ `背景百叶叶缝光斑随空调气流轻晃，窗外远距树冠随风低速摆动，顶灯 Practical 极弱频闪`｜✅ **双层并列** `百叶叶缝光斑轻晃 + 窗外车流光斑缓移；文件柜前簇：一人停笔抬头、一人侧身低语、一人继续敲键盘`｜✅ **SH01** `…百叶轻晃、窗外树影微动；文件柜前簇：停笔抬头/侧身低语/敲键盘` → **SH02** `…顶灯微闪、车流变化；文件柜前簇：翻页/接打电话/整理衣领`（环境+人群均跨镜不重复）｜✅ `后景 Practical 台灯灯丝微闪，杯口热气缓缓升腾`。
...
- 空间：七要素齐全 → **前后位置双轨（镜头+ENV/实体）** → ENV 已写 → **环境实体微动态（§七.7，树枝/灯光/车流等，BG 非完全静止）** → **含人群时双层并列（§七.7-b 完全随机、跨镜禁重复，禁只写人群漏环境）** → 语言逐字可检索（§六.1）→ 实体标准表达（§一.0.1）。
- `Video Content`：自然叙述 + P1/Pn → **只写画面最终结果** → **BG 环境实体微动态已写（§七.7，非完全静止）** → **含人群时 §七.7+§七.7-b 均已写** → **无** Beat 承接说明、规则推演、§/环节引用、Shot Logic 解释句 → **无** 360度拓扑/转角公式/empty_view_delta 等上游推导术语（§七.6 已转译为机位/顺时针回转/BG更替/可见面） → 无「上镜/承接上一镜/上一 Beat」代指。
