...
3. **景深层次与空间落位**：= 环节 2–3；写法见 §七.3；**每镜须显式写出 `ENV:[环境名]`**（§七.4、§八）；**背景不得完全静止**，须写 ≥1 项与 ENV 匹配的环境实体微动态（树枝摇动、灯光闪烁、车流变化等，§七.7）；**含人群簇时另须写 §七.7-b，二者并列不可互替**。
...
1. **对照范围 (强制全检)**：拆镜前逐 Beat 按**前置+六环节**核对：  
   ...
   - 另检：轴线与进出画路径、**P 段时序链是否单向无乱序/抢写、各 P 是否按角色组织且有主拍对象、跨 P 同框/距离/轴线是否承接、新入画角色是否写观察导向运镜（§八.2）**、**跨 Beat/跨镜具名角色画内可见性或面部可读性变化是否继承上游 §11 过渡动作（出画/入画/转身别脸/遮挡/视角切换/切 ENV 反打，禁同视角同站位凭空消失/出现或面部无过渡跳变）**、肢体接触、**Beat 动作/道具/ENV 逐句继承（§一.2，每个 PROP/修饰/层位节点是否在 Video Content 可检索）**、道具连续、**旁白/V.O. 是否显式写画内全员闭嘴（§六.4）**、口型/OS/V.O.、光色连续、特效相位、**环境实体微动态（§七.7，树枝/灯光/车流等，BG 是否完全静止）**、**含人群时是否双层并列（§七.7 环境实体 + §七.7-b 人群，禁只写人群漏环境）**、**背景人群是否完全随机且跨镜无重复描述（§七.7-b，≥2 种异质微动作，禁整簇同步木偶）**、**群演/背景人群是否禁中景/特写主拍（§三.1、§七.7-b，须远景/全景/虚化 BG）**。
...
0.1 **标准实体表达转换（强制，双源交集，完整继承 Stage 2-2）**：拆镜与写作前，逐条交叉核对 `# Scene Subject Index` 与 `# Core Scene Info`——凡双源均出现的实体，在 `Shot Logic (CN)`、`Video Content (CN)`、`Associated Entities` 中**必须**写为标准表达 `TYPE:[名称]`（角色 `CHAR:[@名称]`；环境 `ENV:[名称]`；道具 `PROP:[名称]`），名称逐字取自 Index，**禁止**使用 Core Scene Info 自然语言段、Adapted Script 残留称呼或本镜自造别名。说话人/听者/观察起点/观察目标/空间落位/环境交互/关键道具/环境切换目标等凡涉及已登记实体，一律加类型前缀并写 Index 原名。`Associated Entities` 列仅列本镜涉及的 `CHAR`/`PROP`/`ENV` 标准标签，与正文引用逐字一致。**群演/匿名背景人群**不在 Subject Index 中登记，**禁止**使用 `EXTRA:` 标签或自造 `CHAR:`；须用自然语言写数量、分布、景别、**完全随机差异化动作**（同一簇内 ≥2 种不同微动作，禁整簇同步定格；**同 Scene 跨镜禁止复用相同簇位+动作组合**，每镜须重新随机生成；如「后景文件柜前 2–3 名虚化办公人员：一人停笔抬头、一人侧身低语、一人继续敲键盘」），且不得形成可识别个体或新增 Index 外具名角色。输出前逐镜自检：正文或实体列若出现非 Index 名、未加前缀或 `EXTRA:` → 必须修正后再输出；**同场已写过的背景人群描述/动作组合若在本镜重复出现 → 须改写为全新随机组合后再输出**。
1. **实体与Beat隔离**：角色/道具/群演/场景原样复用；群演不添戏；**落实相邻 Beat 的离镜/入镜与可见性·面部可读性过渡（强制）**：上游 Beat 具名角色画内可见状态或**面部/口型/眼神可读状态**发生变化时，须完整继承 `[Beat切换说明]` 与 `{行为过程}` 中的出画/入画/转身别脸/遮挡/视角切换/切 ENV 反打动作；拆镜时该过渡须落在对应 Shot 的 P 段；**禁止**同视角同站位下上一镜面部可读、下一镜无过渡即不可读却写口型，或角色无过渡即消失/出现；已登记实体须按 §一.0.1 标准表达书写。  
...
1. **拆镜推演**：明确场次 -> 切分分镜 -> 确定实体出入画物理闭环（前一步到后一步如何转接）；**具名角色画内可见性或面部可读性变化**须对应上游 Beat 过渡动作，一 Beat 拆多镜时出画/入画/转身/反打过程不得删略。
...
2. **运镜与动作流**（写作前须按 Beat 六环节与 §三.4、§五、§六、§七 完成推演，正文**只输出画面结果**；**动作/道具/ENV 逐句继承见 §一.2，与 §六.1 对白同级**）：
   ...
   **P 段时序与角色空间链（强制，P1→P2→…→Pn/终段）**
   ...
   - **新入角色观察运镜（硬约束）**：角色**首次入画**、**从画外进入观察范围**或**本段才成为主拍/口型可读对象**时，**禁止** Static 硬切直给终态；须**显式写出运镜过程**（从 §5.6 择项：Push In / Pan / Track / Arc Orbit / Rack Focus / Reframe / OTS Switch / Pull Back→Two Shot / Follow 入画 / Eyeline Match+Reframe 等），将机位**导向可读该角色的合适观察视角**（中景/中近景/OTS/POV 等，§三.4），通常 **1.5–3s**；入画须写**从哪一侧/哪层进入、与已在画角色的距离如何建立**。
...
- **P 段时序与角色空间链**：P1→Pn **严格时序**无乱序/抢写｜每 P **按角色组织**有明确主拍对象｜跨 P 已写**同框/距离/轴线**承接｜新入画/新主拍角色有**显式观察导向运镜**（禁 Static 硬切）｜`P段时序链:`+`跨P空间关系:` 已写 Shot Logic。
- `Video Content`：自然叙述 + P1/Pn → **只写画面最终结果** → **BG 环境实体微动态已写（§七.7，非完全静止）** → **含人群时 §七.7+§七.7-b 均已写且群演为远景/虚化 BG** → **旁白/V.O. 已写画内全员闭嘴** → **无** Beat 承接说明、规则推演、§/环节引用、Shot Logic 解释句 → **无** 360度拓扑/转角公式/empty_view_delta 等上游推导术语（§七.6 已转译为机位/顺时针回转/BG更替/可见面） → 无「上镜/承接上一镜/上一 Beat」代指。
