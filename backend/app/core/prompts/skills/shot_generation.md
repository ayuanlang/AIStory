...
5. **全域光影执行规范（Mandatory · 每镜 Video Content）**
   - **与 §一.3A 联动**：光源动机、Practical 类型、窗光色温、材质反射与照度基调须服从五维转译（年代/地域/语言环境 → 可拍光学语义）；默认**宏大美观 + 柔和自然真实**照度，低照度例外须注明。
   ...
   - **最低输出量（「动态连续光影/焦点」+「光线连动弧光」两段合计）**：须覆盖下列 **9 项中的 7 项**——
     ...
     **(8) 材质与肤色光学响应**：真人须写侧光下面颊半影、鼻梁高光形状、T 区柔和高光（非油亮）；木/金属/玻璃/织物/皮肤在 Key/Fil 下的反射色温差异。
     **(9) 运镜中的光影连续**：Push In/OTS 切换/环境切换时，**光源方向、作用范围、投影、焦点与色温须连续或写清变化过程**；禁运镜后光影方向无理由跳变。
   ...
   - **Shot Logic 配套**：每镜 `Shot Logic (CN)` 须写 `光影继承:`（来源 Beat/上镜光向、本镜 Key/Fil/冷暖 rationale、**各光源作用范围摘要**、跨镜连续风险）+ `光源充足性:`（≥2 动机光源是否满足/低照度例外依据）。
   - **输出前光影自检**：单独阅读「动态连续光影/焦点」段，是否仍能回答——光从哪来？**每个光源作用范围照到哪里、哪里未照到**？产生什么投影/反射/条纹/体积光等**可见效果**？照亮谁/哪层？冷暖如何分层？若不能，须重写。（对话镜面部受光细则见 §六.3）
...
3. **对话布光（Mandatory，联动 §四.5-E）**：除恐怖/剪影设定外，对话必须写**具体光源清单（≥1 动机光）、Key 方向、色温、面部受光面与半影**，保证面部、口型、微表情可见；须写**柔和侧逆光 + 正面 Fill** 如何作用到说话人——如「BG 百叶窗 6500K 柔和侧 Key + 顶侧缘侧逆光分离轮廓，右前方白墙/桌面反射 3800K 暖调正面 Fill 补光，Lin 左颊与鼻梁受光、右颊半影自然柔散无硬边界、眼下纹理仍可读」；真人写实优先自然光/窗光/Practical + 环境反射；禁止无方向平光、硬闪或只写“柔光/电影感”。
...
3. **动态连续光影/焦点（Mandatory，§四.5）**：须按 **光源清单（≥2）→ 各光源作用范围 → 各光源可见效果 → 柔和侧逆光/Rim → Key 方向与 FG/MG/BG 各层受光 → 正面 Fill/环境反射 → 投影/半影/反射（柔散无硬边界）→ Fill/Back/Rim → 冷暖色温 → 材质/肤色响应 → 运镜中焦点与光向/作用范围连续** 顺序写清；随运镜写光源方向、景深、明暗、焦点流转；**Practical/霓虹/窗光/屏光/烛焰等可见光源须写微动**（闪烁、脉动、光斑漂移、光尘流动等，§七.7、§四），禁只写静态灯位或“自然通透”等抽象词。**每个动机光源须至少 1 句作用范围 + 1 句可见效果**。
4. **光线连动弧光（Mandatory，§四.5）**：说明**光源/色温对比如何服务当前情绪阶段**，并写清**光色如何作用到环境各层**——须含**作用范围变化或效果变化**（如 Push In 时 Key 仍来自左侧窗光、作用范围从半身扩至面部、Lin 左颊条纹光渐强、Chen 虚焦肩背退入窗光未覆盖的冷蓝暗区；MG 台灯暖高光稳定、BG 百叶冷光随微动轻晃）；禁只写“氛围感/张力升压”。光色变化可与环境微动联动（如霓虹频闪加剧紧张、百叶光斑轻晃缓和日常节奏）。
...
- **全域光影（§四.5）**：`光影继承:`+`光源充足性:` 已写｜Video「动态连续光影/焦点」+「光线连动弧光」**九项覆盖≥7**｜**§四.5-E 柔和侧逆光+正面Fill+柔散过渡无硬边界** 已写｜**光源≥2+每光源作用范围+每光源可见效果+FG/MG/BG 分层+冷暖+投影/反射** 已写｜对话镜面部受光可读（§六.3）｜ENV 相关镜照度充足或低照度例外已注明｜**夜景 ENV 剧情允许时已写自然主光/暖调覆盖/多人工覆盖网（§3.1.8）**｜**整体氛围柔和自然真实**。
