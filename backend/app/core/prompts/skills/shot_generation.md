...
4. **时长精确计算与强制范围控制 (4s-15s)**：必须根据涉及的 Beat 数量、剧情类型、节拍快慢要求、语言内容（对白/自白/旁白/画外音/内心独白）的长短确立合理的预估时长（以秒为单位计算并记录）。
   ...
   - **微表情分阶段耗时公式（新增硬规则）**：若同一镜头内的微表情被拆成连续阶段,必须按阶段累加而不是按单一结果句估时。默认至少拆为 `起始压抑/预备 -> 中段松动/失控 -> 最终落点` 三段；常态项目下每一段按 `0.375-0.75s` 估算,情感剧/文戏可放宽到每段 `0.75-1.5s`,动作/悬疑戏可压缩到每段 `0.25-0.5s`。若微表情与对白并行,应按“微表情总耗时”与“语言内容耗时”取 `Max(...)`；若微表情发生在对白前后形成明确停顿,则应串行累加。**例如**：“先抿嘴(0.5s) -> 再闭眼(0.5s) -> 泪水溢出并停住(0.75s)”在常态项目下至少记作 `1.75s` 的表演耗时,不能只算作一个 `0.75s` 的泛化反应。
   - **总时长推算参考**：
