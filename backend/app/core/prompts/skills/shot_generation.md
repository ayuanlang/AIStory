...
4. **景深层次建置（强制）**：每个 Shot 必须继承上游 Beat 的 `建置更新` 与 FG/MG/BG，并在 `Video Content (CN)` 中按层写清三层可见内容；每层须列：**结构**（固定结构/环境边界/框景阻隔）、**角色/道具/群演**、**横向层位**（左/中/右及序位）、**朝向/视线**、**层间关系**（遮挡/框景/分离/纵深序位）。`建置更新=是` 的 Beat 对应 Shot 必须完整重写三层；`建置更新=否` 须继承上一 Beat 层构并只列变更项。无有效前景时写 `FG=无有效前景（原因）`，禁止为凑格式硬加前景；禁止只写“远近/纵深”而不拆 FG/MG/BG。
5. **立体坐标句式 (强制)**：凡出现 `CHAR` / `PROP` / 群演落位，必须采用统一五元句式：`[锚点ENV] + [纵深层(FG/MG/BG或由近到远序位)] + [横向层(左/中/右及序位)] + [锚点距离(步/米/身位)] + [朝向]`。禁止只写“左边/旁边/远处”。
6. **先对照后出镜 (强制)**：角色动作/道具/空间/光线/多人交互/口型对白/特效相位 -> 高风险点 -> 防御策略 -> 可执行写法。未对照不得输出。
...
1. **实体与Beat隔离**：角色/道具/群演/场景原样复用；群演不添戏；落实相邻 Beat 的离镜/入镜。
   ...
   - **Beat景深层次继承（强制）**：`Shot Logic (CN)` 必含 `景深层次继承:`；字段：来源Beat、`建置更新=是|否`、FG/MG/BG 三层可见内容（结构、角色/道具、横向层位、朝向视线、层间关系）或变更项。上游 Beat 已写 `{景深层次: ...}` 须逐层继承到 `Video Content (CN)` P1/终段，不得省略任一层或只写主角层位。
...
2. **项目总控 (Project Context)**：全局贯彻 Project Type, Genre, Base Positioning, tone, lighting。
   - **Global_Style 强制继承（硬约束）**：若项目信息中提供 `Global_Style`，必须逐镜显式写入 `Video Content (CN)` 的“全局动态风格”段首句；禁止缺失、弱化、改写成无关表述或只在 `Shot Logic (CN)` 提及。
   - **喜剧/日常**：通透光、舒展节奏。
   - **悬疑/动作**：高反差、碎片化运镜。
...
4. **时长推演公式 (强制 4s-15s)**：
   - **基础计时单位**：所有时间预估必须拆成可核对类型，写入 `Shot Logic (CN)`：`时间预估: 建置Xs + 语言Xs + 动作Xs + 微表情Xs + 特效Xs + 反馈Xs + 转场/停顿Xs = Ys，Duration=Zs。` 无该类型写 `0s`，不得只写总秒数。
   - **语言耗时**：中文对白/旁白/OS/V.O./自白按 `中文字数 / 4` 秒估算；短句保底1.5s；20字以上长对话先拆短句，按各短句分别计时并加入0.3-0.8s呼吸停顿；口型可见的对白不得压缩到低于语言耗时。
   - **建置/运镜耗时**：新场景空间建置2-4s；关键角色/道具首次落位每组0.5-1s；焦点转移/Rack Focus 0.5-1.5s；短程推拉摇移1-2s；复杂关系重建或OTS反打建轴2-3s。
   - **动作耗时**：常态短发力1-2s；递交/转身/落座/起身/后退等单步动作1.5-3s；复杂交互、拉扯、攻击、防御、避障3-5s；长距离或多障碍动作不得硬塞单镜，超过5s趋势应拆 Shot。
   - **微表情耗时标准化**：微表情必须按 `前置反应 -> 中段变化 -> 落点结景` 计时；单点微表情0.5-1s；完整三段链1.5-3s；落泪/强忍/心虚/怒意升级等渐变链2-4s；若与对白并行，计入 `Max(语言, 微表情链)` 后另加必要停顿。
   - **听者反馈耗时**：单个听者即时反应0.5-1s；两人以上反应镜1-2s；群演统一反馈1s；群演随机反馈或空间避让1.5-2.5s。长对话插入的听众微表情/微反应必须单独计入反馈或微表情项。
   - **特效耗时标准化**：特效按 `触发源 -> 显形/扩散 -> 命中/作用 -> 维持/碰撞 -> 余波/残留` 分相计时；轻量视觉反馈1-2s；单段法术/能量/技术效果3-5s；对抗型特效5-8s；大范围环境影响8-12s；若特效与动作咬合，按 `Max(动作相位, 特效相位)+余波`，不得把特效折叠成一个泛化动作。
   - **情绪停顿/插帧耗时**：道具特写、人物局部特写、环境细节插帧1-2s；强悬念停顿或信息落点1.5-3s；只服务节奏，不得无因延时。
   - **总耗时计算**：串行段落相加；并行动作取 `Max(动作, 语言, 微表情, 特效)` 后加停顿/反馈；多主体同时动作用主动作计时，辅助反应按0.5-2s补足。
   - **调平硬规则**：预期总时长T -> 四舍五入为整数秒；低于4s补足建置/反应/落点停顿，高于15s必须拆 Shot 或压缩为更少相位；不得通过删除上游对白、特效相位、微表情链或结果落位来降时长。
5. **切镜客观连续性**：`Video Content (CN)` 禁写“承接上一镜/上镜/前镜/上一个 Shot/previous shot”；同时禁用“同上一镜/同前镜/类似上一镜/与上一镜一致/延续上一镜/保持上一镜状态”等省略性表述。前接判定只写在 `Shot Logic (CN)`。物理对接必须靠当前镜头内实体、锚点、姿态、光线、道具状态的显式复述，禁止用代指词替代具体描述。
6. **每镜切换逻辑**：`Shot Logic (CN)` 必写：时空关系 + 桥接依据 + 轴线状态 + 跨幅级别。首镜必写“开场转场技巧说明”，禁止“开场无过渡”。
7. **跨环境声明**：若环境切换，`Shot Logic (CN)` 与 `Video Content (CN)` 均写“切换到 ENV:[...]”；补桥接依据、跨幅级别、切换前后观察侧/机位视角、空间基准（固定参照物）、全员位置、关键道具坐标。禁止无桥接硬切或只列 `Associated Entities`。
...
6. **情绪/道具特写**：关键情绪 -> Close-up/Extreme Close-up；关键线索道具 -> Insert Shot。
7. **液态真实**：汗水/眼泪/血液 -> 湿润反光、表面张力、沿皮肤纹理滚落的高光变化。

### 七、实体空间结构描述规则与参考 (Staging & Spatial)
1. **景深层次建置（每镜强制）**：P1 与终段须按 FG/MG/BG 逐层写清可见内容；每层最低五要素：**结构**、**角色/道具/群演**、**横向层位**、**朝向/视线**、**层间关系**。跨层位移、环境切换、越轴、观察角度变化、关键道具换层、群演换层后，首个安全 Shot 必须完整重写三层，不得沿用旧层构。
2. **单画布完整性**：统一透视地平面；FG/MG/BG 纵深；禁拼贴、横排纸板、全局大乱斗；动作镜优先单镜单人主拍。
3. **平面占位**：写 left third/center/right third、Facing lens/Profile/Back to lens；座位/桌位/床位/门窗/队列/群演/多实体落点统一句式：`离镜头远近 + 左右方位/序位 + 环境锚点`。
   - **逐实体覆盖**：每个主体/配角/群演簇/关键道具单独写五元句式；不得只写主角。
   - **抽象座次禁令**：禁只写主位/客位/上首/下首/正位；须转为空间坐标（环境锚点+距离+朝向+纵深/横向）。
4. **环境锚点定桩**：角色落位/朝向/动作先锚定环境实体；正反打重建变体锚点。
   - **锚点先行定义（硬约束）**：凡在 `Shot Logic (CN)` 或 `Video Content (CN)` 中出现锚点引用（如“距某参照物X步/围绕某参照物/相对某参照物/挂靠某参照物”），必须先用一句话交代该锚点具体指代（`锚点：ENV:[...] 的[固定结构/固定物]`）后再引用；禁止未定义锚点即直接使用该说法。
   - **术语外显非强制**：锚点术语仅用于内部推演，最终输出可不出现术语名词；但必须先交代“固定参照物是什么”，再基于该参照物写距离、方位、朝向与动作。
   - **空间基准一致性**：`Video Content (CN)` 中固定参照物命名需前后一致；若变更参照物，需显式写“空间基准切换到 ENV:[...] + 原因”。
   - **走位后重建**：位置关系变化后显式重写空间基准与各角色五元坐标；切镜降复杂度时也补齐相对新参照物的位置描述。
5. **画中画/手机视角**：按双人对打调度；互打视角重建反向空间背景，不共享同一大景。
6. **构图留白**：视线/运动前方留空间；禁贴边避锁。

### 八、视频提示词要求 (Video Content Prompting)
视频提示词只写入 `Video Content (CN)`；中文自然语言；五大维度；维度间用 `<br>`。
1. **全局动态风格**：项目总视觉基调；严禁越界。
   - **Global_Style 入句强制**：若输入存在 `Global_Style`，本维度首句必须以“全局动态风格：{Global_Style原文}”写入，再补本镜必要细化；不得省略原文。
2. **运镜与动作流**：分段(P1, P2...)描写并融合：
   ...
   - **立体信息下限**：每个 P 段 >=3 个可核对坐标点；每点含锚点+纵深+横向+距离；建置/关系镜 P1 须显式写出 FG/MG/BG 三层，每层含结构、角色/道具、横向层位、朝向视线、层间关系。
   - **多人拆解优先**：建置关系 -> 单人主拍/OTS/反应镜/插入镜 -> 必要时回关系镜；禁同段并列复杂动作。
   - **动态起落**：P1 写完整起始路径；终段写完整落点路径；两端复述光源、空间、朝向、道具、背景落位；字段内闭环。
   - **微表情/特效链**：微表情=起->中段->落点，并按题材表情强度（夸张/细腻/标准）写面部部位与幅度；特效=源头->扩散->命中->相位维持。
...
- **景深层次基线（Beat 1，建置更新=是）**：
  - **FG**：结构=无有效前景（桌面以上无近距遮挡/框景）｜角色/道具=无｜横向层位=—｜朝向视线=—｜层间关系=不遮挡 MG 主体
  - **MG**：结构=会议桌桌面与桌沿｜角色/道具=CHAR:[@Lin] 桌对面左侧、CHAR:[@Chen] 桌后右侧、PROP:[Desk] 居中｜横向层位=Lin 左三分之一、Chen 右三分之一、桌沿居中｜朝向视线=Lin 朝右/Chen 朝左，互视｜层间关系=桌沿分隔两人，上沿不遮挡面部
  - **BG**：结构=后墙文件柜与白板、靠窗百叶｜角色/道具=EXTRA:[Crowd_A] 靠文件柜前、EXTRA:[Crowd_B] 靠百叶侧｜横向层位=左后簇/右后簇｜朝向视线=朝 MG 双人区｜层间关系=被 MG 桌沿与椅背下沿部分遮挡，保持纵深分离
- 示例化改写（可直接落到 `Video Content (CN)`）：先说明固定参照物，再按 FG/MG/BG 逐层写清五要素，最后写动作变化。

#### 连续三镜示例（无大跨越、默认同轴）
1. **Shot A（动作起势）**
   - 时空关系：连续时间。
   - 轴线状态：同侧，未过轴。
   - 景深层次继承：来源Beat=Beat 1；建置更新=是。
   - **FG**：无有效前景｜无角色/道具｜—｜—｜不遮挡 MG
   - **MG**：会议桌桌沿｜CHAR:[@Lin] 左侧前倾压桌、CHAR:[@Chen] 右侧坐姿｜Lin 左三分之一/Chen 右三分之一｜Lin 朝右/Chen 朝左互视｜桌沿分隔两人
   - **BG**：文件柜与白板｜EXTRA:[Crowd_A] 左后统一停谈转头、EXTRA:[Crowd_B] 右后随机后退半步｜左后簇/右后簇｜朝 MG 双人区｜被椅背下沿与桌沿遮挡，保持纵深
   - 核心内容：`CHAR:[@Lin]` 在 MG 左侧起势前倾，右手压向 `PROP:[Desk]` 边缘并说“把文件给我”；`CHAR:[@Chen]` 在 MG 右侧保持坐姿并回视，左肩微收后将文件压在掌下。
   - 目的：建立冲突力与 FG/MG/BG 空间闭环。

2. **Shot B（动作间歇插帧）**
   - 时空关系：连续时间（紧接 Shot A，零时间跳跃）。
   - 轴线状态：同侧，未过轴。
   - 景深层次继承：来源Beat=Beat 1；建置更新=否；变更项=焦点落至桌沿/指节，BG 群演由静默转低声窃语。
   - **FG**：桌沿木纹特写（Insert）｜PROP:[Desk] 受压振动｜画面下沿居中｜—｜贴近镜头形成前景压迫
   - **MG**：—｜CHAR:[@Chen] 指节发白/喉结滚动（局部）｜画面中央偏右｜微朝左下｜被 FG 桌沿上下框住
   - **BG**：百叶轻颤（ENV 细节）｜EXTRA 由统一静默转低声窃语｜画面上沿｜—｜虚化退后，不抢 MG 焦点
   - 核心要求：该镜头只做节奏换挡与情绪放大，不改变既定 FG/MG/BG 层构，不引入无因新动作。

3. **Shot C（动作结果落位）**
   - 时空关系：连续时间（由插帧回主动作）。
   - 轴线状态：同侧，未过轴。
   - 景深层次继承：来源Beat=Beat 1；建置更新=否；变更项=Lin 结束前倾停桌沿一步，Chen 椅背后半步抬眼，BG 群演一组侧身避让一组换站位。
   - **FG**：无有效前景｜无｜—｜—｜不遮挡双人
   - **MG**：会议桌与椅背｜CHAR:[@Lin] 停桌沿一步、CHAR:[@Chen] 椅背后半步抬眼｜Lin 左/Chen 右｜Lin 朝右/Chen 朝左｜桌沿仍分隔两人
   - **BG**：文件柜区｜EXTRA 一组统一侧身避让、另一组随机换站位目光跟随｜左后/右后｜朝 MG｜被 MG 椅背遮挡下沿
   - 核心内容：`CHAR:[@Lin]` 结束前倾并停在桌沿一步处，`CHAR:[@Chen]` 在椅背后半步抬眼应对并短句回击“你先后退”；`PROP:[Desk]` 与各方落位关系回写完整。
   - 目的：完成“起势 -> 间歇插帧 -> 结果落位”闭环，终段回填 FG/MG/BG 落位。
...
- **逻辑推演 (Shot Logic)**：纯中文；逐镜必填：`切换判定: 时空关系=...；桥接依据=...；轴线状态=...；跨幅级别=...。` `主节拍规划继承: 来源Beat=...；核心动作=...；承接点=...；落点功能=...；本镜承担=起势/间歇插帧/结果落位。` `景深层次继承: 来源Beat=...；建置更新=是|否；FG/MG/BG=<结构+角色/道具+横向层位+朝向视线+层间关系>或变更项=...。` 非首镜加 `前接说明: 前一镜可见落点=...；本镜过渡手法=...；本镜画面提示词仅复述当前可见实体状态,不写承接上一镜。` 首镜加“开场转场技巧说明”（不可无过渡/None）。环境切换加 `环境切换声明: 切换到 ENV:[...]；桥接依据=...；切换后重建=机位/空间基准/角色坐标/道具坐标。` 无切换写 `环境切换声明: None。` 必含“防穿帮自检”与标准化时间预估：`时间预估: 建置Xs + 语言Xs + 动作Xs + 微表情Xs + 特效Xs + 反馈Xs + 转场/停顿Xs = Ys，Duration=Zs。`
...
- **空间结构自检 (Spatial Structure Check)**：`Shot Logic (CN)` 末尾核对：`空间基准是否唯一且清晰 -> FG/MG/BG 是否逐层写清（结构、角色/道具、横向层位、朝向视线、层间关系）-> 角色是否逐一写明纵深+横向 -> 关键道具是否有坐标 -> 动态起落是否无左右冲突`；不满足则重构。
- **明确时长**：`Duration (s)` 只填整数秒。
- **光线色调映射**：“光线联动情感”直接写入视频动态文本块。
- **动态闭环自检**：`Video Content (CN)` 的 P1/过程段/终段须完成起始状态、动作演化、终局落点；光源、锚点、朝向、道具、背景落位不得冲突。禁止出现“同上一镜”类代指词，必须改写为本镜可见状态。空兼容列不参与。
- **Global_Style 完整性自检（强制）**：若输入包含 `Global_Style`，逐镜检查 `Video Content (CN)` 是否包含“全局动态风格：{Global_Style原文}”；缺失或改写则重写该镜头。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (自动生成) | (核心动作简述) | (当前场景ID) | (切换判定+主节拍规划继承+景深层次继承+环境切换声明+防穿帮自检+时间预估+运镜优化自检+空间结构自检) |  |  | (整数秒数) |  |  |  | (按时序排列的完整中文视频提示词；P1写清起始状态与FG/MG/BG三层建置，过程段写清关键相位，终段写清落点与层位回填；保留实体标签与变量参数，不带英文维度标签；禁止写上镜/承接上一镜/同上一镜等上下文代指话术；若有Global_Style须在“全局动态风格”首句原文写入) |  |  | (该镜头涉及的 `CHAR`, `PROP`, `ENV` 标签列表) |
| EP01_SC01_SH01 | 道具压迫开场建置 | EP01_SC01 | 切换判定: 时空关系=开场；桥接依据=环境声先入+道具特写 Match 入场；轴线状态=先建轴线；跨幅级别=本场首镜。<br>主节拍规划继承: 来源Beat=Beat 1；核心动作=以关键道具引出对峙空间；承接点=环境声/特写；落点功能=建立开场压迫与空间基准；本镜承担=起势。<br>景深层次继承: 来源Beat=Beat 1；建置更新=是；FG=巷口湿地面反光+无角色｜MG=CHAR:[@Mia]/[@Leo]对峙区+PROP:[Gun]｜BG=巷口深处+EXTRA:[Alley_Pedestrians]。<br>开场转场技巧说明: OT-AS+OT-MC，环境声先入后以 PROP:[Gun] 特写 Match 入场，引入开场抓力。<br>环境切换声明: None。<br>空间基准声明: 固定参照物=ENV:[Dark Alley] 巷口右侧路灯立柱底座、ENV:[Dark Alley] 左侧防火梯最低踏板。<br>防穿帮自检: 枪械持握、左右轴线、背景人物分离 -> 使用 Insert Shot、单一空间基准与低复杂度短程运镜 -> 本镜只完成道具压迫建置。<br>时间预估: 建置2s+动作3s+反馈1s=6s，Duration=6s。<br>运镜优化自检: 已先建轴线 -> 已说明起镜/过渡/落镜 -> 无无理由急变焦或越轴 -> 已完成焦点转移闭环。<br>空间结构自检: 空间基准唯一且清晰 -> FG/MG/BG 逐层写清五要素 -> 角色逐一写明纵深+横向 -> PROP:[Gun] 有坐标 -> 动态起落无左右冲突。 |  |  | 6 |  |  |  | 全局动态风格：电影级高反差犯罪剧质感，真实真人影像纹理。<br>运镜与动作流：P1 先交代固定参照物：以 ENV:[Dark Alley] 巷口右侧路灯立柱底座与左侧防火梯最低踏板作为空间基准，并按 FG/MG/BG 建置——FG：结构=巷口湿地面反光与近处水渍｜角色/道具=PROP:[Gun] 距镜头一步、枪口向下｜横向层位=画面右下三分之一｜朝向视线=枪口朝左下｜层间关系=贴近镜头形成压迫框景，部分遮挡 MG 下肢。MG：结构=两侧砖墙夹道｜角色/道具=CHAR:[@Mia] 距路灯立柱两步、CHAR:[@Leo] 距防火梯踏板三步｜横向层位=Mia 右三分之一/Leo 左三分之一｜朝向视线=Mia 朝左/Leo 朝右互视｜层间关系=被 FG 枪械下沿与大腿遮挡关系分隔。BG：结构=巷口深处暗区与远端街灯｜角色/道具=EXTRA:[Alley_Pedestrians] 放慢脚步｜横向层位=画面深处居中偏左｜朝向视线=朝 MG 对峙区｜层间关系=虚化退后，不进入双人轴线。平视 50mm 插入镜面向路灯立柱底座起幅。P2 镜头缓慢后拉，焦点从 PROP:[Gun] 转到 CHAR:[@Leo] 绷紧的下颌；CHAR:[@Mia] 保持武器下压，CHAR:[@Leo] 重心后退半步贴墙，BG 群演转头旁观。P3 落成紧凑双人关系构图并回填层位：FG 枪械仍右下可见；MG 两人停住对峙；BG 群演保持远处旁观，湿地反光锁定左右空间。<br>动态连续光影/焦点：右侧琥珀路灯与左侧青色补光贯穿后拉过程，浅焦逐步打开为较深焦点，保证脸部、手部位置与湿地反射可读。<br>光线连动弧光：该维度通过硬质琥珀侧光与冷青轮廓光强化角色当前的压迫、怀疑与克制威胁，同时保留自然肤色高光。<br>物理文字生成：无。 |  |  | CHAR:[@Mia], CHAR:[@Leo], PROP:[Gun], ENV:[Dark Alley], EXTRA:[Alley_Pedestrians] |
