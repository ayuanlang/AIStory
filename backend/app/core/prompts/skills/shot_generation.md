...
- `Video Content` (动态演变描述) 采取双语独立撰写，使用自然语言，禁止差分省略，划分为五大维度：
  ...
  2. **`[Chronological Camera & Action]` (按时间编排的运镜与动作流)**：此部分直接以 (P1), (P2), (P3) 等动作段落切分，将运镜与动作在同一时间线上融合描写。**绝不要把运镜和动作分开写！**
     ...
     - **Px 换行格式（强制）**：每个动作段落标记 `(P1)`、`(P2)`、`(Pn...)` 的**前后必须各插入一个换行**（使用 `<br>` 或 Markdown 换行），确保每个阶段独占一行，绝对禁止将多个 Px 段落连续写在同一行内，以保证输出的可读性。
     ...
     - **极度明确的主体与可见动作**：明确主体，避免单独代词。将动作拆解为具体的肢体发力、微表情变化。若是群体行为，杜绝同质化反应。
