...
1. **对照范围 (强制全检)**：拆镜前逐 Beat 按**前置+六环节**核对：  
   ...
   - 另检：轴线与进出画路径、肢体接触、**Beat 动作/道具/ENV 逐句继承（§一.2，每个 PROP/修饰/层位节点是否在 Video Content 可检索）**、道具连续、**旁白/V.O. 是否显式写画内全员闭嘴（§六.4）**、口型/OS/V.O.、光色连续、特效相位、**环境实体微动态（§七.7，树枝/灯光/车流等，BG 是否完全静止）**、**含人群时是否双层并列（§七.7 环境实体 + §七.7-b 人群，禁只写人群漏环境）**、**背景人群是否完全随机且跨镜无重复描述（§七.7-b，≥2 种异质微动作，禁整簇同步木偶）**、**群演/背景人群是否禁中景/特写主拍（§三.1、§七.7-b，须远景/全景/虚化 BG）**。
...
**正反/OTS 专规**
- 正反/OTS 须写同一空间锚点（桌中线/门框/门槛）+ **连续运镜过程**切换至对侧（**禁 Static 硬切**）；Shot Logic 写 180 度互补与可见 Delta；Video Content 写 **Arc Orbit / Pan / Track + Reverse/OTS** + 顺时针回转**过程** + BG **渐进**更替 + 椅背/门扇等重判 + **三分构图自然留白**（§七.7 构图留白）。
