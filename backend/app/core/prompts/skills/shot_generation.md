...
1. **实体复用原则**：上游会按场景将一系列的实体（角色、道具、空镜、以及可能被明确给出的群演/路人/同事/亲友等背景人物集合）的 beat 及对应的实体描述（已有参考图）传入,**绝对不要修改这些描述,仅做原样复用**。在分镜提示词里,尤其是 `Video Content` / `Video Content (CN)`,必须把这些上游已给出的角色、道具、环境与背景人物信息真正继承下来并写进动作段落,而不是脱离上游描述另起一套泛化表达。**若上游没有明确给出某类背景人物或群演存在,则不得擅自补写“路人很多”“同事围观”“亲友在后景”等内容。**
...
3. **切镜与过渡连贯（Shot切换策略）**：上下镜头如何切镜？上游【Beat切换说明】已明确包含了本镜头如何从上一镜过渡而来,以及其如何为下一个镜头进行“离镜”与“入镜”的铺垫。本阶段必须充分吸纳这些前后画面的详细关联描述（甚至包含用于跨场景运动与转场的专属“过渡Beat”）。**绝对不要抛弃这些过渡说明去盲目硬切**！在遇到“闪回”、“蒙太奇”等特殊剪辑手法时,必须将上游提供的前后镜关联作为绝对依据进行还原。你要在 `Shot Logic (CN)` 逻辑列中用**纯中文**推演清晰并写明你的切换策略依据,描述镜头间的离镜与入镜关系,明确所采用的**过渡手法**（参考下文"过渡手法参考库"）。**并且,你必须运用具体的运镜推拉、景别变化、实体的空间定点编排（Staging）以及光影构图等其他镜头语言,在相关帧的提示词与动态描述中实际地实现这些交接转场与桥接（例如：布置遮挡物游机穿越、刻意匹配相似图形的首尾帧构图等）的视觉呈现,绝对不能仅在逻辑列里嘴上说说。**
   ...
   - **特效相位连续线（新增强制）**：若相邻分镜涉及任何特效宿主实体（角色/道具/环境变体），除上述两条连续线外，还必须额外通过一条**特效相位连续线**：`上一镜特效相位与强度结果 -> 下一镜起始相位与强度起点`。必须明确说明该特效在切镜前后处于 `idle/charging/casting/expanding/sustained/impact/aftermath/dissipating` 中的哪一相，是否升阶、降阶、维持还是切换；并保持其主色、发光源、扩散方向、作用范围、受影响表面与二级反应元素连续。禁止上一镜还在蓄力、下一镜无交代直接爆完；也禁止上一镜已进入余波、下一镜又无故回到初始起势。
   ...
   - **站位写法模板（强制）**：每个分镜在 `Shot Logic (CN)`、`Staging & Spatial` 与 `Video Content` 中都必须按固定顺序表达 `前一镜站位/状态 -> 本镜动作承接 -> 本镜最终站位/状态`。若为 OTS / Reverse OTS,必须额外写明“轴线哪一侧、谁在前景、谁在后景、谁仍在画外但在场”；若涉及出画或局部入画,必须说明是由哪一个动作或遮挡物导致的合法变化,不能让角色或道具直接跳消。
...
4. **时长精确计算与强制范围控制 (4s-15s)**：必须根据涉及的 Beat 数量、剧情类型、节拍快慢要求、语言内容（对白/自白/旁白/画外音/内心独白）的长短确立合理的预估时长（以秒为单位计算并记录）。
   - **统一标准（新增硬规则）**：动作、语言、镜头转折停顿等所有基础计时口径,统一按旧标准执行；不再额外扣减比例；如与旧示例冲突,以本条和下列新数值为准。
   ...
   - **微表情过程计时义务（新增强制）**：凡镜头内存在压抑、迟疑、嘴硬、愣住、委屈、强忍、情绪失守、落泪、怒意回收、心软、惊惧等表演点,不得把微表情只当作一个瞬时结果计入 `基础神态/微表情反应`。必须先判断它是否包含多个连续阶段；只要存在“起始压住 -> 中段变化 -> 最终落点”三个环节中的两个及以上,就必须按**微表情过程**累计时长,并在 `Shot Logic (CN)` 里可回算地写出来。典型序列如“先抿嘴 -> 再闭眼/睫毛发颤 -> 泪水溢出”或“先盯住对方 -> 喉结轻滚 -> 嘴唇张了又收回 -> 视线失焦半拍”都不得压缩成 1 个总括反应时值。
   - **时长计算剧情差异化策略 (Genre-Specific Timing)**：必须根据上游设定的剧情类型调整时长算法以适配剪辑节奏。
      - **动作戏/悬疑戏 (Action/Thriller)**：动作频率高,剪辑节奏快。基础神态耗时调整为 `(0.5-1.0s)`,单次强力肢体切变调整为 `(1.0-2.0s)`。需留心运镜迅猛,分镜整体偏向碎片化短镜头（较多集中在 5s-8s 区间,借以堆叠局势紧张感）。
      - **情感剧/文戏 (Drama/Romance)**：注重情绪延宕,节奏舒缓。面部微表情、叹息或眼神交互的留白延长至 `(2.0-4.0s)`。应运用缓慢的运镜推进（如慢推、微移动）给足观众读取人物内心的空间,分镜整体时长偏长（较多集中在 9s-15s 区间,让情绪充分发酵）。
   ...
   - **语言内容耗时公式（统一口径）**：对白、自白、旁白、画外音（OS）、内心独白（V.O.）全部按同一公式计算；`中文字数 ÷ 4 = 语言内容耗时`（按平均正常语速折算为旧标准,极短语言句保底1.5秒；情感剧的语言停顿需额外 `+1.0-2.0s`）。
   - **动作耗时公式（以常态剧情为准,动作/情感戏视上述策略浮动调整）**：`基础神态/微表情反应 (1.0-2.0s)`；`简单肢体动作/短促发力 (2.0-3.0s)`；`复杂空间位移/多人交互动作 (4.0-5.0s)`。
   - **微表情分阶段耗时公式（新增硬规则）**：若同一镜头内的微表情被拆成连续阶段,必须按阶段累加而不是按单一结果句估时。默认至少拆为 `起始压抑/预备 -> 中段松动/失控 -> 最终落点` 三段；常态项目下每一段按 `0.5-1.0s` 估算,情感剧/文戏可放宽到每段 `1.0-2.0s`,动作/悬疑戏可压缩到每段 `0.33-0.67s`。若微表情与对白并行,应按“微表情总耗时”与“语言内容耗时”取 `Max(...)`；若微表情发生在对白前后形成明确停顿,则应串行累加。**例如**：“先抿嘴(0.7s) -> 再闭眼(0.7s) -> 泪水溢出并停住(1.0s)”在常态项目下至少记作 `2.4s` 的表演耗时,不能只算作一个 `1.0s` 的泛化反应。
      - **总时长推算参考**：
      - **串行执行**（做完动作再说话）：`动作耗时 + 语言内容耗时 + 镜头转折停顿(0.5-1.0s) = 总时长`。
      - **并行执行**（边动作边说话）：`Max(动作耗时, 语言内容耗时) + 镜头转折停顿(0.5-1.0s) = 总时长`。
         - **校验示例（必须可回算）**：某镜包含对白6字+画外音6字（共12字,中速）,语言内容耗时 `12 ÷ 4 = 3s`；动作为简单肢体 `2.0s`,并行执行取 `Max(2.0,3)=3s`,镜头转折停顿 `1.0s`,原始时长 `4s`；因已达到硬下限,写入 `Duration (s)=4`。
...
2. **方向性位移动词强制“起点->终点”描述（硬规则）**：凡出现“走过、跑过、走向、跑向、穿过、越过、绕过、退到、靠近、远离、进入、离开、冲出、拐入、穿行”等具有方向性的动作,必须显式写清“从哪个方向/位置/锚点实体旁 -> 到哪个方向/位置/锚点实体旁”。禁止只写“跑过走廊”“走向门口”这类无起止锚定的描述。强制句式可直接采用：`从 <起点方位或锚点实体> 出发,沿 <路径或方向> 移动至 <终点方位或锚点实体>`。
3. **全员动作描述约束（拒绝木偶背景）**：
   - 只要在画面出现的角色,必须有属于自己的动态描述或反应（视线跟随、肢体防卫、脚步挪动等）。如果画面中存在作为背景氛围的群演、群众、路人、舞者、士兵、服务员、乘客等背景人物,**也不能漏写其位置与动作**。但前提是: **这些背景人物的存在本身必须由上游输入明确支持,不得自行臆造。**允许不逐一命名个体,但默认应把他们写成**群体层**而不是若干漂浮孤点: 必须先交代这批背景人物整体**依附于哪个环境区域/锚点**（如前景桌边一排、后景街道一侧、门口等候区、吧台周围、长椅一列、走廊尽头一团）,再交代其**群体性分布关系**（散开/分层/成列/围绕/零散占位）与**和主体的空间关系**。原则上应以“虚化背景人物/失焦群众层/远景路人群”处理为主,仅保留可读的群体位置、环境关系与动作节奏,而不是抢主体细节。即使采用失焦处理,仍必须明确交代其**所在环境层级与大致分布位置**（如前景左侧桌边、后景门口两侧、远景街道中轴两旁）、**与主体的空间关系**、以及**带有随机性的可见动作**（如有人坐着交谈、有人在街上走着、有人停步回头、有人侧身穿行、有人低头整理物品、有人从画面边缘掠过）。禁止把群演写成完全静止的木偶背景,也禁止只写“背景有人”而不交代其在环境中的群体落位、环境依附关系与随机动态。
...
5. **微表情必须写成生成过程（新增强制）**：凡涉及哭、忍泪、心虚、嘴硬、迟疑、愣住、怒意回收、委屈、崩溃前强撑等表演点,不得只写“眼眶发红”“她哭了”“神情僵住”“露出难过表情”这类结果。必须把微表情拆成**连续可拍的生成过程**,至少写出一个前置动作、一个中段变化和一个最终落点。推荐写法为“先……,再……,最后……”或等价的分阶段句式,例如“先抿嘴,再闭眼,泪水才溢出”“先盯住对方,喉结轻滚,随后嘴唇张了又收回”。
...
- `Video Content` (动态演变描述) 采取双语独立撰写,使用自然语言,禁止差分省略,划分为五大维度：
   1. **`[Global Style]` (全局动态风格)**：在动作连贯的最前方重申全局视听风格,保证视频生成不脱离主基调（如 `cinematic, neo-noir film`）。**该维度必须首先服从上游 `Base Positioning` / `Global_Style` / `tone` / `lighting` 的项目级总控，而且必须显式将上游设定的项目视觉类型（Project Type，如：真人写实、3D动漫、2D等）带入到视频提示词中，绝对不能遗漏，确保最终生成的动态视频画面严格匹配该类型。不得把轻喜、治愈、现实温情类项目写成统一的阴冷 noir,也不得把恐怖悬疑项目写成明媚无压的广告化影像。**
  2. **`[Chronological Camera & Action]` (按时间编排的运镜与动作流)**：此部分直接以 (P1), (P2), (P3) 等动作段落切分,将运镜与动作在同一时间线上融合描写。**绝不要把运镜和动作分开写！**
     - **Px (Phase) 动作段落定义**：Px 代表该分镜内基于时间线推移的“连贯动作阶段”（Phase 1, Phase 2...）。每个 Px 代表一个不可忽略的动作转折或情绪演进。段落的数量（Px）应根据预估时长以及剧情综合决定,并不局限于两个。
          - **微表情过程写入义务（新增强制）**：若某个 `Px` 的核心是情绪变化、倾听反应、压抑、落泪、恐惧外露、关系松动或其他脸部表演推进,必须把该 `Px` 写成**微表情生成链**而非结果标签。推荐顺序为：`起始面部/呼吸状态 -> 某一局部先发生变化（嘴角/眼皮/喉结/鼻息/下颌/视线） -> 第二个连锁变化 -> 该 Px 结束时定格在哪个表情结果`。若该过程跨多个 `Px`,则每个 `Px` 都必须明确承接上一段已经走到哪一步,并说明本段推进到了哪一步。
       - **特效宿主相位写入义务（新增强制）**：若某个 `Px` 中存在特效宿主实体，必须在该 `Px` 内显式写出该实体当前的 `effect_phase`、强度变化、扩散路径与环境反馈，不能只写笼统结果词。推荐写法顺序为：`宿主动作触发 -> 特效从何处出现 -> 以何路径扩散/成形 -> 打到哪些表面/人物 -> 该 Px 结束时停在什么相位`。若特效在该 `Px` 中没有变化，也必须写清“仍维持上一相位”。
     - **Px 逐主体固定写法模板（新增强制）**：每个 `Px` 段落都必须按“**环境锚点与机位 -> 逐个角色 -> 逐个关键道具 -> 背景人物 -> 动作结果回填**”的顺序书写,不得跳项。凡是该 `Px` 中可见的剧情必要角色与关键道具,都必须至少回答清楚以下问题后才能进入下一主体：`它此刻位于哪个 ENV 锚点旁/哪一层(FG/MG/BG)/哪一侧(left third/center/right third)`、`身体或物体朝向哪里`、`头部或开口/尖端/屏幕朝向哪里`、`视线盯向谁/什么`、`双手或接触关系是什么`、`动作从哪里起到哪里止`、`该 Px 结束时它的新位置/新姿态/新持物状态是什么`。若该 `Px` 中有两个及以上主体,不得用合并句把他们糊成一团,必须逐主体依次交代。
     - **先站位后动作、先道具落位后道具动作（新增强制）**：在任何 `Px` 段落里,必须先交代角色与道具的当前落位和朝向,再交代它们发生的动作与运镜反馈。禁止直接写“CHAR:[@Name] 举枪逼近/拿起手机查看/转身离开”而不先写清其起始所在位置、朝向、与 `PROP:[Name]` / `ENV:[Name]` 的接触关系。
   - **双连续线写入义务（新增硬规则）**：在编写每个分镜的 `Video Content` / `Video Content (CN)` 时,必须把它当作连接 `Start Frame` 与 `End Frame` 的唯一动作桥梁,并显式写出该镜如何承接上一镜尾段、又如何把本镜尾段递交给下一镜。至少要能从文字中读出两条链：`上一镜视频提示词尾段 -> 本镜视频提示词起段` 与 `上一镜首帧经本镜运镜动作推进到尾帧 -> 下一镜首帧`。若某镜是场景第一镜或场景最后一镜,也必须分别对“承上”或“启下”中存在的一端保持可校验的状态闭环,确保最终满足“Start Frame + Video Content = End Frame”。
   ...
   - **微表情与时长联动写入义务（新增硬规则）**：只要某镜或某个 `Px` 写入了两步及以上的微表情过程,`Shot Logic (CN)` 的耗时拆解就必须把这些微表情阶段单独计入,不能在逻辑列里只写“大约停顿2秒”或把它们藏进笼统的“情绪反应”。至少要能从公式里看出该镜是否包含“抿嘴/闭眼/泪水溢出”“视线钉住/喉结轻滚/嘴唇收回”等阶段的累计时间。
   - **视频提示词的逐主体站位写实义务（新增硬规则）**：在 `Video Content` / `Video Content (CN)` 的每一个动作段落（P1、P2、P3...）中,凡是出现在画面里的角色与道具,都必须逐一写清其**所在景层（FG/MG/BG）**、**相对环境锚点的位置**、**相对画幅的位置**、**身体姿态**、**相对相机朝向**、**相对其他主体的朝向或接触关系**,并写出该段结束时是否发生变化。**这里的环境锚点、角色状态、道具关系必须优先继承上游输入的场景信息、实体信息与 Beat 描述,不得为了省事把上游已经给出的空间关系洗成抽象概括。**对剧情必要角色与关键道具而言,下列字段不再是“尽量覆盖”,而是默认必须完整回答,除非某字段在该主体上客观不存在：`站在/坐在/跪在/倚靠于哪个 ENV 锚点旁`, `位于 left third / center / right third 或前中后景哪一层`, `Facing lens / Profile Left / Profile Right / Back to lens`, `头部转向谁或哪一侧`, `视线盯向谁或哪个 PROP/ENV 锚点`, `双手各自接触或握持什么`, `重心压在哪只脚/哪一侧`, `PROP:[Name] 朝向哪里、贴近哪里、被谁以何种方式持有或放置`。禁止只写“二人对峙”“举着道具”“看向前方”“在桌边交流”等无法直接生成稳定画面的概括句。
   - **视频提示词的环境内落位强制义务（新增硬规则）**：视频提示词尤其不得把角色和道具写成悬空主体。每个动作段落里,都要把剧情必要角色与关键道具明确写在具体环境里,至少落到“靠近哪个 `ENV:[Name]` 锚点、处于环境的哪一层、与环境结构如何接触或保持距离、沿着环境中的哪条路径或方向移动”。如果上游已经给出更具体的场景或实体描述,必须优先沿用并展开,而不是改写成脱离环境的空泛动作描述。
   - **视频提示词缺项判废自检（新增硬规则）**：在写完每个 `Px` 后,必须逐一反问所有剧情必要角色与关键道具：`它现在在哪里? 朝向哪里? 接触着什么? 相对谁/什么而站? 该段结束后变成了什么位置和姿态?` 只要其中任一主体有一个问题答不出来,该 `Px` 就视为不合格,必须重写,直到所有必要主体都能被这组问题完整回答。绝不允许用抽象动作词、情绪词或运镜词掩盖位置描述缺失。
   - **群演/背景人物落位与随机动作义务（新增硬规则）**：若某个动作段落中存在群演、群众、路人或其他背景人物,则 `Video Content` / `Video Content (CN)` 里也必须交代其**在环境中的位置层级**与**可见动作**。但是否存在这些群演/背景人物,必须严格以上游输入为准: 上游明确给出才可以写,上游没有给出就不得为了补氛围而擅自添加。背景人物原则上默认按**失焦/虚化/低权重群体层**处理,除非上游另有明确要求；即便失焦,也必须先写清这批人**整体依附于哪个环境区域**（如前景桌边、背景街道、门厅两侧、柜台后方、长廊尽头）,再写清其**群体性分布**（成列、散开、围绕、分层、零散占位）以及其随机化动态（如有人在前景桌边坐着,有人在背景街上走着,有人停步,有人回头,有人抬手,有人低头整理物件）。禁止把群演拆成与环境脱节的抽象“有人影”,也禁止把一整群背景人物写成整齐划一的同步动作或全部静止。
   ...
   - **特效结果状态回填义务（新增硬规则）**：只要某个动作段落中发生了特效起势、扩散、对撞、维持、破裂、收束或余波残留,必须在该段结尾明确回填宿主实体的最新特效状态,至少包括：当前相位、强度等级、剩余可见发光/烟尘/粒子、仍被影响的表面、以及哪些二级反应还在持续。下一段与下一镜默认继承这组结果,不得重新发明。
   - **最后一个 Px 显式递交下一镜义务（新增硬规则）**：最后一个动作段落（最后一个 `Px`）中,必须**显式写出**“本镜如何切到下一分镜”,并留下一个可被下一镜首段直接继承的交接钉点。不能只停在当前镜头内部的收束,必须直接说明是通过哪一个动作完成、哪一个朝向保持、哪一个遮挡经过、哪一个构图重心、哪一个视线落点、哪一个机位运动方向或哪一种转场手法,把下一镜可承接的入口交出去。推荐句式：`以……作为离镜钉点/切出锚点,下一镜将由……承接`。若本镜是场景最后一镜或段落终镜,也必须显式写明“本镜在此收束,无下一分镜切入”。
     ...
     - **角色混合光肤色保护规则（硬规则）**：当某个动作段落里的 `CHAR:[@Name]` 同时受当前 `ENV:[环境名]` 中两种及以上可见光源影响,例如冷暖混合光、多光源环境光、霓虹反射、屏幕光叠加、雨夜反光、法器辉光或舞台复合光,必须在镜头描述中额外保护人物肤色与面部层次的自然度。至少要写清: 1) 哪一种光主导人物皮肤基色与正面塑形; 2) 另一种补充色光只落在边缘、半影、局部反射或轮廓区,不得把整张脸染脏; 3) 鼻梁、额头、颧骨、下巴等高光区域应保持可读的高光滚降与质感过渡,避免死白爆点、塑料反光、局部过曝或把皮肤纹理彻底洗掉; 4) 眼周、嘴角、鼻翼与口型区仍保留真实细节和微妙明暗,保证表演可读。目标是让人物与环境合成后的混合光既有电影级氛围,又维持健康、自然、可拍的肤色结构。
