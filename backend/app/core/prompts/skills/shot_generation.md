...
3. **景深层次与空间落位**：继承 Beat `建置更新` 与三层内容；`Video Content (CN)` **先**用前景/中景/背景建立三层框架，**再**在各层内自然语言逐个写清画内每个人物/物体的落位（位置、距离、朝向等），见 §七.1、§七.3；**每镜须显式写出 `ENV:[环境名]`**，见 §七.4、§八。
...
| 字段 | 写法 | 权威章节 |
| :--- | :--- | :--- |
| `Shot Logic (CN)` | 结构化字段：切换判定、**收束落幅判定**、**观察视角继承**、主节拍/景深层次继承、环境切换、**特写/中景/全景切换路径**、防穿帮自检、时间预估、运镜/空间自检 | §一、§二、§三.1、§零 |
| `Video Content (CN)` | 自然语言 + `P1/P2…` + `(Pn)` 对白；**必含 `ENV:[...]`**；前景/中景/背景（禁 `FG=` 字段体） | §八 |
| 景深层次五要素 | **先** FG/MG/BG 三层框架（结构+层间关系），**再**各层内逐个主体落位 | §七.1（`Shot Logic` 可用 FG/MG/BG 登记，`Video Content` 用自然语言） |
| 对白逐字继承 | `(Pn) … — Dialogue/…: "原句" — {听者反应}` | §六.1 |
| 实体落位五元 | 锚点 + 纵深 + 横向 + 距离 + 朝向；**画内每个可见主体逐个写清** | §七.3 |
| 标准实体表达 | Index 与 Core Scene Info 双源均出现的实体 → `CHAR:`/`ENV:`/`PROP:` + Index 逐字原名 | §一.0.1 |
| 位移五元组 | 起点（FG/MG/BG+左中右）→ 发力+立体方向 → 路径节点 → 终点 → 静止/受力；含行进/飞行朝向 | §五.2 |
...
1. **实体与Beat隔离**：角色/道具/群演/场景原样复用；群演不添戏；落实相邻 Beat 的离镜/入镜；已登记实体须按 §一.0.1 标准表达书写，不得因自然语言叙述习惯省略前缀或改用 Stage 1 称呼。
   ...
   - **Beat收束落幅继承（强制）**：逐 Beat 继承上游 `{对白组边界}` 与 `{下一节拍起幅}`（Stage 2-2 Beats generation 成稿）；拆镜写 `收束落幅判定` 时**优先引用**该两字段，不得与上游矛盾；上游缺失时再按 §三.1 从下一 Beat/下一镜内容推导，并在 `Shot Logic (CN)` 注明推导依据。
...
3. **每个新 Scene 三步建置（强制）**：凡新 Scene（含全剧第一场与场间切换后的每场），第1镜/前2镜须按 **先吸睛 -> 再建置 -> 再入戏** 完成环境建置，不得仅首场执行。必写：空间布局、**先 FG/MG/BG 三层框架，再各层内对每一个参与叙事的角色/关键道具逐个写清落位**（横向序位、距参照物距离、姿势、朝向、动作状态、与环境锚点关系）。暂未入画角色须在首次复杂动作前补建置。局部特写吸睛后须后拉/摇拍/鹤移或接全局建置补齐空间，再切入对白或主事件；禁止只建置主角而省略同场配角、关键道具或群演簇。  
4. **时长推演公式 (强制 4s-15s)**：
   ...
   - **建置/运镜耗时**：新场景空间建置2-4s；关键角色/道具首次落位每组0.5-1s；焦点转移/Rack Focus 0.5-1.5s；短程推拉摇移1-2s；复杂关系重建或OTS反打建轴2-3s；**景别切换运镜**（Push In / Pull Back / Track 等）0.5-2s，须排在对应 `(Pn)` 微表情/微动作落点之后（§三.1、§三.4）。**本镜末 Pull Back** 仅当 §三.1 判定须在本镜内完成且 Duration 余量 ≥2s 时全额计入；**下一镜 P1 切镜建置** 的 Wide/Two Shot 建置计入下一镜，不得在本镜重复计时。运镜与对白/动作同相位时并入并行核取 Max，不得全额另计。
...
2. **环境物理交互与方向性位移 (环境避障与空间法则 - 强制)**：
   - **动作交付**：先交代原始位置，再写落点。
   - **位移五元组**：`原始位置锚点（FG/MG/BG+左中右） -> 发力动作（发力部位+立体方向） -> 运动方向/路径（沿三轴变化节点） -> 终点落位（FG/MG/BG+左中右） -> 终点静止/受力结果`；**移动过程须同步写行进/飞行朝向与轨迹路径**，与上游 Stage 1 §11「移动类朝向与轨迹」一致。禁只写“走过去/来到/靠近/球飞出”。
   - **位置变化后二次建置**：起身/落座/逼近/后退/换边/绕位/出入门/进出前后景 -> 首个安全镜头**先**重建 FG/MG/BG 三层框架，**再**各层内逐个写清每个可见主体的横向、朝向、距离、关键道具关系；禁沿用旧坐标。
...
1. **景深层次建置（权威定义）**：P1 与终段须按**上游 Beat `Observer View` 所声明的观察视角**，**先**写清 **FG → MG → BG** 三层框架（每层：结构/边界/框景 + 层间关系），**再**各层内逐个写清落位；落位须满足：**当前视角全局可见**、空间逻辑一致、**身体朝向+视线落点**与**对白主听众/被点名对象**咬合（有对白 `(Pn)` 段必检）；禁止说话人背对主听众承载口型可读对白。`建置更新=是` 完整重写三层及各层内落位；`建置更新=否` 继承层构只写变更项（且须确认观察视角未变）；无有效前景须说明原因，禁止硬凑。跨层位移/环境切换/越轴/**观察视角变化**后，首个安全 Shot 须完整重写三层，并在 `Shot Logic (CN)` 写明视角—环境—建置变化原因。
2. **单画布完整性**：统一透视地平面；禁拼贴、横排纸板、全局大乱斗；动作镜优先单镜单人主拍。
3. **平面占位与实体落位五元**：`离镜头远近 + 左右方位/序位 + 环境锚点`；主体须**先归入 FG/MG/BG 对应层段**，再在该层段内**逐个**写全锚点+横向+距离+**身体朝向+视线落点（对白时指向主听众/被点名对象）**；禁跳过三层直接按人名罗列；禁只写主角而省略其他可见主体。
...
2. **运镜与动作流**（须符合 §三.4 高级运镜、§三.7 三段式、§五、§六、§七）：
   ...
   - P1 写起始状态 + **当前主场 `ENV:[环境名]`** + **先**前景/中景/背景三层框架、**再**各层内逐个主体建置（§七.1）；含口型可读对白的 Shot，P 段须按 §三.1 在中景/特写主拍；收束须按 §三.1 **收束落幅四档**与**执行优先级**（默认下一镜 P1 建置），**禁止**对白落点完成后机械 Pull Back。
   - 含语言时在对应 P 段写入完整原句 + 口型/闭口 + 听者反应 + 对话布光；口型可读对白须写运镜与说话人景别（§三.4）；旁白/画外 V.O./自白、**特效/宏观场面**按 §三.4 例外。
...
- `收束落幅判定:`（含对白镜必填）对白组是否完结=是|否（对齐上游 `{对白组边界}`）；上游下一节拍起幅=…；收束落幅四档=保持特写|维持中景关系镜|回全景建置|下一镜首建置；执行方式=本镜 Pull Back|下一镜 P1 建置|保持近景+桥接依据。
...
- 运镜：已建轴线 → 起镜/过渡/落镜 → 无无理由越轴/急变焦 → 焦点闭环 → **已判定对白类型（含特效/宏观例外）**，口型可读对白 P 段已写运镜+说话人主拍（§三.4） → **收束落幅判定已读上游 `{对白组边界}`/`{下一节拍起幅}` 且符合 §三.1 四档+执行优先级** → 景别无越级跳切（§三.1、§六.9）。
- 空间：基准唯一 → **先 FG/MG/BG 三层框架齐全** → **各层内每个可见主体是否均已逐个写清** → **当前视角下是否全局可见** → **静止朝向/视线是否与对白主听众咬合** → **移动类是否写清移动链（起点→行进朝向→轨迹路径→终点）** → 关键道具有坐标 → 动态起落无冲突。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (自动生成) | (核心动作简述) | (当前场景ID) | (切换判定+收束落幅判定+主节拍规划继承+景深层次继承+环境切换声明+防穿帮自检+时间预估+运镜优化自检+空间结构自检) |  |  | (整数秒数) |  |  |  | (连贯中文叙述体；运镜与动作流按 P1/P2/P3… 分段；**必须显式写出当前主场 ENV:[环境名]，P1 建置段必含**；对白用 (Pn) 格式，含对白 P 段须写运镜+说话人主拍（中景/特写），**收束落幅须读上游 `{对白组边界}`/`{下一节拍起幅}` 并按 §三.1 四档+执行优先级（默认下一镜 P1 建置）**；前景/中景/背景、锚点、运镜、动作等信息融入自然句子，禁止字段体；已登记实体保留 CHAR/PROP/ENV 标签，群演用自然语言禁 EXTRA；禁写上镜/承接上一镜等代指；若有 Global_Style 须在“全局动态风格”首句原文写入) |  |  | (该镜头涉及的 `CHAR`, `PROP`, `ENV` 标签列表) |
| EP01_SC02_SH01 | 对峙压桌综合示例 | EP01_SC02 | 切换判定: 时空关系=连续；桥接依据=同轴关系镜+全景→中近景递进；轴线状态=同侧，未过轴；跨幅级别=小跨幅。<br>收束落幅判定: 对白组是否完结=是（对齐上游 {对白组边界}=完结）；上游下一节拍起幅=全景建置（示例假定下一 Beat 起身离桌）；收束落幅四档=回全景建置；执行方式=本镜 Pull Back（路径 B 次选；生产优先下一镜 P1 建置）；桥接=Dolly Out/Pull Back 复写双人空间关系。<br>主节拍规划继承: 来源Beat=Beat 1；核心动作=Lin 前倾压桌索要文件，Chen 防守回应；承接点=会议桌对峙建置；落点功能=全景建置-对白主拍-反打-为下一镜调度预留空间落点；本镜承担=综合示例。<br>景深层次继承: 来源Beat=Beat 1；建置更新=是；前景以桌沿近距框景和手部遮挡形成压迫，中景是双人对峙与 PROP:[Desk] 的主关系层，背景由文件柜、百叶与虚化办公人群维持纵深。<br>环境切换声明: None。<br>对白覆盖: P1=Two Shot 中全景建置；P2=Lin 主拍 OTS→Micro Push In 中近景+微表情/微动作落点；P3=Chen 反打 OTS→Push In 中近景+末句微表情/微动作落点；P4=本组对白完结且下一镜需调度，Pull Back 回 P1 同级 Two Shot 中全景。<br>空间基准声明: 固定参照物=ENV:[Office] 会议桌右前角、ENV:[Office] 靠窗文件柜上沿外侧角点。<br>构图策略: P1/P4 三分构图双人关系；P2 引导线引向 Lin 口型；P3 反打近对称。<br>防穿帮自检: 双人轴线、口型对白、手部细节、群演反馈、光源连续 -> OTS 正反打+Push In 主拍+按下一镜需求回全景 -> 本镜完成双句对白收束且不越轴。<br>时间预估: 建置2s+语言3.5s+动作2s+微表情2s+景别收束2s+转场/停顿1s=串行12.5s；并行核=P2 Max(语言2,动作1,微表情1)=2s，P3 Max(语言1.5,动作1,微表情1.5)=1.5s（含末句说话/听者微表情微动作落点）；Duration=建置2s+P2并行核2s+P3并行核2s+P4收束2s+转场/停顿1s=9s。<br>运镜优化自检: 已先建轴线 -> 对白 P2/P3 已写运镜+说话人主拍 -> P3 末句微表情/微动作已落点 -> 收束落幅判定已对照下一镜 -> P4 已按判定 Pull Back 回全景 -> 景别切换无越级直切 -> 焦点闭环完成。<br>空间结构自检: 空间基准唯一且清晰 -> 前景/中景/背景五要素齐全 -> 角色逐一写明纵深+横向+距离+朝向 -> 关键道具有坐标 -> 动态起落无左右冲突。 |  |  | 9 |  |  |  | 全局动态风格：现实主义职场剧质感，自然通透光，真实真人影像纹理。<br>运镜与动作流：P1 Eye-level Two Shot 中全景起幅，镜头面向 ENV:[Office] 会议桌右前角，三分构图锁定双人对峙。前景是会议桌上沿与杯口虚焦形成近距框景，PROP:[Desk] 桌沿距镜头约一步、位于下沿中部；中景中 CHAR:[@Lin] 距桌右前角一步、位于左三分之一、朝右前倾压桌，CHAR:[@Chen] 距桌后缘一步、位于右三分之一、朝左端坐回视；背景中文件柜前左后簇 2–3 名虚化办公人员停谈转头，百叶侧右后簇 1–2 人后退半步，目光朝中景双人区。P2 镜头沿桌沿 Steadicam Glide 低速侧移并 Micro Push In，从 P1 中全景推近至 CHAR:[@Lin] 中近景主拍，CHAR:[@Chen] 以虚焦过肩占画左三分之一形成 Dirty Single，焦点锁定 Lin 面部、下颌与口型；(P2) {Lin 前倾压桌发声，Chen 闭口聆听防备} — Dialogue (CHAR:[@Lin]) (voice_type: 对白, tone: 压迫恳切, speed: 中速, volume: 正常): "把文件给我" — {CHAR:[@Chen] 左肩微收、视线不回避，左后簇统一停谈、右后簇低声窃语}；Lin 说完后下颌微绷、唇线落结景，Chen 左肩微收后静止，P2 微表情微动作落点完成。P3 镜头 Left-Shoulder OTS 反打，Track 微移半幅并对 CHAR:[@Chen] Push In 落幅中近景，聚焦 Chen 抬眼开口的面部与口型，Lin 以虚焦肩背占画右三分之一；(P3) {Chen 抬眼开口回击，Lin 闭口压桌倾听} — Dialogue (CHAR:[@Chen]) (voice_type: 对白, tone: 冷静克制, speed: 中速, volume: 正常): "你先后退" — {CHAR:[@Lin] 下颌微绷、视线不退，桌沿手部仍保持压势}；Chen 说完后唇角抿紧、视线定住 Lin 落结景，Lin 指腹收紧桌沿半拍后静止，末句微表情与微动作落点完成。P4 本组对白已完全结束，且下一镜需起身离桌调度，故 Dolly Out / Pull Back 从 P3 中近景退回 P1 同级 Two Shot 中全景，复写 ENV:[Office] 前景/中景/背景：CHAR:[@Lin] 停于桌沿一步外保持压桌，CHAR:[@Chen] 文件仍压在掌下、抬眼与 Lin 对峙，桌沿居中分隔双人，背景群演维持旁观与避让姿态。<br>动态连续光影/焦点：靠窗自然侧光为主、顶灯柔补为辅，光比连续；P2/P3 浅景深锁定说话人面部，P4 Pull Back 后焦点回稳至双人关系平面与三层空间。<br>光线连动弧光：靠窗冷白侧光与室内暖顶光对比，服务对白张力升压至为下一镜调度预留的空间落点。<br>物理文字生成：无。 |  |  | CHAR:[@Lin], CHAR:[@Chen], PROP:[Desk], ENV:[Office] |
