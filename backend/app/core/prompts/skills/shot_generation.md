...
5. **全域光影执行规范（Mandatory · 每镜 Video Content）**
   ...
   - **核心目标**：`Video Content (CN)` 须让 AI 视频模型明确「**光从哪来 → 照亮什么 → 产生什么效果 → 冷暖如何分层**」，不仅是“有电影感/自然光”，而是**可执行的光学链条**；涉及 `ENV:[...]` 的环境须**光源清晰、照度充足**（除非悬疑/恐怖/上游明确压暗）。
   - **最低输出量（「动态连续光影/焦点」+「光线连动弧光」两段合计）**：须覆盖下列 **7 项中的 6 项**——
     ...
     **(3) FG/MG/BG 光影分层与效果**：逐层写**受光面、半影、投影、明暗对比**——FG 框景明暗；MG 主舞台受光与桌下/脚下投影；BG 远层退暗/退冷/虚化与窗外光渗入；**须写光线作用产生的可见效果**（条纹窗光、桌下椭圆影、湿地面反光、金属冷高光条、体积光束等）；**半影须自然柔散、投影边缘无硬切线**。
     **(4) Fill/Back/Rim 与暗部可读**：补光来源（白墙反弹/环境反射/第二 Practical）；轮廓/分离光如何切开人物与背景；暗部是否保留肤质/材质纹理可读。
     **(5) 冷暖色温**：主光/Fil/阴影区色温（5500K 日光/3200K 钨丝/6500K 阴天/混合色温分区域）；至少 1 组**冷暖对照**或明确同温层次。
     **(6) 材质与肤色光学响应**：真人须写侧光下面颊半影、鼻梁高光形状、T 区柔和高光（非油亮）；木/金属/玻璃/织物/皮肤在 Key/Fil 下的反射色温差异。
     **(7) 运镜中的光影连续**：Push In/OTS 切换/环境切换时，**光源方向、投影、焦点与色温须连续或写清变化过程**；禁运镜后光影方向无理由跳变。
   ...
   - **即贴写法结构（可改写）**：`光源清单：{≥2 动机光源及锚点}。Key 从{方位}以{色温}照{MG/CHAR 受光面}，在{位置}形成{投影/半影效果}；Fill 来自{来源}托起{暗部}；BG {远层退冷/窗光渗入/霓虹点缀}；冷暖对照为{具体描述}；运镜中{焦点/光向}随{Push/OTS/切 ENV}如何变化。`
   - **Shot Logic 配套**：每镜 `Shot Logic (CN)` 须写 `光影继承:`（来源 Beat/上镜光向、本镜 Key/Fil/冷暖 rationale、跨镜连续风险）+ `光源充足性:`（≥2 动机光源是否满足/低照度例外依据）。
   - **输出前光影自检**：单独阅读「动态连续光影/焦点」段，是否仍能回答——光从哪来？照亮谁/哪层？产生什么投影/反射/冷暖效果？若不能，须重写。（对话镜面部受光细则见 §六.3）
...
3. **动态连续光影/焦点（Mandatory，§四.5）**：须按 **光源清单（≥2）→ 柔和侧逆光/Rim → Key 方向与 FG/MG/BG 各层受光 → 正面 Fill/环境反射 → 投影/半影/反射效果（柔散无硬边界）→ Fill/Back/Rim → 冷暖色温 → 材质/肤色响应 → 运镜中焦点与光向连续** 顺序写清；随运镜写光源方向、景深、明暗、焦点流转；**Practical/霓虹/窗光/屏光/烛焰等可见光源须写微动**（闪烁、脉动、光斑漂移、光尘流动等，§七.7、§四），禁只写静态灯位或“自然通透”等抽象词。**须写光线作用产生的可见效果**——窗格条纹光、桌下椭圆影、湿地面冷反射、金属高光条、体积光束、人物轮廓分离等。
4. **光线连动弧光（Mandatory，§四.5）**：说明**光源/色温对比如何服务当前情绪阶段**，并写清**光色如何作用到环境各层**——如「P2 Push In 时 Key 仍来自左侧窗光，Lin 面部暖侧光渐强、Chen 虚焦肩背退入冷蓝阴影区；MG 桌面台灯暖高光稳定、BG 百叶冷光随微动轻晃」；禁只写“氛围感/张力升压”。光色变化可与环境微动联动（如霓虹频闪加剧紧张、百叶光斑轻晃缓和日常节奏）。
...
- **全域光影（§四.5）**：`光影继承:`+`光源充足性:` 已写｜Video「动态连续光影/焦点」+「光线连动弧光」**七项覆盖≥6**｜**§四.5-E 柔和侧逆光+正面Fill+柔散过渡无硬边界** 已写｜**光源≥2+FG/MG/BG 分层效果+冷暖+投影/反射** 已写｜对话镜面部受光可读（§六.3）｜ENV 相关镜照度充足或低照度例外已注明｜**整体氛围柔和自然真实**。
