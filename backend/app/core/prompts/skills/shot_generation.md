...
2. **环境物理交互与方向性位移 (环境避障与空间法则 - 强制)**：
   - 动作与走位**必须充分且严谨地考虑环境空间信息**，交付动作前必须先交代清楚改动前的原始位置与落点。
   - 所有位移（跑向、走向、穿越等）不仅要显式写明起落点的环境锚点，还必须写出具体的轨迹，例如**如何绕过或跨越前方的障碍物**。
   - 涉及门窗、抽屉等开合交互时，**绝对必须指明物理方向（向外推开、向里拉开、侧向滑动等）**。
   - **【反面穿帮示例1 - 虚空瞬移】**：“他上一秒还在窗边，接着突然坐在了床上。”（未交代跨越房间的行走轨迹、转身与坐下的动作）。
   - **【反面穿帮示例2 - 忽略障碍/穿墙穿模】**：“他快步走向书桌。”（未交代如何避开去途中的茶几与椅子，仿佛直接穿过去）。
   ...
   - **【正确示例1 - 避障与位移】**：“他从沙发右侧起步，侧身绕过面前挡路的玻璃茶几，最终停在画面左后方的书桌前。”
...
7. **首尾帧七大维度排布（每个维度独立成行，必须使用 <br> 进行换行）**：
...
- **逻辑推演 (Shot Logic)**：纯中文推理。**强制连贯前接判定**：开头必须先明确写出紧承上一镜 `End Frame` 尾态，说明使用了何种运镜/走位【过渡手法】。**防穿帮自检 (Anti-Error Check)**：在此处必须用一句话简要列出本镜中需要重点防御的空间、物理、部位或道具穿帮风险点（例如：“注意：需交代如何绕过桌子，以及拔枪连贯动作，防止虚空位移和武器无中生有”）。然后需附带时间预估(如 P1()+P2()= )。首镜则需解说"开场抓力"。
