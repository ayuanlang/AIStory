...
1. **景深层次建置（权威定义）**：P1 与终段须按**上游 Beat `Observer View` 所声明的观察视角**，**先**用自然语言分别写清**前景、中景、背景**三层框架（每层：结构/边界/框景 + 层间关系），**再**在**每一层内**对该层每个可见主体**逐个**写清落位（横向序位、距参照物距离、朝向/视线、姿态/接触状态；禁止「中景两人/桌上道具」概括替代）。`建置更新=是` 完整重写三层及各层内落位；`建置更新=否` 继承层构只写变更项（且须确认观察视角未变）；无有效前景须说明原因，禁止硬凑。跨层位移/环境切换/越轴/**观察视角变化**后，首个安全 Shot 须完整重写三层，并在 `Shot Logic (CN)` 写明视角—环境—建置变化原因。
2. **单画布完整性**：统一透视地平面；禁拼贴、横排纸板、全局大乱斗；动作镜优先单镜单人主拍。
3. **平面占位与实体落位五元**：`离镜头远近 + 左右方位/序位 + 环境锚点`；主体须**先归入 FG/MG/BG 对应层段**，再在该层段内**逐个**写全锚点+横向+距离+朝向；禁跳过三层直接按人名罗列；禁只写主角而省略其他可见主体。
...
- 空间：基准唯一 → **先 FG/MG/BG 三层框架齐全** → **各层内每个可见人物/物体是否均已逐个写清**横向+距离+朝向 → 关键道具有坐标 → 动态起落无冲突。
