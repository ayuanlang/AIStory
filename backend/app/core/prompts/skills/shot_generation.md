...
- **Beat 完整逻辑**：⓪ 全体站位 ① **ENV 环境—视角匹配**+观察视角+**360度转角继承（衍生 ENV）**+**切换后建置可执行性** ② 三层+逐实体 ③ 运动方向与朝向+动作 ④ 对白咬合 ⑤ 微表演 ⑥ 承接+反馈。**Beat 绝对覆盖**：本镜 Beat 全部信息点已映射；动作/道具/ENV 逐句继承（§一.2）可检索，无泛化/删实体。**§一.3A**：项目语境+**风格定位**已写入全局动态风格段首｜画面细节特征>6。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| EPxx_SCyy_SHzz | (核心动作简述) | EPxx_SCyy | (项目语境继承+风格定位+Beat完整逻辑继承+切换判定+收束落幅+观察视角+360度转角继承+主节拍+景深层次+光影继承+光源充足性+运镜联想+特殊视角运镜+运镜选用依据+打斗运镜技法+特效攻击运镜+快慢节奏+高速跟拍技法+升格技法+宏观群体字段+环境切换+环境切换运镜+防穿帮+时间预估含快慢分项+空间自检) |  |  | (整数秒数) |  |  |  | (纯画面最终结果：全局动态风格含§一.3A项目语境+风格定位+细节>6；P1/Pn运镜动作对白微表演；ENV必写；BG环境实体微动态§七.7+含人群时§七.7-b双层并列；动态连续光影/光线连动弧光须§四.5七项≥6+光源≥2+FG/MG/BG分层效果；衍生视角写机位落点+顺时针回转+BG更替+方向性物体可见面；ACT/FPV/OPV/虫蚁/鸟瞰写机位高度尺度与切换过程；禁拓扑/度数公式/上游推导字段；禁Beat承接、禁规则推演、禁§/环节引用、禁字段体与Shot Logic解释句) |  |  | (CHAR/PROP/ENV) |
