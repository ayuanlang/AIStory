...
启动顺序：**Beat 完整逻辑六环节继承核对** → 穿帮对照 → 拆镜 → 写作。
**最高限制**：
1. **彻底继承**：强制继承上游输入的所有角色、道具、环境、背景人物及 Beat 信息，**禁止臆造**。
   - **Beat 完整逻辑六环节继承（最高优先级）**：完整继承 Stage 1 定义、Stage 2-2 `{Beats}` 工程化字段；拆镜与 `Video Content (CN)` **不得弱化、省略或改写**任一环节；细则见下文专节。
...
Stage 1 每个 Beat 按**六环节**成稿；本阶段**只继承、不重写** Beat，但拆镜与 `Video Content (CN)` 须完整可视呈现六环节。上游权威：`Core Scene Info` 的 `{Beats}`、`Observer View`、`{景深层次}`、`{微表情}`、`{微动作}`、`{行为过程}`、`{结果落位}`、对白字段。

| 环节 | Shot Logic (CN) 继承 | Video Content (CN) 继承 |
| :--- | :--- | :--- |
| **1 观察视角—环境—建置** | `观察视角继承:`（ENV、观察起点/角度/目标、视角变化、建置更新）；环境切换写 `切换到 ENV:[...]` | P1/终段先交代观察侧；`ENV:[...]` 必写；视角变则重建三层 |
...
| **6 连贯 + 全员反馈** | `前接说明`、承接点、Beat 间入出画 | 承接上镜/上 Beat 落点；群演统一/随机反馈；对白后听者反应再切 |

**拆镜前六环节核验（强制）**：逐 Beat 核对上游是否六环节齐全 → 本镜分配是否全覆盖 → 缺项标 `Shot Logic (CN)` 或拆镜补足。
...
| **摄影轴线/视线** | 运镜 | Eyeline Match、对视线、视线 Match；≠ 实体落位 |

**规则分工（避免重复阅读）**
| 字段 | 写法 | 权威章节 |
| :--- | :--- | :--- |
| **Beat 完整逻辑** | 六环节完整继承；拆镜前核验 | 上文专节 + §一、§七、§八 |
...
| 实体落位七要素 | 层位+左中右+距离+运动方向与朝向+视线/传感+动作方式+相对关系 | §七.3 |
...
1. **对照范围 (强制全检)**：拆镜前逐 Beat 按**六环节**核对上游与拟拆 Shot：  
   - **环节 1**：`Observer View` 与 ENV、前景/中景/背景、建置更新是否匹配；视角变是否重建三层。  
   - **环节 2**：三层内**每个可见实体**是否均有落位；禁合并省略。  
   - **环节 3**：三轴+运动方向与朝向+动作方式是否齐全；移动/载具是否含 FG/MG/BG+左中右 轨迹节点。  
...
1. **实体与Beat隔离**：角色/道具/群演/场景原样复用；群演不添戏；落实相邻 Beat 的离镜/入镜；已登记实体须按 §一.0.1 标准表达书写。  
   - **六环节分字段继承（强制，与上文专节表配套，禁止重复遗漏）**：  
     - **环节 1** → `观察视角继承:` + `[环境切换声明]` + `Beat环境切换继承`  
...
3. **层内逐主体七要素（权威，环节 2–3）**：与 Stage 1 §11 同口径，该层内每个可见实体各写一遍：  
   `标签 + 层位(FG/MG/BG) + 左/中/右 + 距参照物 + **运动方向与朝向**（身体/载具面向或行进轴向）+ **视线/传感落点**（分列，看谁/看哪）+ 动作方式 + 相对关系`  
   有 `(Pn)` 时另检**对白咬合**（环节 4）。禁止单独写「朝向/看向」而不锚定三轴。
4. **环境锚点定桩（`Video Content (CN)` 强制）**：每镜须显式写出 `ENV:[环境名]`（取自 `Observer View` 或 `{Env Anchor}`）；P1 建置段必含；环境切换写「切换到 ENV:[...]」+ 物理桥接。合规：`P1 … 镜头面向 ENV:[Office Front] 会议桌右前角 …`；不合规：缺 `ENV:[...]` 仅写结构部位名。
...
2. **运镜与动作流**（须完整呈现 Beat 完整逻辑六环节，并符合 §三.4、§五、§六、§七）：
   - **P1（环节 1+2+3 起幅）**：机位/景别 → `ENV:[...]` → **先**前景/中景/背景框架 → **再**各层内逐实体七要素 → 主节拍起势/建置。
   - **P2…Pn（环节 3–6 过程）**：运镜/动作/对白/微表演/反馈；含 `(Pn)` 时写环节 4–5；移动写轨迹节点 FG/MG/BG+左中右。
...
#### 推荐 `Shot Logic (CN)` 模板
- `Beat完整逻辑继承:` 来源Beat=…；本镜覆盖环节=1|2|3|4|5|6（逐项勾选）；缺口=无|…
- `切换判定: 时空关系=…；桥接依据=…；轴线状态=…；跨幅级别=…。`
- `观察视角继承:`（=环节1）来源Beat=…；当前ENV=…；观察起点/角度/目标=…；视角变化=…；建置更新=…
...
- **Beat 完整逻辑六环节**：① 观察视角+ENV ② 三层+逐实体 ③ 运动方向与朝向+动作方式/轨迹 ④ 对白咬合+情绪 ⑤ 微表演 ⑥ 承接+全员反馈。
- 运镜：轴线 → 起镜/过渡/落镜 → 对白运镜+收束落幅（§三.1、§三.4）→ 景别无越级。
- 空间：七要素齐全 → ENV 已写 → 语言逐字可检索（§六.1）→ 实体标准表达（§一.0.1）。
