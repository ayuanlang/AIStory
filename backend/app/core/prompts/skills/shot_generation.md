...
3. **全员动作描述约束（拒绝木偶背景）**：
   - 只要在画面出现的角色,必须有属于自己的动态描述或反应（视线跟随、肢体防卫、脚步挪动等）。如果画面中存在作为背景氛围的群演、群众、路人、舞者、士兵、服务员、乘客等背景人物,**也不能漏写其位置与动作**。但前提是: **这些背景人物的存在本身必须由上游输入明确支持,不得自行臆造。**允许不逐一命名个体,也允许以“虚化背景人物/失焦群众层/远景路人群”这类方式降低细节权重,但仍必须明确交代其**所在环境层级与大致分布位置**（如前景左侧、后景门口、远景街道中轴两侧）、**与主体的空间关系**、以及**带有随机性的可见动作**（如随机走动、有人停步回头、有人侧身穿行、有人低头整理物品、有人缓慢交谈、有人从画面边缘掠过）。禁止把群演写成完全静止的木偶背景,也禁止只写“背景有人”而不交代其在环境中的落位与动态。
...
- `Video Content` (动态演变描述) 采取双语独立撰写,使用自然语言,禁止差分省略,划分为五大维度：
  ...
  2. **`[Chronological Camera & Action]` (按时间编排的运镜与动作流)**：此部分直接以 (P1), (P2), (P3) 等动作段落切分,将运镜与动作在同一时间线上融合描写。**绝不要把运镜和动作分开写！**
   ...
   - **群演/背景人物落位与随机动作义务（新增硬规则）**：若某个动作段落中存在群演、群众、路人或其他背景人物,则 `Video Content` / `Video Content (CN)` 里也必须交代其**在环境中的位置层级**与**可见动作**。但是否存在这些群演/背景人物,必须严格以上游输入为准: 上游明确给出才可以写,上游没有给出就不得为了补氛围而擅自添加。允许使用“虚化”“失焦”“远景”“低权重背景人物”等表达降低细节,但必须至少写出这些背景人物位于环境的哪一侧、哪一层、围绕哪个 `ENV:[Name]` 锚点分布,以及其随机化动态（如零散走动、交错穿行、有人停步、有人回头、有人抬手、有人低头整理物件）。背景人物的动作应带有自然随机性,不可整齐划一,也不可全部静止。
...
- **Video Content**：每个动作段都要显式继承上游场景/实体/Beat 输入,逐主体写清角色与道具在具体环境中的位置、姿态、朝向、接触关系与状态变化,同时写出动态光线与角色发展的映射；若存在群演/背景人物,示例中也必须写出其环境层级、低权重落位与随机动作
...
BG: 若上游明确存在群演/背景人物,写清其环境层级、分布位置与随机小动作；若上游未给出,则此项不写。
