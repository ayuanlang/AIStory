...
- `anchor_description` 为单个英文字符串，逗号连接 3-5 个高密度短语（优先空间结构、主舞台、核心材质、固定光源/物件）；**禁止** JSON 数组；禁瞬态光影或人物动作。
...
### 3.2 对话正反打与 OTS
- OTS/正反打/明确多角度同场时，必须逐字符继承上游 Subject Index 中的衍生环境名；衍生环境统一为 `{角度}度{主环境名}` 格式，禁止改写为 `_OTS_A`、`_OTS_B`、`A面/B面` 或其他编号名。
- OTS/正反环境仅呈现当前观察方向的物理对立结构，并回扣主环境共同锚点；当前视角只改变可见背景、阻隔物、光源方向、通道与构图 Delta。
