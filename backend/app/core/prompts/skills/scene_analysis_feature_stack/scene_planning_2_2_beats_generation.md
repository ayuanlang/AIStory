...
- **站位表达**：先归属当前环境（主或衍生），再按**当前 Beat 观察视角**描述；**须继承 Stage 1 人物间戏剧关系所决定的距离、左右/前后序位、高低差与隔障**，不得仅写几何坐标；进入衍生环境后不得继续套用主环境 0 度；观察视角变化须与 FG/MG/BG 建置同步继承，禁止只继承微表情/局部特写而遗漏视角与环境。  
...
- **景深层次建置**：每个 Beat 必须按**该 Beat `Observer View` 所声明的观察视角**写清 **FG / MG / BG** 三层可见内容，每层须列：**结构**（固定结构/环境边界/框景阻隔）、**角色/道具/群演**、**横向层位**（左/中/右及序位）、**朝向/视线**、**层间关系**（遮挡/框景/分离/纵深序位）。必须继承 Stage 1 的 `建置更新=是|否`；`建置更新=是` 时完整继承 FG/MG/BG 三层（含视角变化后的重建说明）；`建置更新=否` 时继承 `{景深层次: 继承上一 Beat；变更项:<...>}`，且须确认观察视角与三层构成未变。禁止省略三层或只写“远近/纵深”。  
...
- **{观察视角与空间建置}**：继承当前环境站位表达式、**逐 Beat 观察视角（起点/角度/目标/环境）**、观察基点、视角角度、层位、锚点距离、空间约束、朝向视线、辅助锚定、环境切至/返回；视角变化须与建置更新判定一并继承。
