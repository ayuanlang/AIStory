...
# Prompt Updated At: 2026-06-28 16:00:00 +08:00
...
- **继承范围**：剧情结构、Scene/Beat 边界（**含时间切分边界，不得因时长目的改动**）、Stage 1 全部 Scene 级字段、Beat **前置+六环节**（细则 Stage 1 §11）、对白/OS/V.O.、对白拆句判定、对白组边界、下一节拍起幅、动作链、微表情/微动作/微反应、细节特写、心理可视化、主/衍生环境、站位表达式、Beat 建置更新判定、**速度/节奏/语速（speed/tone/volume）、停顿与维持时长、闪回/转场/首节拍时间上限等一切时间约束**；**武戏/决斗/术法 Beat 须完整继承 Stage 1 逐 Beat 观察侧（OTS/正反/POV/Two Shot/Insert）、FG/MG/BG 背景层变化、各参战者朝向四要素、双人/多人入境出画、环境切换声明与建置重建**，禁止只继承招式/特效名而丢视角—环境—建置三联。
...
- **观察视角—环境—建置（环节 1）**：`Observer View` 与 `{景深层次}`、`{空间}`、`[环境切换声明]` 同拍咬合。落 `Observer View` 前须环境—视角匹配自检（OTS/正反/POV/门窗内外/武戏攻防读招须切衍生环境）；错用主环境 → `{覆盖核销}` 标 `上游环境—视角错配`。须完整继承 Stage 1 已写 ENV、观察起点、角度、目标、可见边界、**相对上一 Beat 观察侧变化**；**武戏/决斗/术法 Beat 须继承 Stage 1 各 Beat 的 BG 层变化、朝向四要素、双人入境/出画**；缺失只标缺口，**禁止补充**。`建置更新=否` 时不得实质视角/环境变化（除非 Stage 1 明确仅层内微动）。
...
- **输出前自检**：Beat数/顺序=Stage1；决战≥10(§25)｜宏观≥6+三要素(§26)；对白/动作/时间/环境/FG-MG-BG/Equivalent Duration；**武戏/决斗/术法场逐 Beat 核对：观察侧+BG 变化+朝向四要素+入境/出画+ENV 切换是否均已继承**；**逐条核对：Core Scene Info 与表格各列中每个 CHAR/ENV/PROP 名称是否均等于 Index 某行 `subject_name_zh`（零自创、零 Stage 1 原名残留）**；实体命名=Index一致；缺口标`{覆盖核销}`。
...
- **{主环境}**：继承 Stage 1【主环境】**空间骨架**；Index 中 `literary_atmosphere` 等文学氛围**仅读继承**，Beat 成稿**禁止**补写；环境实体名**仅填** Index 原名。

- **{Scene实体覆盖}**：继承 Stage 1【Scene实体覆盖】可见主体清单与初始建置；实体名转为 Index 标准表达。

- **{观察视角与空间建置}**：继承 Stage 1 环节 1–2 + §11；视角变化与建置更新一并继承。

- **{衍生环境}**：继承空镜差异、依赖、**`view_angle_from_main` 转角 N 及转角对照（衍生 0 度=主环境 N 度）**、触发、边界；实体名仅填 Index 原名；无则保留 `无：否决证据`；有则同步 `Environment Name`/`Environment Relation`。
