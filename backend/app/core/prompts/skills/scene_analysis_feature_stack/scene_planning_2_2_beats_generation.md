...
# Prompt Updated At: 2026-07-17 07:30:00 +08:00
...
**【输入范围（强制）】**本环节收到的剧本正文**仅含**该场全部 `[BEAT_START:{n}]`…`[BEAT_END:{n}]` Beat 块（及 Scene ID 起止标记）。**不**接收 Stage 1 Scene 级【主环境】/【衍生环境】/【观察视角与空间建置】等说明块——此类字段不在输入中，表格对应项写 `None`，**禁止**补创或从 Index 回填。
...
**【禁止】**改写/概括/补创剧情或建置；改 Scene/Beat 数量/顺序/边界；对 Index 无行实体套任何类型前缀（含自创 `EXTRA:` 等）；用 Index 元数据回填 Stage 1 未写字段；缺项除标 `{覆盖核销}` 外的回填/重算/补建；补写输入中不存在的 Scene 级【】说明块。

> **职责边界**：Beat 创作、Scene 切分、环境拓扑、ENV/PROP 归属、`project_visual_backfill`、Scene 级【】说明块等归上游；本环节只转译落表本场 Beat。

## 硬约束

### 输入

- **Stage 1 单场 Beat 块**：内容唯一源（只读）。形态：
  ```
  [SCENES_BLOCK_START]
...
- Scene/Beat 数量/顺序/边界以输入 Beat 块为准；同 Scene 全部 Beat 写入同一 `{Beats}`，**保留** `[BEAT_START:{n}]`…`[BEAT_END:{n}]` 与 `────【建置】────` / `────【入戏】────` / `────【Beat切换说明】────`。
- 禁因 Index 更全回头改 Stage 1 / 补 Scene 级【】块。
...
Beat 数/顺序=输入｜编号规范｜Beat 正文原样落入（含分隔符）｜叙述层仅实体名 Index 化｜**台词无 CHAR/ENV/PROP**｜全部锚点可追溯 `subject_no`｜CHAR/PROP 已做衍生族匹配（无明文→基础版）｜每个 `ENV:[]` 为 Index 整场行且 Beat 已写｜输入未提供的 Scene 级【】字段为 `None`｜缺口已标｜无新增 Beat 未写实体/情节。

## Core Scene Info 与 Part 1: Scenes Table

每 Scene 一行。本环节**主责**是 `{Beats}`；其余 `{字段}` 若输入未提供 → 写 `None`（禁止补创）。

| 字段 | 规则 |
| :--- | :--- |
| `{Beats}` | **强制**：输入全部 Beat 块原样落入（仅叙述层实体名 Index 化；保留 `[BEAT_START:{n}]`…`[BEAT_END:{n}]` 与内部分隔符；台词原样） |
| `{故事内核}` / `{节拍时间规划}` / `{Duration Estimate Basis}` / `{决战 Beat 规划}` / `{宏观 Beat 规划}` / `{细节特写规划}` / `{Scene识别}` / `{主环境}` / `{衍生环境}` / `{Scene实体覆盖}` / `{观察视角与空间建置}` / `{场景切换与首节拍转场}` | 输入未含对应【】块 → **`None`** |
...
| `Environment Name` / `Linked Characters` / `Key Props` | 仅从本场 Beat 可核销实体汇总；无则 `None` |
