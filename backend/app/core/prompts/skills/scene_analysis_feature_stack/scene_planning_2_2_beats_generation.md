...
**逐 Beat 快检**：视角变化 → 落 Index 视角衍生 ENV（`Observer View`、`[环境切换声明]`、`Environment Name` 与 Stage 1 一致）；角色/道具态变化 → 落对应衍生 CHAR/PROP；Stage 1 写切换但 Index 缺行 → 标缺口；Stage 1 写 `衍生环境=无：否决证据` → 可写 `None`。**环境块快检**：【主环境】/【衍生环境】/`derivative_view_360_entities` **禁人物/人称/站位/交互**（仅空椅/空桌布局）；同一具名固定实体**跨衍生世界朝向一致**（状态/特效或明示受力移位除外）。
...
Beat 数/顺序 = Stage 1｜`Episode ID`/`Scene ID`/`Scene No.` 规范且与 Stage 1 场序一致｜**Stage 1 说明性字段全集已逐条落入 Core Scene Info、无省略写法**｜语义 = Stage 1（仅实体 Index 化）｜衍生选用与 Stage 1 观察/状态一致｜**衍生四向实体与主环境固定清单无矛盾、同等具体度；方向性物体成稿须锚点/视角表述、禁度数回转**｜全部 CHAR/ENV/PROP = Index 逐字一致｜时长已复核｜缺口已标 `{覆盖核销}`。
...
| 【衍生环境】 | `{衍生环境}` | **须含 `0度{主环境名}` Master**；**每条衍生 ENV 独立写** `derivative_view_360_entities`（四向自然语言，每段 **【{N}度方向】仅用本衍生自有角度**；**只写本机位推导结果**（同 Stage 1 §11）；**方向性物体成稿须锚定具名门/窗/桌/墙段等，锚点不可见时用相对镜头/Observer View 视角表述**——**禁**成稿写度数回转/公式/角度镜像重判；实体须与主环境固定清单逐字一致；**固定实体世界朝向跨衍生须一致**（状态/特效或明示受力移位除外）；每实体含 FG/MG/BG+垂直上/中/下+开闭态+锚点朝向/正反面）；**FG/MG/BG 纯空间层次**；**`spatial_axis`**、**`lens_profile`**、**`axis_crossing`**；**禁人物/人称/站位/交互**；另有转角 N、触发、空镜差异；无则 `无：否决证据` |
...
| `{主环境}` | 继承 Stage 1【主环境】**全文**（含俯视/仰视 360° 等基准定义）；环境实体名 Index 化；**注明不可直接引用**；**禁人物/人称/站位/交互** |
