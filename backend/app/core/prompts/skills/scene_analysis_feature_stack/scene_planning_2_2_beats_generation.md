...
# Prompt Updated At: 2026-06-20 22:00:00 +08:00
...
- **站位表达（先三层，再层内逐个）**：先归属当前环境（主或衍生），再按**当前 Beat 观察视角**，**先写 FG/MG/BG 三层框架（结构+层间关系），再在各层内对每一个可见人物与物体逐个**写清站位（横向序位、距参照物距离、朝向/视线、姿态/接触状态）；**须继承 Stage 1 人物间戏剧关系所决定的距离、左右/前后序位、高低差与隔障**，不得仅写几何坐标或笼统关系句（如「两人在桌边」）；进入衍生环境后不得继续套用主环境 0 度；观察视角变化须与 FG/MG/BG 建置同步继承，禁止只继承微表情/局部特写而遗漏视角与环境。  
...
- **动作字段**：`{行为过程}`/`{环境交互}`/`{结果落位}` 仅转换 Stage 1【动作/视觉节拍】；须继承 `主节拍/主动作` 与完整因果链（施力/接触/受力反馈不得与结果轨迹/落位脱节）；**主节拍归属主动施力方**，被动物件轨迹/落点/碰撞须写入施力方同一 Beat，**禁止**为客体单独新建 Beat。位移动作保留 `原始位置锚点 -> 发力动作 -> 运动方向/路径 -> 终点落位 -> 终点静止/受力结果`。
...
- **{动作/视觉节拍}**：继承主节拍/主动作（归属主动施力方；转场专拍时主节拍=转场桥接；客体运动不独立成 Beat）、**观察视角—环境—建置三联**、**微表情**（部位+动态过程链）、微动作、细节特写、完整因果链（施力/接触/受力反馈与结果轨迹/落位同链）、动作链、环境与视角状态、落点、朝向、视线、阻隔、切换承接、建置更新判定、FG/MG/BG；后续 Beat 继承上一 Beat 当前环境状态、观察视角与层位（视角未变时）。
