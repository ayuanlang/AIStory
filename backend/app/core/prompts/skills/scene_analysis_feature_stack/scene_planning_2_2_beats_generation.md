...
Beat 数/顺序 = Stage 1｜`Episode ID`/`Scene ID`/`Scene No.` 规范且与 Stage 1 场序一致｜**Stage 1 说明性字段全集已逐条落入 Core Scene Info、无省略写法**｜语义 = Stage 1（仅实体 Index 化）｜衍生选用与 Stage 1 观察/状态一致｜**衍生四向实体与主环境固定清单无矛盾、同等具体度、镜像映射对齐**｜全部 CHAR/ENV/PROP = Index 逐字一致｜时长已复核｜缺口已标 `{覆盖核销}`。
...
| 【衍生环境】 | `{衍生环境}` | **须含 `0度{主环境名}` Master**；**每条衍生 ENV 独立写** `derivative_view_360_entities`（四向自然语言，每段 **【{N}度方向】**；**实体须与主环境固定清单逐字一致、同等具体度，逻辑严格匹配，仅观察角度做各向镜像映射对齐（可见/不可见可变），禁止增删改矛盾或简化实体描述；每实体含 FG/MG/BG+垂直上/中/下**）；**FG/MG/BG 纯空间层次（每层每实体须写清上/中/下）**；**`spatial_axis`**、**`lens_profile:Wide|Standard`**、**`axis_crossing`**；另有转角 N、触发、空镜差异；无则 `无：否决证据` |
...
| `{主环境}` | 继承 Stage 1【主环境】**全文**（含俯视/仰视 360° 等基准定义）；环境实体名 Index 化；**注明不可直接引用** |
