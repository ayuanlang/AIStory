...
- **继承范围**：剧情结构、Scene/Beat 边界、对白/OS/V.O.、**对白拆句判定**、**对白组边界**、**下一节拍起幅**、动作链、微表情/微动作/微反应、每场细节特写、心理可视化、环境补充、主/衍生环境、站位表达式、**Stage 1「Beat 完整逻辑」前置+六环节**（细则以 Stage 1 §11 为准）、Beat 建置更新判定。
...
## 映射规则
> **Beat 级总纲**：Stage 1「Beat 完整逻辑」**前置 + 六环节**为唯一基准；空间/站位/微表演细则以 Stage 1 §11 为准。
- **承接上一 Beat 全体站位（= 环节 0，强制）**：Beat 2+ 须继承上一 Beat `{结果落位}`/`{景深层次}` 终态；`建置更新=否` 时 `{景深层次}`/`{空间}` 须写明「继承上一 Beat 全体站位 + 变更项」；`建置更新=是` 仍须写清相对上一 Beat 哪些主体位移/入出画/换层。Scene 首 Beat 继承 Stage 1 开场建置或上场承接。
...
- **站位表达（先三层，再层内逐个）**：= Stage 1 环节 2–3；继承 §11 七要素、**前后位置双轨**（相对镜头 + 相对 ENV/实体）与四步核验。
- **观察视角—环境—建置三联（Beat 粒度）**：= Stage 1 环节 1；`Observer View` 与 `{景深层次}`、`{空间}`、`[环境切换声明]` 同拍咬合。  
  - **环境—视角匹配自检（强制，继承 §12）**：每个 Beat 落 `Observer View` 前须核对：当前 `ENV` 是否与观察角度匹配；**OTS/正反/POV/门窗内外**等须切衍生环境并在 `[环境切换声明]` 写明；错用主环境 → `{覆盖核销}` 标 `上游环境—视角错配`。  
  - **必填**：当前 ENV、观察起点、观察角度、观察目标、可见范围边界。  
  - **视角变化**：须完整落入 `Observer View`、`[环境切换声明]` 与 `建置更新`；`建置更新=否` 时不得发生实质视角或环境变化（除非 Stage 1 已明确仅层内微动）。  
  - **缺口**：Stage 1 缺观察侧或环境，在 `{覆盖核销}` 写 `上游观察视角—环境—建置缺口`。
- **站位表达式**：`[当前环境] + [视角角度] + [具名主体] + [FG/MG/BG] + [左中右] + [距参照物] + [运动方向与朝向] + [动作方式] + [环境切换]`；画内每个可见主体各写一条，不得合并省略。
- **景深层次建置**：= 环节 2 工程化写法；**先** FG→MG→BG 框架，**再**各层内逐主体落位（七要素+动作方式/力度/速度须继承）。`建置更新=是` 完整继承；`建置更新=否` 继承上一 Beat + 变更项。禁止省略三层或「中景两人」概括。
...
- **动作字段**：`{行为过程}`/`{环境交互}`/`{结果落位}` 仅转换 Stage 1【动作/视觉节拍】；须继承 `主节拍/主动作`（归属主动施力方；客体轨迹不独立成 Beat）与完整因果链；位移动作保留位移五元组，**运动方向与朝向**细则见 Stage 1 §11。
...
- **{观察视角与空间建置}**：继承 Stage 1 Beat 完整逻辑环节 1–2 + §11 落位；视角变化与建置更新一并继承。
...
- **{动作/视觉节拍}**：完整继承 Stage 1 **前置 + 六环节**（见 Stage 1 总纲表）；含 `{对白组边界}`、**{下一节拍起幅}**；Beat 2+ 须含环节 0 承接说明。
