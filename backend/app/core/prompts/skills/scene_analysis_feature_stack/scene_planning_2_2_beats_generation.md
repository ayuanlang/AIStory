...
# Prompt Updated At: 2026-08-04 08:55:00 +08:00
...
> **🏆 Beat 正文完全继承（最高硬约束）**：`{Beats}` 内全部正文——含 `────【建置】────` / `────【入戏】────` 及其下每一句、每一词、每一标点、站位/姿态/视线/入戏动作/对白——**必须完全继承 Stage 1 原文**。除下方「唯一允许改动」外，**禁止任何修改**（润色、改写、概括、补全、删减、重排、同义替换、语序调整、补推断、纠「错」、补漏、合并拆句、改景别/机位表述等）。改一处 = 整场废弃重写。
>
> **【Beat切换说明】不注入、不输出（最高）**：上游管道注入本环节前已剥离 `────【Beat切换说明】────` 整块；输入中**不应**出现该段。`{Beats}` **禁止**补写/还原/自拟切换说明、自检、速查更新、视角换算台账等。只落建置+入戏（+外层 `[BEAT_START/END]`）。

**【零丢失（Beat 内）】**每个 Beat 的建置/入戏与内部分隔符须**逐字原样**落入 `{Beats}`；禁「同上/同前/见上/略/摘要」。**唯一允许改动**＝叙述层具名实体 → Index 名字符串替换并套 `CHAR:`/`ENV:`/`PROP:`（见「仅此五项」）；**台词正文绝不 Index 化**（见「对白豁免」）。Index 化只换称呼串，**不得**连带改写周围建置/入戏语句。
...
**【禁止】**改写/概括/润色/补创/删减建置或入戏或对白；补写【Beat切换说明】或任何自检/台账块；改 Scene/Beat 数量/顺序/边界；对 Index 无行实体套任何类型前缀（含自创 `EXTRA:` 等）；用 Index 元数据回填 Stage 1 未写字段；缺项回填/重算/补建；补写输入中不存在的 Scene 级【】说明块或环境块；改写或省略输入的 `【场景名称】`。
...
| `{Beats}` | **强制主责**：输入全部 Beat 块**逐字完全继承**落入（含建置/入戏全文；**不含**【Beat切换说明】；**仅**叙述层实体名 Index 化，其余字句标点零改动；保留外层/内层分隔符；台词原样） |
