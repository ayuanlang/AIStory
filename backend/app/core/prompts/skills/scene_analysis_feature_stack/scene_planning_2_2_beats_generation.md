...
> **🏆 Beat 正文完全继承（最高硬约束）**：`{Beats}` 内全部正文——含 `────【建置】────` / `────【入戏】────` 及其下每一句、每一词、每一标点、站位/姿态/视线/入戏动作/对白、**卖点四通道落点（行为/表情/空镜/语言，供分镜映射 Video）**、**`构图=` 档（三分／中心／纵深等，供分镜与 Index `scene_mood` 一并综合：运镜/构图/色彩/灯光/焦距表达剧情与情绪）**——**必须完全继承注入原文**。除下方「唯一允许改动」外，**禁止任何修改**（润色、改写、概括、补全、删减、重排、同义替换、语序调整、补推断、纠「错」、补漏、合并拆句、改景别/机位/构图档表述等）。改一处 = 整场废弃重写。
...
5. **场景名称落表**：输入有 `【场景名称】{短名}｜{日·夜}·{内/外}｜{季节}｜{气候}｜{正常叙事/闪回/倒叙}` 时，将 `{短名}｜{日·夜}·{内/外}｜{季节}｜{气候}｜{正常叙事/闪回/倒叙}` **逐字原样**写入 `Scene Name` 列；禁润色/改写/拆分/省略附加信息；输入无该行 → `None`。`{短名}` 应为上游按本场情节总结的标题，本环节**不得**改写成地名或另拟。
...
| 字段 | 规则 |
| :--- | :--- |
| `Scene Name` | 输入有 `【场景名称】{短名}｜{日·夜}·{内/外}｜{季节}｜{气候}｜{正常叙事/闪回/倒叙}` → 写入 `{短名}｜{日·夜}·{内/外}｜{季节}｜{气候}｜{正常叙事/闪回/倒叙}`（**逐字原样**，含日·内/外；**不含** `【场景名称】` 前缀；`{短名}`=上游本场情节总结，禁改写成地名）；无该行 → `None`；禁自创/润色/拆短名 |
