...
| 【衍生环境】 | `{衍生环境}` | **须含 `0度{主环境名}` Master**；**每条衍生 ENV 独立写** `derivative_view_360_entities`（四向自然语言，每段 **【{N}度方向】**；**实体须与主环境固定清单逐字一致、同等具体度，逻辑严格匹配，仅观察角度做各向镜像映射对齐（可见/不可见可变），禁止增删改矛盾或简化实体描述**）；**`spatial_axis`**、**`lens_profile:Wide|Standard`**、**`axis_crossing`**；另有转角 N、触发、空镜差异；无则 `无：否决证据` |
