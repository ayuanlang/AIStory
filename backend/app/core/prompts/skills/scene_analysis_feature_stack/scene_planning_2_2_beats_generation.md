...
# Prompt Updated At: 2026-07-03 22:02:00 +08:00
...
- 对白/OS/V.O.、站位、微表演、观察视角：**逐字继承** Stage 1，仅替换其中实体名为 Index 标准表达。
...
Beat 数/顺序 = Stage 1｜`Episode ID`/`Scene ID`/`Scene No.` 规范且与 Stage 1 场序一致｜**Stage 1 全部【】说明块原样落入 Core Scene Info、无省略写法**｜语义 = Stage 1（仅实体 Index 化）｜**全部 `CHAR:`/`ENV:`/`PROP:` 可反向追溯到 Index `subject_no`；Index 外实体无类型前缀标注**｜时长原样继承 Stage 1｜缺口已标 `{覆盖核销}`。
...
| EP01 | EP01_SC01 | 1 | {场名} | {Xs} | - **{故事内核}**: …<br>- **{节拍时间规划}**: …<br>- **{Duration Estimate Basis}**: …<br>- **{细节特写规划}**: …<br>- **{Scene识别}**: …<br>- **{主环境}**: ENV:[{主环境Index名}] …<br>- **{Scene实体覆盖}**: …<br>- **{观察视角与空间建置}**: …<br>- **{衍生环境}**: ENV:[0度{主环境名}] …<br>- **{场景切换与首节拍转场}**: …<br>- **{Beats}**:<br>- Beat 1: 观察视角…当前 ENV:[0度{主环境名}]…**林医生**…**陈医生**…inline 对白…`[Beat切换说明]` `{Beat终态画内实体}=…`<br>- Beat 2: …<br>- **{覆盖核销}**: …<br>- **{登场实体}**: CHAR:[@…], ENV:[…], PROP:[…] | {Adapted Script 头尾片段} | {可拍ENV Index名, …} | VARIANT_OF:{主环境名}, … | {主环境Index名}, … | {Delta或None}, … | {入场态} | {出场态} | CHAR:[@…], … | PROP:[…], … |
