...
- **变体继承与四面拓扑**：基准实体 `dependency_strategy.type=Original`, `visual_dependencies=[]`；在设计基准环境（作为其他衍生环境依赖，名称不带角度前缀）时，必须在其 `description_cn` 中写清楚 4 面（0度正面、90度右侧、180度反面、270度左侧）的拓扑情况（存在的物体或景物结构），后续同系的环境（包括基准环境自身及衍生环境）都要严格基于此 4 面描述进行设计与编写提示词。派生实体 `type=Type A/Type B` 并指向**剧情时序上紧邻的上一完整形象**（读取上游 `dependency_reference` / `base_entity`，禁止跳链直挂远端基础版）。`visual_dependencies` 禁填 `S001/E001` 等编号，必须用逐字符一致的实体名引用（如 `ENV:[...]`、`CHAR:[@...]`、`PROP:[...]`）。若 Subject Index 中 `base_entity` 已标注主环境名，或环境名以 `{角度}度{主环境名}` 构成，则该实体视为主环境的**视角衍生**环境：`visual_dependencies` 必须包含 `ENV:[主环境名]`，`dependency_strategy.logic` 必须说明继承主环境的共同锚点并只强化当前视角/区域/方向差异。若上游为**状态/破坏衍生**（`{主环境名}_{状态/域场标识}`），`visual_dependencies` 必须回挂上游 `dependency_reference` 所指的**紧邻上一完整状态**（首个状态衍生回挂主环境；后续破坏/崩塌态回挂前一完整状态，不得跳过中间态直挂主环境）。
...
- **跨越 180 度轴线（越轴）方位重映射**：当机位跨越同一空间锚点建立的 180 度轴线（越轴）时，必须执行方位重映射：主环境左/右关系按新机位翻转重判，前景/后景与近距/远距按当前可见性重新分配；不得沿用基准环境的左右与前后表述。
