...
- **变体继承与四面拓扑**：基准实体 `dependency_strategy.type=Original`, `visual_dependencies=[]`；在设计基准环境（作为其他衍生环境依赖，名称往往不带度数方向）时，必须在其 `description_cn` 中写清楚 4 面（0度正面、90度右侧、180度反面、270度左侧）的拓扑情况（存在的物体或景物结构），后续同系的环境（包括基准环境自身及衍生环境）都要严格基于此 4 面描述进行设计与编写提示词。派生实体 `type=Type A/Type B` 并指向上一阶段。`visual_dependencies` 禁填 `S001/E001` 等编号，必须用逐字符一致的实体名引用（如 `ENV:[...]`、`CHAR:[@...]`、`PROP:[...]`）。若环境名以“已存在主环境名 + 空格 + 衍生类型/观察区域/可见方向”构成，则该实体视为主环境的衍生环境：`visual_dependencies` 必须包含 `ENV:[主环境名]`，`dependency_strategy.logic` 必须说明继承主环境的共同锚点并只强化当前视角/区域/方向差异。
...
#### 3.1.5 变体与正反环境逻辑
- **基准环境四面拓扑强制设定**：对于作为其他衍生环境依赖的基准环境（名称通常没有度数或特殊方位），在设计时，必须在其 `description_cn` 内明确写清楚该空间 4 面（0度正面、90度侧边、180度反面、270度另一侧边）的四方情况（明确描述各方向存在的物体或景物结构）。后续同系的所有环境（包括基准环境自身的当前视角，以及其他衍生视角），都必须完全基于这个四面拓扑描述进行进一步设计与编写提示词，禁止各视角出现拓扑矛盾。
- **四向一致性与现实逻辑强校验**：凡涉及 0/90/180/270 度方向描述，必须保证四向信息彼此匹配、可互相推导、无左右颠倒或前后冲突，并符合真实空间结构与摄影机轴线逻辑。涉及门、窗、柜门、屏风等可开合构件时，必须逐视角明确：开合状态、内开或外开、铰链（门轴）位于哪一侧、门扇当前朝向与可见面；禁止只写“有门/开着门”而不交代开向。
