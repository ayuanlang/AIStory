...
- **变体继承与 360 度拓扑**：基准实体 `dependency_strategy.type=Original`, `visual_dependencies=[]`；在设计**主环境**时，须先确认上游 **0 度观察基点与开场建置 Master 轴一致**（如桌面对坐取桌子长边侧面），再在其 `description_cn` 中写清楚该空间 **360 度方位拓扑**（以 0 度正面、顺时针标注各方向存在的物体或景物结构；至少写清 0/90/180/270 四向，复杂空间须补全 45 度倍数方位），后续同系的环境（包括主环境自身当前视角及所有衍生环境）都要严格基于此 360 度拓扑进行设计与编写提示词。**视角衍生转角对照（强制）**：若环境名以 `{N}度{主环境名}` 构成，则该衍生环境的 **0 度正面（Viewing Direction 基准）** 必须严格采用主环境 360 度拓扑中 **N 度方位** 的描述；衍生环境 90/180/270 度依次对应主环境 `(N+90)%360`、`(N+180)%360`、`(N+270)%360` 方位，禁止脱离主环境拓扑另造空间或仅替换方位词而不执行转角映射。派生实体 `type=Type A/Type B` 并指向**剧情时序上紧邻的上一完整形象**（读取上游 `dependency_reference` / `base_entity`，禁止跳链直挂远端基础版）。`visual_dependencies` 禁填 `S001/E001` 等编号，必须用逐字符一致的实体名引用（如 `ENV:[...]`、`CHAR:[@...]`、`PROP:[...]`）。若 Subject Index 中 `base_entity` 已标注主环境名，或环境名以 `{角度}度{主环境名}` 构成，则该实体视为主环境的**视角衍生**环境：`visual_dependencies` 必须包含 `ENV:[主环境名]`，`dependency_strategy.logic` 必须说明继承主环境的共同锚点、**转角 N 与 0 度→N 度对照关系**并只强化当前视角/区域/方向差异。若上游为**状态/破坏衍生**（`{主环境名}_{状态/域场标识}`），`visual_dependencies` 必须回挂上游 `dependency_reference` 所指的**紧邻上一完整状态**（首个状态衍生回挂主环境；后续破坏/崩塌态回挂前一完整状态，不得跳过中间态直挂主环境）。
...
#### 3.1.5 变体与正反环境逻辑
- **主环境 0 度锚定（强制，先于 360 度拓扑）**：主环境 360 度拓扑的 **0 度正面** 须与上游 Stage 1 **开场建置/首 Beat Master 视角** 同轴对齐。须先读取【主环境】中的 0 度观察基点与开场人物关系，再展开 360 度各向描述。**典型**：开场两人在桌前对坐 → 0 度须为**桌子长边侧面 Two Shot**（左右分置、桌面作中轴），禁止把桌头/桌尾单人正面或背墙当 0 度；并排同向、门内外、舞台面对观众等须按 Stage 1 已锚定的 Master 轴设定 0 度。
- **主环境 360 度拓扑强制设定**：对于**主环境**（名称通常没有度数或特殊方位，作为衍生环境依赖），必须在其 `description_cn` 内明确写清楚该空间 **360 度方位拓扑**（以已锚定 **0 度正面**、顺时针标注各方向存在的物体或景物结构；至少写清 0/90/180/270 四向，复杂空间须补全 45 度倍数方位）。后续同系的所有环境（包括主环境自身的当前视角，以及其他衍生视角），都必须完全基于这个 360 度拓扑进行进一步设计与编写提示词，禁止各视角出现拓扑矛盾。
- **视角衍生转角对照映射（强制）**：环境名 `{N}度{主环境名}` 表示相对主环境 0 度顺时针旋转 **N** 后的机位。该衍生环境的 **0 度正面** 必须严格采用主环境 360 度拓扑中 **N 度方位** 的描述作为正面基准（例：90 度衍生 → 主环境 90 度描述 = 衍生 0 度正面）；衍生 90/180/270 度依次对应主环境 `(N+90)%360`、`(N+180)%360`、`(N+270)%360` 方位。`description_cn` 与 `generation_prompt_cn` 须显式写明「转角 N → 衍生 0 度 = 主环境 N 度」对照，再写当前机位 Delta。
  ...
  "environments": [
    {
      ...
      "description_cn": "港口办公区夜景。项目基础定位为情感悬疑，项目全局风格为电影级写实冷暖对比与克制压迫感。主环境 360 度方位拓扑：0度正面为有雨痕斑驳的百叶窗与玻璃；45度右前为文件柜侧缘与桌角；90度右侧为一排铁皮文件柜；135度右后为白板侧缘；180度反面为半开实木门及外侧幽灰通道；225度左后为门轴侧墙；270度左侧为挂满线索照片的白板；315度左前为百叶窗侧墙与桌沿；老旧实木桌位于中央。当前视角由门外（180度）向内（0度）看，老旧木桌与百叶窗拉出纵深。Key Light 来自桌面黄铜台灯，照亮核心舞台；Fill Light 来自窗外冷蓝雨夜反射，保留暗部细节。后景可有少量匿名深夜办公群演，只作远景氛围。美学参考：《银翼杀手2049》— 雨夜窗面冷蓝反射与半遮光百叶窗的 noir 压抑层次；《社交网络》— 低调办公台灯与旧木桌的冷暖对冲布局；《真探》S1 — 九十年代夜班办公的磨损木作与文件柜秩序感。",
