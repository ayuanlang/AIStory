...
# Prompt Updated At: 2026-06-25 20:30:00 +08:00
...
- **视角衍生转角对照映射（强制）**：环境名 `{N}度{主环境名}` 表示相对主环境 0 度**顺时针**旋转 **N** 后的机位。该衍生环境的 **0 度正面** 必须严格采用主环境 360 度拓扑中 **N 度方位** 的描述作为正面基准（例：90 度衍生 → 主环境 90 度描述 = 衍生 0 度正面）；衍生 90/180/270 度依次对应主环境 `(N+90)%360`、`(N+180)%360`、`(N+270)%360` 方位。`description_cn` 与 `generation_prompt_cn` 须显式写明「转角 N → 衍生 0 度 = 主环境 N 度」对照，再写当前机位 Delta；**须列相对主环境 0 度新可见/不可见半空间**；**180 度衍生须写明 BG = 0 度不可见/被遮挡的对侧内容**。
...
- **180 度互补覆盖**：正反视角通常不是两个相似构图，而是围绕同一锚点分别看向相反半空间；两条衍生环境合并后应能形成主环境的 360 度空间理解。每条只展示当前镜头可见的半圈空间，**必须说明与配对视角互补的对面背景、入口/出口、墙面、窗面、家具背面或通道**——**180 度视角的 BG 须明确呈现 0 度视角不可见/位于机位后方的内容**（如 0 度 BG 的窗墙在 180 度变为机位后方，而 0 度不可见的门/对侧墙段成为 180 度 BG），不得把同一背景重复给正反双方。
...
- **状态/特效衍生环境**：上游命名为 `{主环境名}_{状态/域场标识}` 时，须按上游 `dependency_reference` 回挂 `visual_dependencies`（首个状态衍生回挂 `ENV:[主环境名]`；后续破坏/崩塌/余波态回挂**紧邻上一完整状态**，如 `ENV:[办公室会客区_符阵覆盖态]`，禁止跳链直挂主环境）；`generation_prompt_cn` 先写继承的直接依赖基准之共同锚点（地面、墙界、主舞台方向），再写相对该基准的 `empty_view_delta`（崩裂、符阵覆盖、域场光、再生完成态等）；禁止把状态衍生生成成与依赖基准无关的新空间。
...
```json
{
  "environments": [
    ...
    },
    {
      ...
      "generation_prompt_cn": "电影级写实衍生环境，50mm 自然焦距，胸口高度，镜头水平。180 度视角衍生：衍生 0 度正面严格对应主环境 180 度半开实木门及门外走廊。从办公桌内侧靠窗一侧作为 Viewpoint Anchor，沿办公桌中线反向看向半开实木门框和门外走廊，Viewing Direction 为衍生 0 度（=主环境 180 度）。继承主环境共同锚点：旧木地板为深褐色磨损纹理，实木办公桌位于 MG 中央，左侧可见铁皮柜（主环境 90 度→衍生 270 度），右侧可见白板（主环境 270 度→衍生 90 度），黄铜台灯在桌面左后角，金属百叶窗在机位后方（主环境 0 度，本视角不可作当前背景），半开实木门框保持同一木纹、尺度和暖光边缘。FG 为桌面文件边缘和台灯暖光反射；MG 为办公桌内侧与两把空转椅的椅背背面，椅背朝向门外、椅面朝向机位，桌前/桌后关系按 180 度转角对照重新判定；BG 为半开门扇与门外冷蓝走廊，门扇向办公室内侧开启并停在画面右侧。黄铜台灯作暖色 Key Light，从画面左中部照亮桌沿、椅背背面和门框内缘；走廊冷蓝雨夜反射作 Fill Light，分离门外深处与室内暗部；门框边缘形成柔弱 Backlight。中等景深，办公桌与门框锐利，走廊远端略软。色彩继承旧木褐、煤灰蓝、琥珀暖光与冷白雨痕反射，当前 Delta 只强化入口半空间、椅背背面、向内开启门扇和走廊来向。Clean Plate，无人物、无肩膀、无可识别人脸。",
      "generation_prompt_en": "",
