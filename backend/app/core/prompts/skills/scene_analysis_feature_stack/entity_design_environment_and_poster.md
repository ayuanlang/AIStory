...
- **变体继承与四面拓扑**：基准实体 `dependency_strategy.type=Original`, `visual_dependencies=[]`；在设计基准环境（作为其他衍生环境依赖，名称不带角度前缀）时，必须在其 `description_cn` 中写清楚 4 面（0度正面、90度右侧、180度反面、270度左侧）的拓扑情况（存在的物体或景物结构），后续同系的环境（包括基准环境自身及衍生环境）都要严格基于此 4 面描述进行设计与编写提示词。派生实体 `type=Type A/Type B` 并指向上一阶段。`visual_dependencies` 禁填 `S001/E001` 等编号，必须用逐字符一致的实体名引用（如 `ENV:[...]`、`CHAR:[@...]`、`PROP:[...]`）。若 Subject Index 中 `base_entity` 已标注主环境名，或环境名以 `{角度}度{主环境名}` 构成，则该实体视为主环境的衍生环境：`visual_dependencies` 必须包含 `ENV:[主环境名]`，`dependency_strategy.logic` 必须说明继承主环境的共同锚点并只强化当前视角/区域/方向差异。
...
- **状态/特效衍生环境**：上游命名为 `{主环境名}_{状态/域场标识}` 时，须回挂 `ENV:[主环境名]`；`generation_prompt_cn` 先写继承的共同锚点（地面、墙界、主舞台方向），再写 `empty_view_delta`（崩裂、符阵覆盖、域场光、再生完成态等）；禁止把状态衍生生成成与主环境无关的新空间。
