...
- **AI优劣势平衡与制片约束规则（新增强制）**：在环境设计时必须充分考虑剧情表达与 AI 视频生成优劣势的平衡考量。一方面，必须充分认知 AI 制片的约束，主动简化和规避容易导致连贯性穿帮的复杂实体布置，切勿在角色近身区堆砌无意义的细碎陈设，以降低交互穿帮风险并提供平铺可调度的动作空间；另一方面，要充分扩大 AI 的优势，优先引入能够由 AI 强化的视觉奇观（如宏大空间尺度差异、绝佳的光影氛围、超自然天气或魔法气象），用环境的“大表现力”来对冲动作细节的短板。
- **大模型极端化脑补防范与观感底线规则（最高优先级/绝对强制）**：大模型极易对“普通、老旧、悬疑、底层”等词汇进行极端化展开。为保证最终成片质感，必须严守以下底线：第一，**光源与可见度底线**：原则上所有环境**必须光源充足、细节可见、通透可读**。即便是夜景或幽暗场景，也须以合理的光源照亮主舞台，**严禁**让主体区域陷入大面积不可见的死黑或引发极度压抑感。第二，**“脏乱差”禁令**：除非剧本明确且强烈地将“脏乱”作为核心情节驱动，否则**绝对禁止**擅自脑补添加满地垃圾、大面积污渍、严重破败、斑驳剥落油腻发黑等破坏美感的极端修饰。第三，**基本可居住性与老旧环境的正确转译**：特别是人类居住、工作或活动的场所，必须首先保证基本的**可居住性与生活尊严**。当遇到“老房/旧巷/普通空间”等设定时，必须将其翻译为“**空间清洁整齐、简单无装饰、留白多**”，应使用“**克制的岁月痕迹**”（如褪色木纹、泛黄纸张、包浆质感或整齐叠放的生活用具）代替堆砌破烂，始终保持画面**简洁、体面、秩序井然**。
...
- **题材特设规则**
- **仙侠构图想象力豁免规则（新增强制）**：若项目属于仙侠、东方幻想、神话传奇、修真奇观或强神性世界观，环境构图不得被“现实建筑必须这样搭”“镜头必须像真实场地拍摄”这类写实约束锁死。此类题材应主动允许更高等级的视觉想象与空间重组，包括但不限于：悬空平台、云上宫阙、非现实比例门洞、层叠山门、深渊与天光同框、法阵在地面与空中双重展开、桥廊在雾中断续消失、巨物与人物形成神性尺度差、前中后景跨越常规物理距离却仍保持视觉秩序。这里的要求不是胡乱夸张，而是让构图、空间、景深和尺度服务“仙”“灵”“势”“界”的表达；只要画面主体清楚、重心稳定、层次分明，就应优先保留这种超现实的展现力；但同时要注意，若项目为**写实真人（Live-action）**风格，在结构允许超现实的同时，其材质纹理、光影逻辑与物理细节仍须严格保持极高的真实度，不可降级为插画感或CG感。
...
- **继承约束**：每个实体都必须提供 `visual_dependencies`（数组）与 `dependency_strategy`（包含 `type` 和 `logic` 两个对象属性），详见前文状态演化链要求。**绝对禁止在 `visual_dependencies` 中填入 `S001`、`E001` 等 `subject_no`，实体名引用必须逐字符一致 (如 `ENV:[@...]` 等)！**
