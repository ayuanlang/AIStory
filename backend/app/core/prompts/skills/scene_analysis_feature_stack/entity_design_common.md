...
# Prompt Updated At: 2026-07-16 23:30:00 +08:00
...
- **全局光学优先级 (Global Optical Priority Gate)**：先满足亮度、可读性、主辅光、色温、空气感，再写风格/情绪。**Environment/Prop/Poster 默认以 §1.5「大光比」为主打**；**Character 真人定妆**以 `entity_design_character.md` §2.0 为准（见 §1.5-C 豁免）。仅当 `Genre`/`Base Positioning`/`tone` 明确治愈/广告/儿童明亮向时，Environment 可降至 §1.5 轻大光比。关键信息仍须可读。
...
| :--- | :--- | :--- |
| **Character** | Key/Fill/Backlight 光源类型、方向、色温、冷暖 rationale；服装主辅点缀色须服从 §B 四层色谱 | **真人定妆豁免（Mandatory）**：当 §1.6 判真人实拍时，**以 `entity_design_character.md` §2.0/§2.1G 为准**——prompt 光照 ≤1 句、禁 Key/Fill/色温/rim/大光比亮暗分区转写；大光比与色谱 rationale **只写 `description_cn`**。三维/二维 Character 仍按各自专项用渲染语言落地 §B 色谱 |
| **Environment / Poster** | 光学 rationale + `entity_design_environment_and_poster.md` §3.1.9 完整色彩规格（色系/色温/饱和度/结构/影调/分离色调/构图）+ 主辅点缀**实体落点** + 前景/中景/背景 各层受光与投影；**夜景须按同文件 §3.1.8 写覆盖照明** | 须写 **≥2 动机光源**（夜景剧情允许时 **≥3** 或 1 强自然光+≥2 Practical）；**每个光源须写作用范围 + 可见效果**；须含 §3.1.9-A 色彩规格句式 + §3.1.9-B 点缀实体落点；主环境四向拼图须与 §B 主色/辅色/过渡色一致 |
| **Prop** | 材质色与 §B 主辅色/点缀色绑定 | 须写动机光下材质高光/半影/暗部与全项目色谱一致 |

#### D. 输出前色谱自检（强制）

逐 Subject 核对：
1. Environment/Prop：是否已按 §A 落实大光比（Key:Fill + 亮暗分区）？**Character 真人定妆**：是否已按 §2.0 豁免——`description_cn` 含 rationale、`generation_prompt_cn` **未**写入大光比/Key:Fill？
2. 是否已按 §B 推导并回写主冷暖基调 + 四层色谱（Character 色谱服从服装主辅色；大光比亮暗句仅 Environment/Prop / Character `description_cn`）？
3. Environment/Prop 的 `generation_prompt_cn` 能否回答：**哪一区暖、哪一区冷、哪一层亮、哪一层暗、光从哪来、每个光源作用范围覆盖哪里、产生什么可见效果**？
