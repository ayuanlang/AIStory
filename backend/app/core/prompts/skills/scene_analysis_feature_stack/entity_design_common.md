...
- **主光源先行与光影排序**：角色/道具/环境/海报均先写清主光源：来源、方向、**作用范围**（照亮哪些 FG/MG/BG 区段/具名表面、未覆盖暗区）、**可见效果**（投影/半影/反射/高光/体积光等）；再写补光/轮廓光与材质色彩响应。禁止只列灯位不写作用范围与效果；禁止先堆风格/配色/装饰，再泛写“电影感光影”。
...
| **Environment / Poster** | 光学 rationale + 主辅色谱 + FG/MG/BG 各层受光与投影；**夜景须按 `entity_design_environment_and_poster.md` §3.1.8 写覆盖照明** | 须写 **≥2 动机光源**（夜景剧情允许时 **≥3** 或 1 强自然光+≥2 Practical）；**每个光源须写作用范围 + 可见效果**；夜景须写自然主光/暖调覆盖/多人工覆盖网之一；主环境四向拼图须与 §B 主色/辅色/过渡色一致 |
...
4. **夜景 Subject（剧情允许时）**：是否已按 `entity_design_environment_and_poster.md` §3.1.8 写**自然主光 / 暖调覆盖 / 多人工覆盖网**之一，且 MG 主舞台+动线可读？
