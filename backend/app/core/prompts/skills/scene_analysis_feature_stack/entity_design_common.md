...
- **[Node 1] World Bible (世界观与视觉流派强绑定)**
  ...
  - **视觉正向注入矩阵**：所有 `generation_prompt` 必须写入匹配类型的正向视觉词：
    ...
    - **真人写实硬约束**：若 `Project Context.Type`、`Genre`、`Base Positioning`、`Global_Style` 任一命中真人写实/现实主义/都市家庭/职场/社会派/纪实向，所有 character 须按 `entity_design_character.md` **§2.1F-A 七必锚（0–6）**全量写入：**0 真人实拍语义锚** + 保留毛孔纹理、matte-to-satin 分区光泽、薄透自然妆面、真人面部组织、微瑕个体标记（≥1）、发丝真实结构。禁止：玻璃肌、零毛孔、强 idol 妆、二次元比例、3D/CGI/插画风、过度雾化、棚拍广告感、油腻/塑料质感。服装/体态/场景保持基本可信。
