...
- **Character / Prop 四宫格画幅**：`generation_prompt_cn` 须 16:9 横向、浅灰色背景、四视角同一横排、连续统一浅灰画板。禁上下两排、2×2、换行断裂、错层。留白开阔；第一宫占横向 35%、纵向居中；其余三宫共享 65%。**Character** 第一宫=面部特写，其余三宫=正/侧/背全身。**Prop** 第一/第二宫=道具+标准化尺度参照合成格（细则以 prop 分型 §1.1/§1.1A 为准，覆盖本条「细节特写/正面单体」）；第三/四宫=侧/背道具单体。**Character 全身格隐脸（最高；仅单人四视图）**：仅第一宫生成完整可读面部；第二/三/四宫全身**禁止生成脸部**——面部区域（发际以下至下颌，含眼鼻口颊）须用与画板相同的浅灰底色完全填覆，呈无脸；禁五官、模糊脸、马赛克、黑条、半透明脸。发型轮廓、颈、衣着、体态、鞋履仍须完整。侧视可见面庞同样覆盖，可留耳廓与发际；背视保留后脑发型、不露脸。群演簇与 Prop 不适用。**群演簇不适用本条四宫格**——改为 character §2.4：单张**全身**群体设定图（鞋履可见；有坐骑则人+骑同框全身），人数与构图跟剧情群体建置形式，不限 4 人/4 格；**衣着下限与单人相同**（衣本体 `clothing` ≥7，**不含**配饰/器械/旗帜/坐骑等附属物；服色散 3–7：主色锁身份制度 + 辅色 + 点缀，禁制服/军装空壳）；附属物另写满槽、不占 ≥7；**不抽独立 PROP**，一律写进该簇 `clothing`。`generation_prompt_en` 固定 `""`。环境/海报画幅见分型文件。
- 示例/模板中的职业、人种、服装、环境名等仅格式参考；每次须按当前剧本专属设计。

### 1.2 语言与项目语境
- **语言总契约**：自然语言默认跟随剧本原语；`Project Context.Language` / `project_language` 明确时覆盖。仅 ID、固定键名、约定标签可保留既定格式；其余禁中英混杂。**Prop 尺度标注例外**：`L=`/`H=`/`V=`/`T=` 与 `mm`/`cm`/`m`/`mL`/`L`/`cm3`/`m3` 等英文单位缩写、以及线性图上的参照全名与 `长=`/`高=`/`宽=` 比例句，可进 `generation_prompt_cn`、画面标注与描点。**同义中英重复**：叙述字段只保留项目语言侧（`_cn`/`_en` 固定双语列各填各语除外，禁同格堆叠同义双语）。
- **双语字段**：`_cn` 中文；`_en` 英文；`anchor_description` 英文短语（3–5 个高密度短语定位同一实体；Character 优先身份/相貌轮廓/区分服饰）。**Prop 例外**：描点第一条须为参照物全名+对参照的长/高/宽比例，其后为数字英文单位（如 `成人女性手掌线性图｜长=1/2掌长｜高=2/3掌宽｜宽≈1/6掌厚, 86mm L, 54mm H, 18cm3 V`），其余短语仍英文。第一/第二宫线性图上须有同核可读文字（参照全名+三轴比例）。例外：`generation_prompt_en` 恒为 `""`；完整生图词只写 `generation_prompt_cn`。
