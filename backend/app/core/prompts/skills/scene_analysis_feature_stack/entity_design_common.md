...
- **[Node 1] World Bible (世界观与视觉流派强绑定)**
  ...
  - **色彩审美专家与高级配色体系总则（新增强制）**：模型在处理所有资产（环境、角色服装、道具等）的色彩时，必须全程代入“**色彩审美专家与美术指导**”视角。当剧本未强限制绝对单一色调时，应主动设计配置更丰富、更美观且具高级感的色彩体系。禁绝扁平、单调或高饱和度的廉价感原色。必须采用主色、辅色、点缀色的层次搭配（如莫兰迪色系、电影级青橙对比、冷暖互补或同类色高级过渡），并将颜色附着在具体的材质细节、光影反射或空间距离上。色彩既要服务于相关资产的情绪基调与时代背景，更要主动提升最终成片的视觉观赏性与艺术美感。
...
- **灯光设计具体要求 (Lighting Design)**：
  - **类型与基础定位绑定 (Genre & Base Positioning Binding)**：所有的灯光设计（包括亮度、光质和反差）必须严格服从项目的类型（`Genre`）与剧本的基础定位（`Base Positioning`）。例如：喜剧/都市日常应偏向明亮亲和、平滑唯美；悬疑/惊悚剧需强化高反差硬光与强烈的视觉压迫感。
  - **三点布光 (Three-Point Lighting)**：必须包含并明确主光（Key Light，确立基调）、辅光（Fill Light，控制反差）、轮廓光/背面光（Backlight，将主体从背景中剥离）。
  - **光质控制（软硬光）**：区分光线质感，硬光制造强烈的阴影和戏剧冲突（如黑色电影的百叶窗阴影）；柔光箱制造平滑、唯美、没有攻击性的面部光线。
  - **色彩情感 (Color Motivation)**：利用冷暖光对比（如经典的 Teal & Orange 橙蓝对比），或者用特殊的颜色（如红光代表危险、绿光代表诡异）来烘托气氛。
- **全局立体光影与色彩层次规则（新增强制）**：所有资产 prompt 必须先建立明度、色温、清晰度和材质反射差，再展开装饰；阴影保留细节，特别是环境空间必须有明确的前/中/后景体积光与层次差，角色/道具/海报也需通过定向光感拉开立体纵深。
- **全局色彩谱系约束（新增强制）**：色彩描述不能只停留在冷暖/饱和度判断，必须建立主辅色系与过渡色层，确保画面有清晰色彩秩序，不得写成单色平铺。符合本节前文明示的“色彩审美专家与高级配色体系总则”。
- **一级调色规则 (Color Correction, 新增强制)**：必须在画面描述中统一对应场景所有镜头的曝光、白平衡级别，匹配不同机位（如正反打、特写与全景）素材的基本色彩与反差，使同一场景下的资产在视觉上无缝衔接。
- **二级调色与风格化规则 (Color Grading, 新增强制)**：必须具备电影后期调色意识。通过主动干预指定颜色的色相、饱和度、明度（HSL），为影片或资产加上确切的风格化影调，如特定的胶片感（Film Look）、赛博朋克风或更特殊的冲洗工艺（如：跳银洗 Bleach Bypass、降饱和高对比）。风格化调色维度必须与项目类型和情绪氛围深度绑定。
- **自然语言流动的全维度覆盖 (Natural-Prose & Dimension Gate)**：`generation_prompt_en` / `negative_prompt_en` 必须是短句/短段的连贯自然英文 (prose)（推荐：主体 -> 主光源与光影结构 -> 构图机位 -> 调色色彩 -> 细节 -> 风格约束）。每条段落必须逐条核对并覆盖本实体的“最低必要维度”：
...
- **全局变体与继承链契约 (Global Dependency Strategy)**：处理派生变体（换装/老龄/破损/正反打等）时：基准实体设置 `dependency_strategy.type=Original`, `visual_dependencies=[]`；派生实体设置 `type=Type A/Type B` 并指向上一阶段。提示词中必须显式写清“继承了哪些不变的特征锚点，当前仅呈现什么新变化”，确保变体在文本中也被视觉化。**绝对禁止在 `visual_dependencies` 中填入 `S001`、`E001` 等 `subject_no`，实体名引用必须逐字符一致 (如 `CHAR:[@...]` 或 `PROP:[...]` 等)！**
...
- **全局视觉舒适度与模型服务商政策合规边界规则（适用于所有描述及锚点，极度严格）**：所有涉及资产外观设计的提示词，必须充分注意避免违反各AI模型提供商（如Midjourney、OpenAI、Stable Diffusion等）的内容安全审查政策。各实体的 `description`、`appearance`、`generation_prompt_cn/en` 及其他描述必须采用完全安全、可播出、非血腥、非恶心、非强视觉不适的温和表达。**绝对禁止描述任何血腥、断肢、内脏、严重伤痕、肉体变异、令人作呕的污物或任何可能触发平台封禁的涉暴/涉黄/猎奇词汇**。如因剧情强烈需要表现“受伤/战损/战后”，必须改写为完全非图形化、极度克制且意向化的描述（如“轻微擦痕”、“衣服破损”、“灰头土脸”、“疲惫的神态”），严禁描写具体的伤口形态或流血量。此规则同样严格适用于 `anchor_description`，锚点仅许可稳定且安全的身份识别点。此条款优先于一切“写实需求”。

### 1.5 全局最高审美与防平庸规则 (Global Aesthetic & Quality Baseline)
- **非现场舞台剧限制与极致审美想象力（新增强制）**：必须绝对应知所有的角色服化道、场景与环境设计**不是真实的现场舞台剧、主题乐园或低成本实拍剧**！在符合故事逻辑的前提下，**绝对不要考虑现实中的服化道制作成本、物理实现限制或舞台拍摄局限性**。必须展现出极具高度、远超常规影视美术预算的顶级审美想象力。对于任何题材（尤其是古装、仙侠、科幻、奇幻），服装面料的奢杂程度与剪裁结构、道具的细节工艺与奇特功能、环境的无穷空间尺度与视觉奇观，都应向最顶级的史诗级电影概念艺术、高定时尚或数字造梦艺术看齐，彻底粉碎任何现实服化道的塑料感、布景感、廉价感和局促感。
- **无条件朝最高审美收敛**：所有角色、场景与道具必须在符合剧情、时代、阶层、地域语境与写实度的前提下，自动指向“当前条件下的最高审美/最好看版本”。若审美优化与剧情事件冲突，必须平衡处理；若无冲突，确保呈现高质感、经打磨的专属视觉解。
- **审美与剧情设定的平衡策略（新增强制）**：必须平衡剧情需求与审美要求。绝不能为了体现主角落魄、身处底层或环境的贫穷，就极端化地将其设计得过度脏乱差、不堪入目或引起生理不适。角色即使穷困也应保持基本的整洁与镜头美感（如粗布衣服也可剪裁得体、穿着规范）；环境即使破旧也必须具备高审美的废墟美学、光影层次与陈设秩序。始终保持影视呈现所必须的艺术化处理。
