...
- **`generation_prompt_cn` 开篇**：首段写入上述维度的可执行视觉转译；风格定位独立可检索，并跟 **2–4** 条可视化落点（尺度/配色/光气质/材质/构图/情绪外显）。已有维度不得省略。CHAR 服饰特征槽须落入【衣着】穿戴句；ENV 场所特征槽须落入全局围合句+四面后景；PROP 命中机甲/机车/军械/舰机等须 `器类=` 全槽落入结构/工艺画面句（见各分型）。
...
- **实体物件可视性核验（最高；描述前强制）**：对任一实体写 `generation_prompt_cn` / 角色【衣着】配饰落点时，须先核「该细节在对应视图是否可见」——①道具四视图：第一/第二合成格写该对照视角可读细节+尺度参照，背面格写背面结构，侧面写厚度/侧缘，禁正面铭文出现在背视面板；②角色四视图+配饰：仅**不可离身**升格配饰（CHAR `accessory_props`）与未升格纯装饰按 Index `accessory_mount`/`wear_side` 落位——正面挂件出现在正视/¾，背面挂件出现在背视，侧面挂件在对应侧视可读；可离身升格 PROP 不进角色生图词；命中 character §1.1A 时第三宫侧面另须可见线性参照+尺寸标注（参照不是配饰、不进挂位合同）；③ENV 四向/衍生：**扇区立面**的可见面写入对应 `{N}度…` 衍生的 `anchor_description` 与该宫格成稿；立面不可见面严禁写入该衍生锚点/生图词（禁对向串写立面、禁机位后方冒充可见）；**中区地面家具（茶几/矮几/中央桌/沙发/椅/轮椅）四格守恒**——开篇锁 `锚=`/`宫格=`（现场编排同套 XOR+3×3，主语=家具），四面只写切角投影落位+可见面，实体不得因换角从成稿消失（全挡须显式遮挡，禁静默删除；任一格不得漏开篇已锁座具；不得把靠 `M` 墙座具在 `N=(M+180)` 格写成邻墙深处或当成邻向立面省略）；挂靠小件随宿主入镜，禁顶部出画死角只出现一格；**中区台面可数件四格同核 `数量=N`**（禁一格蒸发、禁改数、禁同款克隆；禁 CHAR 宠物/已抽 PROP 上台）；**保持约 35mm–50mm 适度景深与空间纵深感，正对输入该向主景（四方正交），左右两侧立面与侧向结构在纵深透视中基本可见**；**场内实体按几何投影规则动态变换落位与朝向（座椅 F 查座椅可见面表，与楼梯 U 光学相反，禁套楼梯表导致 0°/180° 椅面互翻；长短边、楼梯上行方向等），跨视角守恒**；座椅惯例朝向：主位朝门、客位朝桌心、驾驶座朝仪表盘（F=0），禁无据背对大门或背对仪表盘；**输入该向内容=该向正面面对的围合立面，四方正交，禁偏斜/墙角主构图**；④遮挡/Clean Plate：不写镜头外、板外、被遮挡才「应存在」的细节当成本视图可见事实。缺挂载/朝向上游证据→标缺口，禁臆造可见面。
...
- **色彩**：主/辅/点缀/过渡色绑材质、光源、距离层；**须先核销全局 `color_palette`**（无则 Env 主动按顶级大片锁卡）；**Env 辅色与点缀须与主色强烈对比**，FG/MG/BG 纵深色层须有立体感与对比度，防单调发灰；执行见 §1.5 与 environment §2.9。
- **中文提示词维度**：连贯自然中文短句/短段。
  - Env/Prop 推荐序：主体 → 主光源与光影 → 构图机位 → 调色 → 细节 → 风格约束。
  - Character 定妆序（强制）：开篇其他（画布+项目风格+拍摄载体）→ 相貌 → 衣着 → 光线（≤1 句）→ 收尾其他（姿态+四视图）；权威 character §1.3。
  - 最低维度——Env：机位、观察朝向、前景/中景/背景、光照、材质/空间、去人物化。Character：固定机位、全身含鞋、四格顺序、稳定锚点、服装一致、浅灰底。Prop：固定机位、第一/第二宫尺度对照合成格、侧/背单体、材质锚点、柔和静物光、单状态、浅灰底。
...
**推导输入**：Backfill 的 `Global_Style` / `tone` / `lighting` / **`color_palette`（首要色卡，不得冲突）** / **`color_spectrum`（须与色卡同向）** / `borrowed_films` + Context 的 Genre、定位、时代/地域 + **Index `basic_positioning`（ENV 定位目标）+ `scene_mood` / `scene_mood_cue`（场级环境主情绪；光色构图须响应，不得与色卡主冷暖打架）**。Node 1 前一次把已锁色卡落成全剧大光比与主冷暖色谱，再按定位目标与场级情绪微调各实体落地；禁各 Subject 各自发明冲突色系或回退均匀平光。缺 `color_palette` → logic 标 `upstream_missing_color_palette`；**Env 必须主动锁卡**（禁止弱推交差）：优先用 `borrowed_films`，无则自锁 1 部真实顶级大片（院线/经典，须有可核验标志性调色），写出该片可核验的主/辅/点缀/过渡具名色 + 调色机制，`对标片=` 点名该片；禁只写冷暖标签、禁另造与题材/`color_spectrum` 冲突的色。
...
2. **四层具名色**：先抄 Stage 1 `color_palette` 的主/辅/点缀/过渡，再绑**具名表面+材质**落地；可按场级情绪微调明度/饱和，禁换主干色相。缺色卡则按上条主动锁的顶级大片写出同向具名色。主色 1–2｜辅色 1–2｜点缀 ≤2（面积约 3%–10%，**Env 默认至少 1 个提神点缀**）｜过渡 1–2；禁抽象词。**Env 辅色与点缀须与主色强烈对比**（色相或冷暖对冲，禁同灰不同明、禁同族邻色弱差冒充）；画面须有立体感与对比度。
...
2. 已继承 Stage 1 `color_palette`（或标回流后由 Env **主动按顶级大片锁卡**，非弱推）并回写主冷暖+四层色谱（Character 色谱落服装；大光比亮暗句仅 Env/Poster prompt 与 Character logic）？
...
6. **Env 色彩美观有对比**：已继承全局色卡或主动锁顶级大片 `对标片=`？辅色与点缀相对主色强烈对比（非同灰不同明）？FG/MG/BG 纵深色层已写且有立体感/对比度？无灰褐单色糊？
