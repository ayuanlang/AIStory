...
- **Character / Prop 四宫格画幅**：`generation_prompt_cn` 须 16:9 横向、浅灰色背景、四视角同一横排、连续统一浅灰画板。禁上下两排、2×2、换行断裂、错层。留白开阔；第一宫（面部/细节特写）占横向 35%、纵向居中；其余三宫（正/侧/背全身）共享 65%。**Character 全身格隐脸（最高；仅单人四视图）**：仅第一宫生成完整可读面部；第二/三/四宫全身**禁止生成脸部**——面部区域（发际以下至下颌，含眼鼻口颊）须用与画板相同的浅灰底色完全填覆，呈无脸；禁五官、模糊脸、马赛克、黑条、半透明脸。发型轮廓、颈、衣着、体态、鞋履仍须完整。侧视可见面庞同样覆盖，可留耳廓与发际；背视保留后脑发型、不露脸。群演簇与 Prop 不适用。**群演簇不适用本条四宫格**——改为 character §2.4：单张**全身**群体设定图（鞋履可见；有坐骑则人+骑同框全身），人数与构图跟剧情群体建置形式，不限 4 人/4 格；**衣着与附属物（配饰/器械/旗帜/坐骑）细节下限与单人相同**（`clothing` ≥7 且 >3 色，禁制服/军装/持械空壳）；**不抽独立 PROP**，一律写进该簇 `clothing`。`generation_prompt_en` 固定 `""`。环境/海报画幅见分型文件。
...
- **Character `clothing`**：匹配项目语言语境、身份、时代；含定位、流行穿搭方向、版型/材质/配色、可复用风格词；无法检索时用通用时尚知识给可信参考。**群演簇不豁免本条**——簇共性装束与附属物须写满与单人/道具同等槽（款式/版型/材质/配色/鞋履/配饰/器械/坐骑）；**禁止**为簇另挂 `PROP:[…]`；军警/仪仗/甲胄另须远读剪影+近读徽识/甲片/靴履/器械（见 character §2.4）。**成人向年轻女性**剪裁关键词（露肩/深V领/开叉/短裙/短裤/短靴/露背）见 character 分型；`clothing_req`/制服/礼制仍优先。
