...
- **三选一互斥执行（Mandatory）**：一旦判定为三维动画或二维动画，本文件与各实体文件中**所有**标注"真人写实/实拍/Live Action/Photoreal"触发的条款（含毛孔纹理、matte-to-satin 分区光泽、演员选角参考、无感打光七必锚等）**全部不生效**，改为执行对应实体文件中的「三维动画专项」或「二维动画专项」章节；三维/二维项目仍须遵守本文件通用的项目语境注入（§1.2A）、光影大光比与冷暖色谱（§1.5，按各自渲染语言转译，如三维走 PBR 高光/环境反射、二维走块面明暗与色块过渡）、审美基调（§1.4）与四宫格/画幅基线（§1.1）。
