...
- **群演簇例外**：命中 §2.4——**不**走单人四视图/四宫格；按剧情群体建置形式出一张群体设定图（人数与队形跟上游，不限 4 人/4 格）。不适用全身格隐脸。
...
- **细节下限**：单人定妆面目段 ≥8 锚点；`appearance_cn` ≥6；`clothing` ≥7 且 >3 色；三字段合计 >8。**群演簇不适用单人 8 锚点**——改走 §2.4：簇共性 1 句 + 前排可读个体**逐人面容卡**（禁止把 8 锚点写成一张平均脸再铺满阵列）。
...
- **机位**：平视静态站姿、四格横排；焦段见 §2.3 层5。全身格隐脸见 §1.1。
...
- **画幅与构图（最高；不锁四宫格）**：16:9 横向、浅灰色连续画板；**禁止**强制套用单人「四宫格/四等分横排/第一宫特写」模板；**禁止**为凑 4 格而把群体拆成固定 4 人四格。成稿=**一张群体设定图**，构图直接体现上游群体建置形式（见下）。
...
- **先判制式（读 Index `服饰倾向` / 身份 / 时地礼制 / 上游明文）**：
  1. **普通人群（默认）**：无制服/制式/军警僧道仪仗等统一着装要求 → **服饰 + 相貌均须差异**。衣着：款式/配色/新旧破损/配饰 **≥2 类**可分，且前排可读者各有可核销着装差，不得像同一套服装复制品。相貌走上面面容卡。
  2. **制服/制式类**：上游明文或身份制度要求统一着装（军警/卫兵/僧侣/仪仗/校服/工装/甲胄同制等）→ **服饰统一**（同款制式、同主色与徽章层级；允许品秩微差如袖章/肩章/新旧磨损，但廓形与制式识别须一致）；**面容卡仍强制、不得因制服同脸**。阵列几何仍服从上游整齐队形。
  3. **异兽/神兽/魔兽/妖兽/灵兽/坐骑群等非人集群**：无「衣着」时以皮毛/鳞甲/角冠/体型斑纹/伤痕/光泽/体态落实——同种同系但**禁止**同模复制；每张可读卡 **≥4 处**可核销特征差（体型大小、斑纹位置与形状、角叉数/角向、鳞色深浅、伤疤位点、耳形、瞳色或虹膜纹、姿态气质等），相邻个体禁止同一斑纹模板。若上游要求鞍辔/甲胄/符纹**制式统一**，则装具统一、本体特征仍须分异；群体落位仍跟上游建置形式。
- **【相貌】结构（强制）**：①簇共性 1 句（族群/时代气质/是否制式发饰，**不含**一张标准五官）→ ②逐条面容卡（站位+8 槽短句）→ ③远景「参差延续、禁克隆」1 句。缺逐条卡或仅有共性=废弃。
- **`negative_prompt_en`（群演强制置前，可并入既有串）**：`identical faces, cloned extras, duplicate faces, same face repeated, copy-paste crowd, twin extras, uniform beauty-filter faces, identical bone structure, same hairstyle everyone`。人态另保留真人 Tier1 肤质词；三维/二维改用对应假人/同模词，仍须含 identical/cloned/duplicate faces。
- 仍须 §1.2A（群演走面容卡下限）、分段标签、光线 ≤1 句、色调柔和。
- 单条 `characters[]`；logic 须可检索：`群演簇：{制服类|普通人群|非人集群}｜建置形式=…｜规模=…｜面容卡数=N｜随机种子={SceneID}|{簇名}｜龄档跨度=…｜性别穿插=…｜与主角骨相碰撞=否｜非四宫格群体图；差异：{服饰+相貌|仅相貌/外形|装具统一+本体特征差}`。
...
- 群演：按 §2.4（**非四宫格**；人数/队形跟上游；**【相貌】禁止一张平均脸**；前排逐人面容卡 8 槽互斥+随机抽样；普通=服饰+相貌差；制式=服饰统一+面容仍差；非人=特征卡 ≥4 差）。
