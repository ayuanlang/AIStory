...
- **细节下限（统一）**：`generation_prompt_cn` 面目段 ≥ **8** 项可检索五官/肤质锚点；`appearance_cn` ≥6 稳定外貌锚点；`clothing` ≥7 可见服饰细节；三字段合计细节 >8。
...
- **服装**：`clothing` 首句固定「播出安全等级：成人|非成人。」；服饰的设计要符合朝代，地域的特征；须含 1 组潮流/风格关键词 + 版型/材质/配色/鞋履可见落点，并回写进 `generation_prompt_cn`。
