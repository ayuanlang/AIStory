...
- 服装时尚对标强制写回：Character 的 `clothing` 字段应详细写入“时尚对标”信息，并与当前项目语言语境、角色身份和时代现实一致。**`clothing` 第一语句必须先声明“播出安全等级：成人。”或“播出安全等级：非成人。”，后续内容再据此展开服装设计。**`时尚对标` 至少要包含：1) 基本角色定位，如“女性教师 / 女性调查记者”；2) 当代流行穿搭方向；3) 与该身份相容的版型/材质/配色参考；4) 可复用的风格关键词。若无法外部检索，必须基于当前通用时尚知识给出可信的当代高级参考。
...
- **特定字段转化规范（从概念到画面）**：
  ...
  - `appearance_cn` / `clothing`：具体版型、材质与配色必须全部进入 Prompt。`clothing` 的第一句必须固定先写“播出安全等级：成人。”或“播出安全等级：非成人。”，随后再按该等级展开服装设计；当“播出安全等级=成人”时，必须按“男频/女频唯一口径”写入衣着分流特征，并在 `clothing` 与 `generation_prompt_cn/en` 中保持一致表达。
  ...
  "characters": [
      {
          ...
          "clothing": "播出安全等级：成人。当前服装：深海军蓝深V领吊带上衣，外搭修身截短款轻机能短夹克，下身穿高腰侧开叉黑色短裤与及膝平底皮靴。时尚对标：角色定位=都市调查记者；当代风格参考=实用机能风（Techwear）与高级极简主义；版型/材质/配色关键词=V领、吊带、开叉、短裤、高腰修身、利落实用、深蓝与冷灰渐变、防水哑光材质。其他剧本衣着描述：无。",
