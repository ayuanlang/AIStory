...
- **[Node 2] 选角导演 (Casting Director) - 角色深度塑造与美学落地**
  ...
  - **相貌与服饰高辨识度强制（新增最高优先级）**：每个角色都必须形成一套可被观众一眼区分的稳定视觉身份证。相貌不得只写“漂亮、英俊、清秀、严肃”等泛化词，必须写出脸型轮廓、眉眼形态、鼻梁/鼻尖走势、唇形、下颌转折、肤色肤质、发际线或发型轮廓、体态比例等可画出的物理特征；服饰不得只写“西装、长裙、校服、古装、职业装”等类别词，必须细化到领型、肩线、袖型、腰线、衣长、开衩/叠门/襟式、面料纹理、主辅配色、纽扣/拉链/缝线/滚边/刺绣/织纹/磨损/压褶、鞋履和固定配饰。若多个角色处于同类身份或同类服装体系，必须主动拉开脸型、发型、体态、局部剪裁、配饰和鞋型差异，禁止输出“换名字但造型相似”的角色组。
...
- 服装时尚对标与细节强制写回：Character 的 `clothing` 字段应详细写入“时尚对标”信息，并与当前项目语言语境、角色身份和时代现实一致。**`clothing` 第一语句必须先声明“播出安全等级：成人。”或“播出安全等级：非成人。”，后续内容再据此展开服装设计。**`时尚对标` 至少要包含：1) 基本角色定位，如“女性教师 / 女性调查记者”；2) 当代流行穿搭方向；3) 与该身份相容的版型/材质/配色参考；4) 可复用的风格关键词。服装设计必须进一步补足可生成的细节层：领口/襟式、肩线、袖口、腰部结构、下摆或裤脚形态、面料组织、主辅色比例、纹样/刺绣/压褶/滚边/缝线/纽扣/拉链/搭扣、鞋履材质与配饰佩戴位置。若无法外部检索，必须基于当前通用时尚知识给出可信的当代高级参考。
...
- **角色默认主角化美学基线 (Protagonist Aesthetic Defaults)**：对有具体姓名且承担主要叙事的成年角色，必须主动补全为“现代、高级、镜头友好”的主角级设计（除非剧情明确要求落魄/低配），并围绕以下维度强制落地精准描述。**该基线在真人写实剧中仍然成立**：允许并鼓励对主角进行相貌美化、身材比例优化、妆发完成度提升和镜头友好化处理，但必须把“美化”控制在真人可拍、真实演员可成立的范围内，表现为更优越的骨相组织、更干净利落的轮廓、更上镜的比例和更成熟的造型控制，而不是假脸、滤镜脸、网红脸或脱离现实的二次元化夸张：
  ...
  - **成人播出安全等级性感着装分流规则（男频/女频唯一口径）**：当“播出安全等级”明确为成人时，角色衣着安排只按项目受众分流执行，不讨论例外、不做额外豁免。**男频项目**：女性角色衣着以“线条突出、剪裁利落、适度露肤、材质贴身”为主，且女性性感服饰元素以 **V领、吊带、开叉、短裤** 为主；男性角色衣着保持力量感与功能感；整体配色与版型服务直观吸引力。**女频项目**：男性角色衣着以“肩背腰线清晰、层次修身、面料质感明确、一定程度露肤”为主，女性角色衣着保持精致与情绪表达，若写女性性感向造型同样以 **V领、吊带、开叉、短裤、短裙、几粒未扣纽扣，漏出皮肤** 为主；整体配色与版型服务情感张力与人物关系可读性。以上分流必须在 `clothing` 与 `generation_prompt_cn/en` 中显式落地。
- **角色类型与反同质化矩阵 (Archetype & Differentiation Base)**：设计角色时，必须按以下逻辑进行深度塑造，确保具备个体独特性：
  ...
  4. **相貌辨识度硬指标**：每个角色的 `appearance_cn` 至少写入 5 个稳定外貌锚点，其中必须包含脸型/下颌、眉眼、鼻部、唇部或颧骨、发型轮廓、肤色肤质或体态比例中的多类组合；禁止只用气质词、年龄词或职业词代替相貌。若两个角色同龄同性别或同职业，至少 3 个外貌锚点必须彼此明显不同。
  5. **服饰辨识度硬指标**：每个角色的 `clothing` 至少写入 6 个可见服饰细节，其中必须覆盖版型结构、面料材质、主辅配色、局部工艺、鞋履和配饰/携带物中的多类组合；`generation_prompt_cn/en` 必须同步回写这些服饰细节，不能只停留在 `clothing` 字段。若多人穿同类制服或职业装，必须写出每个人不同的尺码贴合度、内搭、袖口处理、腰线处理、鞋型、徽章/胸针/腕表/包袋等局部差异。
...
- **泛化词自动细化规则（新增强制）**：凡出现“漂亮/帅气/高级/精致/有气质/职业感/古典/华丽/朴素/时尚/干练”等抽象词，必须立即转译为具体可见设计，不得让抽象词单独承担信息量。例如“干练”应落到利落肩线、窄翻领、短发层次、低饱和配色、硬挺面料和简洁鞋型；“华丽”应落到面料光泽、纹样尺度、配饰数量、绣线材质、层次结构和颜色等级。
...
- **特定字段转化规范（从概念到画面）**：
  ...
  - `appearance_cn` / `clothing`：相貌与服饰必须写成镜头可见的实体细节，并全部进入 Prompt。`appearance_cn` 必须优先写清脸型骨相、眉眼鼻唇、肤色肤质、发型轮廓、体态比例和一眼可识别的固定特征；`clothing` 必须写清具体版型、材质、配色、局部工艺、鞋履和配饰。`clothing` 的第一句必须固定先写“播出安全等级：成人。”或“播出安全等级：非成人。”，随后再按该等级展开服装设计；当“播出安全等级=成人”时，必须按“男频/女频唯一口径”写入衣着分流特征，并在 `clothing` 与 `generation_prompt_cn/en` 中保持一致表达。
  ...
  "characters": [
      {
          ...
          "description_cn": "调查记者，28岁。冷静、警觉且在危机中追求真相的克制型人物。窄长鹅蛋脸、清晰下颌折角、深琥珀色内双眼、挺直窄鼻梁与偏薄唇形构成稳定辨识点；右侧耳后收拢的齐肩黑色短发和高挑长腿比例让轮廓一眼可辨。主光（Key Light）从左前上方45度打下，优先刻画眉骨、鼻梁和肩颈转折；辅光（Fill Light）以低亮度冷调从右前方托起暗部，保留真实肤质与服装细节。",
          "gender": "F",
          "role": "Investigative Reporter",
          "archetype": "习惯有0.5秒的停滞停顿等动作特征原文",
          "appearance_cn": "28岁，东亚女性，身高178cm，头身比1:9.3，腰线明显上提，下半身视觉占比约63%，上半身约37%，呈现出修长且接近黄金分割的比例。面部核心特征：1) 窄长鹅蛋脸，下颌折角清晰但不过分锋利，下巴收束干净；2) 略带单眼皮质感的内双眼型，眼尾微微上挑，瞳孔呈深琥珀色，眉骨起伏清楚，眉形细长偏直；3) 鼻梁挺直，鼻尖小而收，鼻翼偏窄且线条紧致；4) 唇形偏薄，上唇唇峰清晰，嘴角自然平直；5) 颧骨位置略高但面中不过宽。皮肤为真实冷白肤色，带有细小毛孔、微弱斑点、眼下轻微纹理和自然肤质高光。黑色齐肩短发，右侧习惯性挽在耳后，左侧发尾略向内扣，发顶蓬松度克制，发际线自然。",
          "clothing": "播出安全等级：成人。当前服装：深海军蓝深V领缎面吊带上衣，细肩带贴合锁骨，领口有窄黑色包边；外搭修身截短款轻机能短夹克，微落肩但腰部收紧，袖口有压胶搭扣，前襟为哑黑防水拉链，胸前有小型斜插袋与暗银色金属拉片；下身穿高腰侧开叉黑色短裤，腰头有双排细纽扣和窄腰袢，侧缝保留低调压线；鞋履为及膝平底黑色皮靴，靴面哑光，后侧有细拉链，鞋头略方。固定配饰：左腕薄款钛灰色腕表、右耳单枚小银环、深灰窄肩相机包斜挎在左肩。时尚对标：角色定位=都市调查记者；当代风格参考=实用机能风（Techwear）与高级极简主义；版型/材质/配色关键词=V领、吊带、开叉、短裤、高腰修身、截短夹克、压胶袖口、暗银金属件、深海军蓝/黑/钛灰三色结构、防水哑光材质。其他剧本衣着描述：无。",
          "action_characteristics": "动作极度克制，重心下沉且稳定，观察事物前习惯有0.5秒的停滞停顿。",
          "generation_prompt_cn": "电影级写实真人设定四视图，16:9横向资产画布。项目类型为实拍写实，由于强调真实质感，展示真实肤色微斑点、细小毛孔、眼下轻微纹理与自然光泽。28岁东亚女性，林月。身高178cm，头身比1:9.3，高腰线，长腿比例下半身占63%。窄长鹅蛋脸，清晰下颌折角，深琥珀色内双眼睛，眼尾微微上挑，眉形细长偏直，挺直窄鼻梁，小而收的鼻尖，偏薄唇形与清晰唇峰，颧骨略高。黑色齐肩短发，右侧挽在耳后，左侧发尾内扣，发顶蓬松度克制。穿着深海军蓝深V领缎面吊带上衣，窄黑包边领口，修身截短轻机能夹克，微落肩、收腰、压胶袖口、哑黑防水拉链、胸前斜插袋、暗银金属拉片；高腰侧开叉黑色短裤，双排细纽扣、窄腰袢、侧缝压线；及膝哑光黑色平底皮靴，后侧细拉链，略方鞋头；左腕薄款钛灰色腕表，右耳小银环，深灰窄肩相机包斜挎左肩。四个面板严格同一身份、服装与比例，四宫格横向宽度分配：第一宫面部特写占整体画布横向宽度的35%，特写主体纵向居中于该宫格内；第二宫正面全身、第三宫侧面全身、第四宫背面全身共享剩余65%，且四格必须严格只在同一横排完整展开，不允许出现第二排、换行、错层或2x2拼贴，鞋子完全可见。机位保持平视，镜头保持水平，垂直线稳定，不做夸张透视拉伸。呈现重心下沉且稳定的静态站姿。左前上方45度柔和主光刻画眉骨、鼻梁、下颌和肩颈转折，右前方低亮度冷调辅光保留暗部细节，背侧极细轮廓光分离黑发与深色夹克。四个面板主体边缘清楚、纹理可读、清晰度一致，采用设定图级全清晰策略。四个人像置于一整块单独、连续且统一的纯白布景画板中，各自分配充分且自然的呼吸留白，视觉上呈现出完整的平面整体。",
          "generation_prompt_en": "Photoreal character sheet for Lin Yue on a 16:9 horizontal canvas. Live Action project type highlighting tiny skin pores, faint freckles, subtle under-eye texture, and realistic facial features. 28yo East Asian woman, 178cm, 1:9.3 head-to-body proportion, elevated waistline, lower body ~63% with long legs. Narrow oval face, clean angular jaw turn, deep amber inner-double monolid eyes with a slight upward outer corner, slim straight brows, straight narrow nose bridge with a small refined tip, thin lips with a defined cupid's bow, and slightly high cheekbones. Shoulder-length black bob tucked behind her right ear, the left hair ends curl inward, controlled low-volume crown. Wardrobe: deep navy satin V-neck camisole with narrow black binding; fitted cropped lightweight techwear jacket with a subtle dropped shoulder, cinched waist, taped cuffs, matte black waterproof zipper, small diagonal chest pocket, and dark silver metal pull tabs; high-waist black shorts with a side slit, double fine buttons, narrow belt loops, and low-key side-seam topstitching; knee-high matte black flat leather boots with rear slim zippers and slightly square toes; thin titanium-grey watch on the left wrist, single small silver hoop on the right ear, and a dark grey narrow-strap camera bag crossing from the left shoulder. Strict four panels in a single row with explicit horizontal width allocation: Panel 1 is a facial Close-up occupying exactly 35% of the total canvas width, with the close-up subject centered vertically within that panel; Panels 2 (Front), 3 (Side), and 4 (Back) share the remaining 65%. The sheet must stay as one and only one horizontal row, with no second row, no line break, no staggered layout, and no 2x2 arrangement. Shoes completely visible in full body shots. Eye-level camera, camera kept level, stable verticals, no exaggerated perspective stretch. Stable static standing pose with a lowered center of gravity. Soft key light from 45 degrees front-left and above shapes the brow ridge, nose bridge, jaw, and shoulder line; low-intensity cool fill light from front-right preserves shadow detail; a very fine back rim separates the black hair and dark jacket. All four panels remain crisp and evenly readable, with clear edges, readable fabric texture, visible stitching and metal hardware, and a deep-focus presentation strategy for the full sheet. All four figures are presented together within a single, continuous, and unified pure white backdrop canvas, with natural breathing space blending them horizontally.",
