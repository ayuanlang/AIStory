...
- **群演簇例外**：命中 §2.4——**不**走单人四视图/四宫格；按剧情群体建置形式出一张群体设定图（人数与队形跟上游，不限 4 人/4 格）。
...
- **画幅与构图（最高；不锁四宫格）**：16:9 横向、纯白（或极浅灰）连续画板；**禁止**强制套用单人「四宫格/四等分横排/第一宫特写」模板；**禁止**为凑 4 格而把群体拆成固定 4 人四格。成稿=**一张群体设定图**，构图直接体现上游群体建置形式（见下）。
...
- 真人：`description_cn=""`；`dependency_strategy.logic` 含内部光学 rationale + **`场级情绪`/`服化响应`** + 选角参考；`generation_prompt_cn` 含分段标签 + 七必锚 + 五层可检索 + 光线 1 句；【衣着】配色可核销 `scene_mood_cue`；单人细节面目≥8。**群演簇**：不套七必锚全量/单人 8 锚点；须面容卡数达标、【相貌】含逐人 8 槽、`negative_prompt_en` 含 identical/cloned faces、logic 含随机种子与龄档跨度；否则废弃。
