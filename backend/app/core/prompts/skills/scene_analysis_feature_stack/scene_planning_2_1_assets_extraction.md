...
### 四、环境组（ENV）与空镜
- **主环境 vs 可拍衍生（强制分层）**：
  - **主环境（基准定义，Beat 不可用，生图参考源）**：`env_role:主环境基准定义`；`referenceable:No`（Beat 不可直接引用）；`generatable:Yes`（Stage 3 输出空间基准参考图）；只写空间骨架、**0 度坐标轴**、**俯视全局 360** + **仰视全局 360**（四向+中心，可不写 FG/MG/BG；两视角对称一致）、固定实体清单与实体间关系；**禁止**写某一机位可拍空镜、**禁止**作为 Beat 当前 ENV；**是全部衍生生图参考图唯一来源**。
...
- **纯空镜提取原则（强制，本阶段 ENV 最高优先级之一）**：`environment` 行只描述**无人、无临场道具、无交互痕迹**的可复用拍摄空间。Stage 1【主环境】/【衍生环境】说明、Beat 环节 1 环境建置、以及任何上游自然语言环境段落，**常夹杂角色站位、人称、肢体/视线/动作、持握物、可移动物件与交互状态**——提取 ENV 时须**主动剥离**上述内容，**禁止原样带入** `entity_attributes` 任一字段。
  ...
  - **输出前空镜终检**：逐 ENV 行检索是否残留人物名、人称、肢体/动作/视线、持物、可移动物件或已单列 `PROP` 同名同物；有则删并归位至 `CHAR`/`PROP` 后再输出。**跨衍生朝向一致终检**：同一具名固定实体在全部视角/状态衍生中的世界空间朝向、开闭态、锚点相对关系须与主环境一致（仅可见面/左右序位可随机位改写为锚点或视角表述）；**禁止**无依据改变桌头/门轴/椅背朝桌心等物理朝向——**唯一例外**：状态/特效衍生或 Stage 1/Beat 明示受力移位且已写 `empty_view_delta`。
...
- **视角衍生行（含 `0度{主环境名}`）**写 `view_angle_from_main`、`empty_view_delta`、FG/MG/BG 纯空间层次（**按 Stage 1 §11：BG=正前方远端/N 度方位，MG左=(N+270)°/MG右=(N+90)°，FG=视轴纵深近端；四向—景深交叉校验；轴线回转公式仅内部推演**；每层每固定实体须写清层位+左/中/右+垂直上/中/下即高/中/低，与主环境垂直区段一致），**并强制写 `derivative_view_360_entities`**（**每条衍生 ENV 独立一份**）：以**本衍生观察轴为 0 度**、**顺时针**用**自然语言**写 **0/90/180/270 四段**（每段 **【{N}度方向】** 起头）。**成稿规则（与 Stage 1 §11 跨阶段契约一致，不重述）**：四向 **【{N}度方向】仅用本衍生自有角度**；四向与 FG/MG/BG **只写本机位推导结果**；**方向性实体成稿须锚定具名门/窗/桌/墙段等，锚点不可见时用相对镜头/Observer View 视角表述**——**禁**成稿写度数回转/公式/轴线回转向/「角度镜像重判」；要素完整（FG/MG/BG+左/中/右+上/中/下+具名定语+开闭态+锚点朝向/正反面+实体关系+可见/不可见）；**方向性实体类目见 Stage 1 §11「方向性实体参考清单」**。**不同衍生行四向描述禁止照抄**。**内部推演（禁入成稿）**：同角继承、转角对照、镜像映射、分层推演、交叉校验、轴线回转=(主环境基准朝向角−N) mod 360、OTS 两步确认——细则 Stage 1 §11。**须写 `spatial_axis`**、**`lens_profile:Wide|Standard`**、**`axis_crossing`**。**`referenceable:Yes`**。
- **主环境一致性（最高硬约束，推演用+成稿颗粒度）**：`derivative_view_360_entities` 四向中的**每一个具名固定实体**须**逐字可追溯**至主环境固定清单与俯视/仰视 360 拓扑；**全名、定语、开闭态、门轴侧、椅背/桌头/主客位、相对位置与方位归属须与主环境完全一致**——衍生**仅允许**写当前观察轴下的**可见/不可见**与**方向性物体锚点朝向/正反面/左右序位**（内部推演后须转写为具名锚点或视角表述，**禁**成稿写度数回转/公式）；**固定实体世界朝向跨衍生一致（强制）**：同一具名固定实体在**全部**视角/状态衍生中的**世界空间朝向、开闭态、锚点相对关系须一致**——各衍生仅可因机位不同改写**相对镜头的可见面/左右序位**；**禁止**无依据改变桌头相对侧墙、门扇开向与门轴侧、椅背朝桌心等。**唯一例外**：① 状态/特效衍生且已写清可持续结构/布局变化；② Stage 1/Beat 明示明显受力、撞击、拖拽、翻转、移位，须在触发 Beat/`empty_view_delta` 写清后后续衍生方可沿用新朝向。**禁止**增删改物体、改名、改开闭态/连接关系/材质尺度定语、另造主环境未列物件；**禁止**照抄 0 度 Master 或主环境对应方位的朝向表述；**成稿本身不得出现主环境角度回指**。
- **镜像映射对齐（内部推演+成稿锚点表述）**：衍生四向实体须与主环境保持**同等具体度**；空间逻辑**严格一致匹配**；成稿朝向须**锚定具名门/窗/桌/墙段等或锚点不可见时用相对镜头/Observer View 视角表述**；**禁止**四向/FG/MG/BG 沿用 0 度或主环境的椅背/门扇/桌向/主客位表述而不做锚点/视角重述。
...
- **视角衍生转角对照（内部推演，禁入 `derivative_view_360_entities` 成稿）**：`view_angle_from_main=N` 表示相对主环境 0 度**顺时针**转角；衍生 **0/90/180/270** 须对应主环境 **N/(N+90)%360/(N+180)%360/(N+270)%360** 拓扑做镜像映射对齐（细则 Stage 1 §11，不重述）。`empty_view_delta` 写当前机位可见半空间 Delta + 新进入/退出可见实体。**OTS/反打 ENV 两步确认（强制）**：**① N_对手 → ② (N_对手±180)%360**；禁止盲套 180°。**方向性物体（强制）**：四向/FG/MG/BG **成稿须锚定具名门/窗/桌/墙段等，锚点不可见时用相对镜头/Observer View 视角表述**；**禁**成稿写度数回转/公式/「角度镜像重判」；**禁止**照抄 0 度 Master 或主环境对应方位的朝向表述。
...
- **环境（ENV）**：
  - **主环境（基准定义，Beat 不可用，生图参考源）**：`subject_name_zh` = Stage 1 主环境名（不加角度前缀）；`base_entity` = `None`；`entity_attributes` 须含 **0 度坐标轴**、**俯视全局 360**、**仰视全局 360**（四向+中心，可不写 FG/MG/BG；两视角对称一致）、固定实体清单与实体间关系（**含垂直上/中/下**）；**必须**写 `referenceable:No`、`generatable:Yes`、`env_role:主环境基准定义`**禁止**写某一机位 FG/MG/BG 可拍描述。
...
- **逻辑一致性终检（推演用，禁写入成稿）**：输出前逐条视角衍生 ENV 核对——四向/FG/MG/BG 实体是否均可追溯至主环境固定清单；颗粒度是否达标；**成稿是否零主环境回指、零度数回转/公式**；**方向性物体朝向是否已转写为具名锚点或视角表述（禁成稿写角度镜像重判/轴线回转）**；FG/MG/BG 每层每实体是否均已写清垂直上/中/下；是否存在主环境未列物件或矛盾；任一不一致须修正衍生行或回流 Stage 1。
