...
- **有动作+对白角色 ENV 绑定与补位（强制，继承 Stage 1 §12）**：凡 Beat 内某 `CHAR` **同时**承担**具名主动作**（位移、递交/接触/拾取/格挡/Walk-and-Talk 走位等）与**画内对白/OS/V.O.**（含拆句各子句落点），Stage 1 已写或强暗示该角色须在**匹配观察视角**下可读口型/肢体/道具交互时，Index 须为其配置可复用的**主或衍生 ENV** 空镜资产；**禁止**仅留 Beat 落位而不落 ENV 行、或把 OTS/正反/POV 硬并入主环境而不建衍生行。
  ...
  - **多角色轮次**：正反打/轮次切换时，**当前说话人**与**当前主听众**若各自需要可读 ENV（如 A 侧 OTS 读 B 口型、B 反打读 A 口型），须**分别判定各对手可读角 N_对手**并落表或补位对应 `(N_对手+180)%360` 衍生 ENV，**禁止**多人共用错配主环境、禁止所有反打共用一个 180 度 ENV。
  ...
  - **正反例**：❌ 林医生 OTS 递文件并对陈医生说台词，Stage 1 Beat 写切桌后反打读陈医生口型，【衍生环境】未列 → Index 仅主环境「办公室会客区」。✅ 陈医生正面可读角 180° → 补位 `180度办公室会客区_桌后反打`。❌ 对手落位/readable 角 90° 却一律补位 `180度…_反打`。✅ 对手可读角 90° → 补位 `270度{主环境名}_反打`，`view_angle_from_main:270`。
...
- **环境（ENV）**：
  ...
  - **视角衍生环境**：`subject_name_zh` = `{观察角度}度{主环境名}`，如 `45度办公室会客区`、`180度办公室会客区`、`270度办公室会客区`；`view_angle_from_main` = 相对主环境 0 度的**顺时针**转角 N。**OTS/反打角（强制）**：N 须由对手可读角 **N_对手** 推导：**`(N_对手 + 180) % 360`**，禁止不经判定一律填 180。**转角对照（强制）**：衍生 0 度正面 = 主环境 N 度方位；衍生 90/180/270 = 主环境 `(N+90/180/270)%360` 度方位。`empty_view_delta` 须列新可见/不可见半空间；180 度衍生 BG 须对应 0 度不可见内容（N≠180 时按实际角位写 Delta）；方向性物体须按当前视角重判朝向。同 Scene 内同角度需区分多个观察区域时，追加 `_{衍生类型/观察区域/可见方向}`，如 `180度办公室会客区_桌后反打`、`270度办公室会客区_桌尾反打`。禁止 `主环境名 空格 衍生类型`、`_OTS_A/B`、A面/B面 等编号式命名。
