...
# Prompt Updated At: 2026-08-08 10:55:00 +08:00
...
- **群演簇 vs 龙套（最高硬约束；先判动作再落表）**：
  1. **群演簇门槛（须同时满足）**：Stage 1 以簇/阵/列/若干描述；**无具名个体**；无独立对白/OS；无跨 Beat·Scene 可持续身份；且簇内**每一个**成员均**无具体剧情动作**——仅统一反馈、簇状动势、氛围密度、齐进齐退/齐跪齐立等集体行为。→ **一条** collective `character`；名与 Stage 1 逐字一致；属性含 `plot_role:群演簇`、`crowd_role:群演簇`、数量区间、FG/MG/BG 分布、反馈模式、景别约束、服饰倾向。禁拆纯氛围 filler/编号个体；禁把具名角色并入簇。簇已入 Index 时 ENV `literary_atmosphere` 不得重复写同名簇密度。
...
- **人态化特效主体→CHAR（有可持续身份时）**：Stage 1 已具名且跨 Beat 可辨外形的**法相**/化龙凤兽/巨化体/拟人灵体等，若有独立叙事功能或可继承外观 → 提 `character`（`entity_kind:法相|化形|巨物|其他` 择一；`plot_role` 多为其他/配角）；命名与 Stage 1 逐字一致。仅单拍瞬时无定形介质→不提 CHAR，留 Beat。建置朝向等仍由 Stage 1 承载，本阶段只落可复用行。
- **结构化三键（每 CHAR 必填）**：
