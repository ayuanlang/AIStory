...
- **视角衍生行（含 `0度{主环境名}`）**写 `view_angle_from_main`、`empty_view_delta`、FG/MG/BG 纯空间层次（**按 Stage 1 §11：BG=正前方远端/N 度方位，MG左=(N+270)°/MG右=(N+90)°，FG=视轴纵深近端；四向—景深交叉校验；轴线回转=(主环境基准朝向角−N) mod 360**；每层每固定实体须写清层位+左/中/右+垂直上/中/下即高/中/低，与主环境垂直区段一致），**并强制写 `derivative_view_360_entities`**（**每条衍生 ENV 独立一份**）：以**本衍生观察轴为 0 度**、**顺时针**用**自然语言**写 **0/90/180/270 四段**（每段 **【{N}度方向】** 起头）。**成稿规则（与 Stage 1 §11 跨阶段契约一致，不重述）**：四向 **【{N}度方向】仅用本衍生自有角度**（与主环境同名度数不是同一参照）；四向与 FG/MG/BG **只写本机位推导结果**，自洽独立，**禁**照抄/提及主环境/Master/0度Master 各度方位与四向段落，**禁**主环境/基准/继承/映射/转角对照/分层推演/轴线回转向/「沿…观察轴」「顺时针右转」等回指；要素完整（FG/MG/BG+左/中/右+上/中/下+具名定语+开闭态+长边/桌头桌尾+正反面/椅背椅面/车头车尾/显示面+门轴侧+实体关系+可见/不可见）；**方向性实体须按 `(主环境基准朝向角 − N) mod 360` 做轴线回转+角度镜像重判**（PlannedReverse 另加重判可见面）；**方向性实体类目与须写朝向要素见 Stage 1 §11「方向性实体参考清单」**。**不同衍生行四向描述禁止照抄**。**内部推演（禁入成稿）**：同角继承、转角对照、镜像映射、**衍生 FG/MG/BG 分层推演**、**四向—景深交叉校验**、**方向性实体轴线回转**、OTS 两步确认——细则 Stage 1 §11；`view_angle_from_main=N` 时衍生【k°】↔ 主环境【(N+k)%360°】。**须写 `spatial_axis`（共同锚点）**、**`lens_profile:Wide|Standard`**、**`axis_crossing:None|PlannedReverse|MotivatedCross`**（见下条）。**`referenceable:Yes`**。
- **主环境一致性（最高硬约束，推演用+成稿颗粒度）**：`derivative_view_360_entities` 四向中的**每一个具名固定实体**须**逐字可追溯**至主环境固定清单与俯视/仰视 360 拓扑；**全名、定语、开闭态、门轴侧、椅背/桌头/主客位、相对位置与方位归属须与主环境完全一致**——衍生**仅允许**写当前观察轴下的**可见/不可见**与**方向性物体（椅背/椅面、门扇开向与铰链侧、桌头/桌尾/主客位、正反面/内外侧/文字面等）的朝向/左右/前后/可见面角度镜像重判**；**禁止**增删改物体、改名、改开闭态/连接关系/材质尺度定语、另造主环境未列物件；**禁止**照抄 0 度 Master 或主环境对应方位的朝向表述；**成稿本身不得出现主环境角度回指**（含照抄/提及主环境【N度方向】段落、Master/主视角/主0度BG 等）。
