...
- **镜像映射对齐（强制）**：衍生四向实体描述须与主环境保持**同等具体度**（材质/结构/尺度/工艺/定语/开闭态/连接关系等颗粒度**不得低于主环境**）；空间逻辑**严格一致匹配**；**唯一变化**为观察角度旋转后的可见/不可见分配与各角度**镜像映射对齐**；**禁止**简化、泛化或缩写主环境已有实体描述。
...
- **主环境一致性终检（强制）**：输出前逐条视角衍生 ENV 核对——`derivative_view_360_entities` 四向具名实体是否**全部**来自主环境固定清单；**各实体描述颗粒度是否不低于主环境对应方位**（禁止简化/泛化/缩写）；是否存在主环境未列新物件、改名、开闭态/门轴/椅位/桌向与主环境矛盾；衍生 k 度方向是否与主环境 `(view_angle_from_main+k)%360` 拓扑一致；任一不一致须修正衍生行或回流 Stage 1 补主环境。
