...
### 四、环境组（ENV）与空镜
- **主环境 vs 可拍衍生（强制分层）**：
  - **主环境（基准定义，Beat 不可用，生图参考源）**：`env_role:主环境基准定义`；`referenceable:No`（Beat 不可直接引用）；`generatable:Yes`（Stage 3 输出**俯拍全景**空间基准参考图，16:9 横幅，正上方鸟瞰）；只写空间骨架、**0 度坐标轴**、**头尾双锚点**（各从四向固定实体择一，分居 Viewing Direction 轴线两端）、**俯视全局 360**（四向+中心，可不写 FG/MG/BG）、固定实体清单与实体间关系；如有仰视全局 360 可一并写入供空间互证参考，但不强制；**禁止**写某一机位可拍空镜、**禁止**作为 Beat 当前 ENV；**是全部衍生生图参考图唯一来源**。
...
- **视角衍生转角对照（内部推演，禁入 `derivative_view_360_entities` 成稿）**：`view_angle_from_main=N` 表示相对主环境 0 度**顺时针**转角；衍生 **0/90/180/270** 须对应主环境 **N/(N+90)%360/(N+180)%360/(N+270)%360** 拓扑做镜像映射对齐（细则 Stage 1 §11，不重述）。`empty_view_delta` 写当前机位可见半空间 Delta + 新进入/退出可见实体。**OTS/反打 ENV 两步确认（强制）**：**① N_对手 → ② (N_对手±180)%360**；禁止盲套 180°。**方向性物体（强制）**：四向/FG/MG/BG **成稿须锚定四向固定实体**——继承主环境**头尾双锚**；优先挂可见头/尾锚，首选不可见时**须改挂对向尾/头锚**；头尾均不可见时才用相对镜头/Observer View 视角表述；**禁**成稿写度数回转/公式/「角度镜像重判」；**禁止**照抄 0 度 Master 或主环境对应方位的朝向表述。
...
- **逻辑一致性终检（推演用，禁写入成稿）**：输出前逐条视角衍生 ENV 核对——四向/FG/MG/BG 实体是否均可追溯至主环境固定清单；颗粒度是否达标；**成稿是否零主环境回指、零度数回转/公式**；**方向性物体朝向是否已转写为四向固定锚点（头尾双锚优先，首选不可见须改挂对向锚；禁成稿写角度镜像重判/轴线回转）**；FG/MG/BG 每层每实体是否均已写清垂直上/中/下；是否存在主环境未列物件或矛盾；任一不一致须修正衍生行或回流 Stage 1。
