...
- **视角衍生行（含 `0度{主环境名}`）**写 `view_angle_from_main`、`empty_view_delta`、FG/MG/BG 纯空间层次（**每层每固定实体须写清层位+左/中/右+垂直上/中/下即高/中/低，与主环境垂直区段一致**），**并强制写 `derivative_view_360_entities`**（**每条衍生 ENV 独立一份**）：以**本衍生观察轴为 0 度**、**顺时针**用**自然语言**写 **0/90/180/270 四段**（每段 **【{N}度方向】** 起头）。**成稿规则（与 Stage 1 §11 跨阶段契约一致，不重述）**：四向 **【{N}度方向】仅用本衍生自有角度**（与主环境同名度数不是同一参照）；四向与 FG/MG/BG **只写本机位推导结果**，自洽独立，**禁**照抄/提及主环境/Master/0度Master 各度方位与四向段落，**禁**主环境/基准/继承/映射/转角对照/「沿…观察轴」「顺时针右转」等回指；要素完整（FG/MG/BG+左/中/右+上/中/下+具名定语+开闭态+长边/桌头桌尾+正反面/椅背椅面/车头车尾/显示面+门轴侧+实体关系+可见/不可见）；**方向性实体类目与须写朝向要素见 Stage 1 §11「方向性实体参考清单」**。**不同衍生行四向描述禁止照抄**。**内部推演（禁入成稿）**：同角继承、转角对照、镜像映射、OTS 两步确认——细则 Stage 1 §11；`view_angle_from_main=N` 时衍生【k°】↔ 主环境【(N+k)%360°】。**须写 `spatial_axis`（共同锚点）**、**`lens_profile:Wide|Standard`**、**`axis_crossing:None|PlannedReverse|MotivatedCross`**（见下条）。**`referenceable:Yes`**。
