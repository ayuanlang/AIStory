...
- **主环境一致性（最高硬约束，推演用+成稿颗粒度）**：`derivative_view_360_entities` 四向中的**每一个具名固定实体**须**逐字可追溯**至主环境固定清单与俯视/仰视 360 拓扑；**全名、定语、开闭态、门轴侧、椅背/桌头/主客位、相对位置与方位归属须与主环境完全一致**——衍生**仅允许**写当前观察轴下的**可见/不可见**与**方向性物体锚点朝向/正反面/左右序位**（内部推演后须转写为具名锚点或视角表述，**禁**成稿写度数回转/公式）；**可数同类实体个数一致（强制）**：椅/杯/窗/柜等同类型多件的总个数、逐具名称与区分特征须与主环境**完全一致**，禁止衍生中省略、合并或增删数量。**固定实体世界朝向跨衍生一致（强制）**：同一具名固定实体在**全部**视角/状态衍生中的**世界空间朝向、开闭态、锚点相对关系须一致**——各衍生仅可因机位不同改写**相对镜头的可见面/左右序位**；**禁止**无依据改变桌头相对侧墙、门扇开向与门轴侧、椅背朝桌心等。**唯一例外**：① 状态/特效衍生且已写清可持续结构/布局变化；② Stage 1/Beat 明示明显受力、撞击、拖拽、翻转、移位，须在触发 Beat/`empty_view_delta` 写清后后续衍生方可沿用新朝向。**禁止**增删改物体、改名、改开闭态/连接关系/材质尺度定语、另造主环境未列物件；**禁止**照抄 0 度 Master 或主环境对应方位的朝向表述；**成稿本身不得出现主环境角度回指**。
...
- **逻辑一致性终检（推演用，禁写入成稿）**：输出前逐条视角衍生 ENV 核对——四向/FG/MG/BG 实体是否均可追溯至主环境固定清单；**同类多件个数与逐具区分特征是否与主环境一致**；颗粒度是否达标；**成稿是否零主环境回指、零度数回转/公式**；**方向性物体朝向是否已转写为四向固定锚点（头尾双锚优先，首选不可见须改挂对向锚；禁成稿写角度镜像重判/轴线回转）**；FG/MG/BG 每层每实体是否均已写清垂直上/中/下；是否存在主环境未列物件或矛盾；任一不一致须修正衍生行或回流 Stage 1。
