...
- **视角衍生行（含 `0度{主环境名}`）**写 `view_angle_from_main`、`empty_view_delta`、FG/MG/BG 纯空间层次，**并强制写 `derivative_view_360_entities`**（**每条衍生 ENV 独立一份**）：以**本衍生观察轴为 0 度**、顺时针写 **0/90/180/270 四向**；**每一向须逐条具名列出具体固定实体**（来自主环境清单，禁止「侧墙/背景/…」泛称或省略号），并**分列该向可见实体 / 该向不可见实体**。**不同衍生行四向清单禁止照抄**。**`referenceable:Yes`**。
...
- **环境（ENV）**：
  ...
  - **0 度视角衍生（强制，首场 Master 可拍 ENV）**：`subject_name_zh` = `0度{主环境名}`；`view_angle_from_main:0`；`referenceable:Yes`；`generatable:Yes`；`reference_env:{主环境名}`；**须含 `derivative_view_360_entities`（四向逐向具名实体，独立撰写）**；**每场至少 1 行**；Beat 中 0 度 Master/Two Shot 须引用此行。
  - **其他视角衍生环境**：`subject_name_zh` = `{观察角度}度{主环境名}`；`view_angle_from_main` = N；`referenceable:Yes`；`generatable:Yes`；`reference_env:{主环境名}`；**须含 `derivative_view_360_entities`（四向逐向具名实体，与本行机位匹配，禁止照抄其他衍生行）**。
...
- 衍生环境至少包含：`env_role:衍生环境`、`referenceable:Yes`、`generatable:Yes`、`derivative_base_zh/en`、`view_angle_from_main`、`derivative_trigger_type`、`empty_view_delta`、**`derivative_view_360_entities`（四向逐向具名实体，每条衍生独立，强制）**、`spatial_axis`、`return_or_continue`；**全部衍生**建议含 `reference_env:{主环境名}`；**`0度{主环境名}` 强制行**须含 `view_angle_from_main:0`；主环境行须含 `referenceable:No`、`generatable:Yes`、`env_role:主环境基准定义`；主/衍生 ENV 建议含 `literary_atmosphere`（文学环境氛围，非技术参数）；自动补齐另含 `auto_completed_derived_env` 与触发证据；无法安全建依赖时写 `upstream_missing_derived_env` 回流标记。
...
| S004 | environment | 0度办公室会客区 | 0 Deg Office Reception Area | 办公室会客区 | Office Reception Area | env_role:衍生环境；referenceable:Yes；generatable:Yes；reference_env:办公室会客区；view_angle_from_main:0；empty_shot_only:Yes；derivative_base_zh:办公室会客区；angle_mapping:衍生0度=主环境0度轴Master；empty_view_delta:Master Two Shot 可见半空间；derivative_view_360_entities:衍生0度可见=百叶窗墙+会议桌长边+主位/客位转椅(空椅);衍生0度不可见=半开木门+外侧通道;衍生90度可见=桌头+侧墙高窗;衍生90度不可见=文件柜背面;衍生180度可见=文件柜+白板墙;衍生180度不可见=百叶窗墙(机位后方);衍生270度可见=会议桌桌尾+侧墙段;衍生270度不可见=半开内开木门(机位后方)；literary_atmosphere:继承主环境；topology继承:Yes；FG/MG/BG:仅空间层次（禁层内人物/道具）。纯空镜；严禁角色/临场道具/动作；严禁技术光学参数。 | 0度办公室会客区、主环境名、Master Two Shot |
| S005 | environment | 180度办公室会客区_桌后反打 | 180 Deg Office Reception Area Desk Reverse | 办公室会客区 | Office Reception Area | env_role:衍生环境；referenceable:Yes；generatable:Yes；reference_env:办公室会客区；empty_shot_only:Yes；derivative_base_zh:办公室会客区；view_angle_from_main:180；angle_mapping:衍生0度=主环境180度；empty_view_delta:BG=文件柜与白板墙;退出可见=百叶窗墙;主位/客位椅背按180度重判;derivative_view_360_entities:衍生0度可见=半开木门+外侧通道+会议桌(椅背朝镜头)+主位/客位转椅(椅背朝镜头);衍生0度不可见=百叶窗墙(机位后方);衍生90度可见=会议桌桌头+侧墙高窗段;衍生90度不可见=文件柜列侧面;衍生180度可见=文件柜+白板墙段;衍生180度不可见=会议桌桌尾(机位后方);衍生270度可见=侧墙段+侧墙高窗顶界;衍生270度不可见=半开内开木门(对侧)；literary_atmosphere:继承主环境；topology继承:Yes；FG/MG/BG:仅空间层次（禁层内人物/道具）。纯空镜；严禁角色/临场道具/动作；严禁技术光学参数。auto_completed_derived_env:Yes；trigger_evidence:action_dialogue_env_bind:角色=陈医生;主动作=倾听/接文件;对白=把文件给我;观察视角=正反;Beat=2;Stage1证据=切至桌后反打 | 0度办公室会客区、180度办公室会客区_桌后反打、empty_view_delta、陈医生、把文件给我、桌后反打 |
