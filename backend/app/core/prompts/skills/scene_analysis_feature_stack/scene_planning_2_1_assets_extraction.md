...
# Prompt Updated At: 2026-08-01 12:15:00 +08:00
...
本环节「优化后剧本」已按 Scene **瘦身**为（可选）【角色设定】+ 环境块 + **【场景切换与首节拍转场】（含服化道核销摘要）** + Beat 块（非整场 Stage 1 全文）：
...
[ENV_BLOCK_END]
【场景切换与首节拍转场】…
服化道核销摘要：环境细节=…｜服饰/换装=…｜道具细节=…（无则对应项写「无」）
...
- **不在输入中的** Scene 级说明块**不得臆补**；缺覆盖排期时仅依【角色设定】+ ENV + 服化道摘要 + Beat 可核销证据。
...
- **服化道核销摘要（最高；单权威）**：格式 `环境细节=…｜服饰/换装=…｜道具细节=…`（「无」=原文未写）。必须识别并消费：
  1. **服饰/换装→CHAR**：非「无」时解析具名+着装/换装。换装/更衣/第二套及以上可区分装束 → 按「剧情明文换装」拆 ≥2 条 CHAR；仅同套已着装 → 写入当前行 `clothing:`。
  2. **环境细节→ENV 属性**：写入主环境或已声明状态衍生；**禁**另建未声明衍生行。
  3. **道具细节→PROP 或 ENV 定语**（服从 XOR）。
  4. **与 Beat 冲突**：画面终态以 Beat+【角色设定】为准；摘要多出可核销细节仍入库；摘要写换装而 Beat 仅新装现态 → 仍按换装多行（入场新装=换装后衍生）。
  5. ❌ 摘要更衣却只落一行｜跳过摘要只扫 Beat｜✅ 先摘要再 Beat 双核销。
- **闪回/回忆**：
...
- **CHAR 重评估（新 Scene）**：禁因同名跳过重检。触发（其一且可持续→衍生或独立）：①时间跨度大；②身份体系变化；③性情表现体系变化（非瞬时情绪）；④任务/职能实质差异；⑤摘要/明文换装。先判新增再复用。
- **衍生门禁（默认）**：`重大变化 + 持续变化`；缺一不可。重大=稳定换装体系/年龄态/长期遮面/可持续宿主特效/非紧密转场/重检差异等；持续=跨多 Beat/Scene 或可继承。未过门禁→只写基础版属性或留 Beat。**例外**：剧情明文换装（下条）豁免「持续」时长门槛。
- **剧情明文换装 → 强制多行（最高；覆盖门禁持续项）**：服化道「服饰/换装」/Beat/【角色设定】/「服装要求」出现换装/更衣，或两套及以上可区分服饰体系 → **≥2 条** character（基础版+`{角色}_{装束标识}`）；禁单行混装。命名/链：`base_entity` 指上一稳定版；该态写清 `clothing:`；`plot_role`/`gender`/`age_tier` 继承。扫描：先摘要再 Beat（换装/更衣/换上…/卸甲换便衣等）。**边界**：撩袖/解扣/披外套未换体系、瞬时湿衣未成新装束、纯情绪「看起来不一样」→不建行；摘要「无」且无证据→不适用。❌ 更衣礼服只一行｜✅ 基础+礼服衍生。
...
| 7 | 服化道三项已消费；换装→≥2 CHAR；上游描述已入库；`clothing_req`/`clothing_env` 命中已写 |
...
**S002**：明文更衣/换装须另建衍生行，禁与基础版混写两套服装。
