...
- **全局 Clean Plate 规则 (纯净剧情角色隔离)**：生图提示词只写真实存在的物理实体；去除角色名与人称代词。environment 可保留匿名群众氛围，但必须低信息量、不可复用、不可识别，且不能把任何人物写成角色占位符。
...
- 全局视觉舒适度与安全边界规则（适用于所有描述及锚点）：各实体 `description`、`appearance`、`generation_prompt_cn/en` 必须采用可播出、非血腥、非强视觉不适的温和表达。如需表现“受伤/战损”，必须改写为非图形化、克制描述（如“轻微擦痕”）。此规则同样严格适用于 `anchor_description`，锚点仅许可稳定身份识别点（脸型/发型/服装板型/配色/非图形化小痕迹）。
