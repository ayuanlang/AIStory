...
- **角色与道具四宫格与画幅绝对强制**：所有角色（character）与所有道具（prop）的 `generation_prompt_cn/en` 必须严格采用**四宫格/四视图设定图**格式（所有视角横排展现），默认对齐 `16:9` 横向画布。**必须在提示词中强烈强调各宫格之间以图片背景色（如纯白/纯色背景）明确分隔开，画面边缘与格间严禁跨格粘连。**绝对禁止以任何理由简化为单张、双格、三格或其他非四格变体或自行发明冲突排版指令（如 `vertical / portrait sheet`）。
  ...
  "props": [
    {
      ...
      "generation_prompt_cn": "写实英文项目道具四视图：警徽挂绳证件卡。固定于16:9横向单画布。居中转台相机视角。该道具呈现深蓝尼龙织带、边缘磨损的硬体宽卡套、划痕做旧的铜表面警徽及英文证件图文。视图呈现为四个独立面格：第一宫微距特写(Close-up展现金属划痕与纤维细节)，第二宫正面视图(Front)，第三宫侧面视图(Side)，第四宫背面视图(Back)，采用规整的4宫格表现形式。侧重打光材质厚度，顶部柔和关键光源分离轮廓深度。静物单体展示，各格间以全白纯净背景色明确物理分隔。",
      "generation_prompt_en": "Photoreal prop sheet for a Police ID Badge Lanyard on a 16:9 horizontal canvas. Centered turntable view. Exactly four panels in a single row: Close-up (highlighting brass badge scratches, acrylic texture, and nylon fibers), Front, Side, and Back. Display navy woven strap, worn brass clip and badge, rigid clear acrylic holder, and English layout ID card. Soft top-angle key light with clean rim separation to emphasize material depth and wear. Pure white background clearly separating each panel.",
