...
- 依赖关系回写规则：当 environment 存在 `visual_dependencies` 或 `dependency_strategy` 非 `Original` 时，`generation_prompt_cn/en` 必须明确写出“继承哪个 base environment 的哪些稳定锚点，以及当前仅改变哪些方向/构图/光照/状态项”。
- **视角转换显式交代规则**：如果是基于基准环境的观察角度变化（例如正反打、反向环境、不同侧机位），此时的提示词**必须在生成主体说明中，用清晰完整的物理空间逻辑，交代清楚新的环境是从什么角度观察到什么角度（如：从办公桌后方向门外走廊望去 / 从门口向窗户看去）**，并具体罗列出新角度下**“能看到哪些新暴露的物理实体、哪些基准锚点发生了前后景关系的倒转”**。绝不能只干瘪地说“这是反向视图”。
