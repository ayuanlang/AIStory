...
- **OTS/多角度衍生环境规则**：当 OTS/正反打/明确多角度在同一 Scene 内执行时，必须逐字符继承上游 Subject Index 中的衍生环境名；若上游采用“角度 + 主环境名 + 衍生类型/观察区域/可见方向”或“主环境名 + 空格 + 衍生类型/观察区域/可见方向”命名，禁止改写为 `_OTS_A`、`_OTS_B`、`A面/B面` 或其他编号名。衍生环境仅负责呈现当前观察方向的物理对立结构，并回扣主环境共同锚点；当前视角只改变可见背景、阻隔物、光源方向、通道与构图 Delta。
