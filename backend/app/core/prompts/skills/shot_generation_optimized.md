...
3. **推拉摇移硬约束 (Camera Moves)**：每场戏至少应用1个高级运镜（不可凭空随机使用，须通读整个场景，寻找经典影视作品场面，整体规划在哪个分镜使用，和其他分镜怎么衔接，并在使用高级运镜的分镜中要在Shot Logic中概要说明）。OTS (过肩镜头) 须写满四要素：过谁的肩｜肩主可见面（左侧面/右侧面/正面/背面）｜左肩或右肩（Left/Right-Shoulder）｜过肩后看谁。不可越轴。
...
视频需使用自然语言并维持双语，包含四段加收束。**强烈要求每个维度独立成行（必须使用 <br> 进行换行）**：
...
5. **禁止另起物理文字段**：名牌、花字、信/屏幕/书/牌匾等字案写入对应 P 段，禁止单独开「物理文字生成」段。
6. **品质收束（强制，每镜末尾）**：四段写完后，中英文视频提示词均须以独立收束句结尾；中文原文逐字为：`人物面部稳定不变形，动作自然流畅，无卡顿无闪烁，无背景音乐，无字幕，视频全程禁止出现外形、着装、配饰完全一致的人物，禁止生成同款分身、双胞胎效果，同一画面中仅保留单个对应人物，不出现人物重复复刻，高清，细节丰富，电影质感，色彩自然，光影柔和`。用 `<br>` 与上段分隔；禁止改写、删减或漏写。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (自动生成) | (核心动作简述) | (当前场景ID) | (前接状态+转场手法+分段耗时加法公式等内容) |  | (按时序排列的完整视频提示词，含起始状态、过程相位、终局落点；末尾品质收束句) | (整数秒数) |  |  |  | (对应 Video Content 中文文本，保留实体标签；末尾：人物面部稳定不变形，动作自然流畅，无卡顿无闪烁，无背景音乐，无字幕，视频全程禁止出现外形、着装、配饰完全一致的人物，禁止生成同款分身、双胞胎效果，同一画面中仅保留单个对应人物，不出现人物重复复刻，高清，细节丰富，电影质感，色彩自然，光影柔和) |  |  | (该镜头涉及的 `CHAR`, `PROP`, `ENV` 标签列表) |
