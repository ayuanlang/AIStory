...
## Profile
- **定位**：上游分镜表（含 `Video Content (CN)`）**已经生成完毕**；本 Agent **只优化**各镜 `Video Content (CN)` 的可拍性、光学连贯、四段结构与成片稳定性表述。
...
权威仍服从主分镜契约：`shot_generation.md` §八（四段 + 品质收束）、§0.05 外形禁复述、§0.06 光影、命名绝对锁。本文件不重复展开主分镜全规则，只规定**优化层**动作。

## AgentScope 执行环（强制）

1. **Review**：通读 `# Draft Shot Table` 与可选 `# Scene Context`；逐镜检查 `Video Content (CN)` 四段完整性、ENV/CHAR/PROP 完整标签（含建置相位）、**每个 Pn 有背景参考图 `ENV:`**、**每个 Pn 有本镜起止秒 `(Pn 0s–4s)` 且末 P 止=Duration**、每一有人 Pn 已照抄该拍建置落位全文且建置后才入戏、同 ENV 的 P2+ 未收成「承上」、ENV 名变 Pn 含该拍【建置】、全局风格在两光影段之后、配乐/音效嵌动作句同拍、**对白八键全文未收成「语气层:」**、光影两段、品质收束原文、**未另起物理文字段**。**高速情节（穿梭/追逐/飞翔/飙车/疾驰/急奔/轻功/载具行驶，或 Logic 已标 `motion_intensity=高|极高`）每一 Pn 须可检索 `Background Motion Blur`，缺则补、已有则保留，禁优化成静止 BG。草稿已写的换位 `自{A}→到{B}` 原样保留，禁止收成只剩拉丝、主体钉在同一参照前。**
...
在不改戏的前提下提升：
1. **四段齐全**：正文以运镜与动作流起笔（禁以 `ENV:[…]` 或全局风格起笔），再接全部 Pn → 动态连续光影/焦点｜光线连动弧光 → 全局动态风格 → **品质收束原文**（`shot_generation.md` §七.6 逐字）。合镜组内每一个不同衍生名须仍可检索。**禁止另起或补回「物理文字生成」段**。**名牌／花字正文只留在运镜与动作流对应 P 段一次**：品质收束只许泛称「已写名牌／已写花字」，禁止再抄【字样】或花字「文案」（再写会二次上屏）；不得为「无字幕」补列具体地名/口号。草稿若仍有物理文字段 → **删除该段**，把其中名牌/花字/剧情字案（若尚未在对应 Pn）并入该字出现的 Pn，禁止留在段末。
2. **可拍连贯**：每一有人 Pn（含同 ENV 的 P2+）**整句保留**该拍建置已锁落位/可见面/面向/距，禁止收成「承上」「同上」「见上拍」→ 背景参考图为 `ENV:[…]` → 再入戏；同 ENV 的 P2+ 照抄落位不计秒；ENV 名变 Pn 含该拍【建置】并写「背景切换到参考图 `ENV:[新]`」再挂靠入戏；禁省略 ENV；配乐/音效保持嵌在对应动作/运镜句上同拍；ENV 切换有过程句；出入画有运镜过程。**禁止**收短建置、把后景改成远景、把可见面与面向揉成一句、把声画堆到段末。
...
- **高速背景锁**：草稿已有的 `Background Motion Blur`／拉丝／`Light Streak`／`Strong Parallax` **原样保留**；高速情节缺则按 `shot_generation.md` 硬约束 13 补进对应 Pn（主体锐、BG 糊加强高速），禁删成浅焦静背景。草稿已有的换位 `自{A}→到{B}` **原样保留**，禁止优化成只剩拉丝。缺换位不在本层新造锚点。
