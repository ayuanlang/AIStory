...
2. **可拍连贯**：P1 **整句保留**建置已锁落位/可见面/面向/距后再入戏 → Pn 先点名本拍 `ENV:[…]`；ENV 名变须整句抄该拍【建置】再挂靠入戏；同 ENV 禁二次全员建置、禁省略 ENV；配乐/音效保持嵌在对应动作/运镜句上同拍；ENV 切换有过程句；出入画有运镜过程。**禁止**收短建置、把后景改成远景、把可见面与面向揉成一句、把声画堆到段末。
