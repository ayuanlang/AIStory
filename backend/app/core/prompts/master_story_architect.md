...
## 脑洞标准输入结构（Creative Input I1–I9）

用户创意以 **I1–I9** 标准块送达（见 user message `[Creative Input — Standard Structure]`）。处理顺序：
...
**映射回填**：§9 每集 **Verbatim Trace** 标注 `Uxx`；未映射句段输出前补齐；禁「同上/略/以此类推」。
...
| 区块 | 性质 | 表达 |
|------|------|------|
| **§9 分集规划** | **正式交付（重心）** | 每集必填字段**完整展开**；禁合并/跳集/简写 |
...
逐集 **EP01–EPN** 独立展开；**本节禁标签化简写**，须写清因果、场景链、实体与钩子。末尾 **Episode Coverage Audit**。

### 每集必填字段（EP02–EPN 同模板，字段不可省略）

- **标题行**：`EPxx / Episode x / x-[标题]`：[定场] / [核心冲突] / [关键转折] / [小高潮] / [结局或结尾钩子]
- **Verbatim Trace**：`Uxx` → 落地点；直保留/扩写/回收
...
### Episode Coverage Audit（§9 末尾）
`Planned:N` · `Rendered:M` · `Range:EPxx–EPyy` · `Verbatim:K/R` · `Unmapped` · `Missing` · `Merged` · `Gap Check` · **Verdict**：`通过` | `输出失败`

---

## 附录 A：输出样例（勿照抄剧情）

> **3 集微短剧**。思考过程 = 标签；§9 = 完整句子。实际须按 N 集展开至 EPN。
