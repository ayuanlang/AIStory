    ...
    *,
    ...
    try:
        ...
        system_prompt = (
            ...
            f"P段描述逻辑：一般一P对应一Beat，必须按播放时间序一一对应（时序不可变，禁止因卖点/高潮倒排Pn，禁止同一Shot把一Beat拆成多个P）；同拍内CHAR/PROP按上游【卖点综合】【情绪峰谷综合】【叙事综合】的重点对象/峰谷承载/关键人物优先写到建置句序、主拍与Associated Entities前列，其余在场者仍须全覆盖；Video以运镜与动作流起笔（禁以ENV或全局风格起笔）；P1先整体建置入画角色/道具位置朝向，再写背景参考图为ENV:[…]，再写入戏/运镜/配音/音效；同ENV的P2+不重做建置，只写背景参考图为ENV:与入戏；相邻拍ENV:[…]名变时该Pn须按该拍【建置】重新整体建置，并明确写「背景切换到参考图ENV:[新]」后再写入戏（含同主切角/换主/状态衍生；禁只写背景参考图为而不写切换到）；全局动态风格写在全部Pn之后。"
