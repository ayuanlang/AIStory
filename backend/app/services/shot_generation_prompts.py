...
def _build_scene_subject_image_prompts_cn_section(
    ...
    *,
    ...
    body_parts = [
        ...
        "Copy staging's relative/absolute/enclosed placement for CHAR/PROP; keep the structure and use those "
    ...
    *,
    ...
    try:
        ...
        system_prompt = (
            ...
            f"合镜计时：Beat小计=建置串行+并行核Max(语言/动作/微表情/特效/运镜/音效)，禁止七类分项全额相加灌到MaxShotSeconds；同ENV的P2+建置=0s（落位全文仍须照抄，照抄不计秒，禁止只写承上）；跨P同一连续运镜只计一次；ENV微动/窗雨/雨丝/配乐特效=0s。无对白且无复杂动作且无宏大特效的建立/凝视合镜Duration落5–8（~可到9），禁止无撑满内容却填MaxShotSeconds。"
            ...
            f"P段描述逻辑：一般一P对应一Beat，必须按播放时间序一一对应（时序不可变，禁止因卖点/高潮倒排Pn，禁止同一Shot把一Beat拆成多个P）；每个Pn必须标本镜内起止秒，写法=(Pn 0s–4s)，P1从0s起、相邻P首尾相接、末P止秒=本镜Duration，禁止只写(P1)无秒；同拍内CHAR/PROP按上游【卖点综合】【情绪峰谷综合】【叙事综合】的重点对象/峰谷承载/关键人物优先写到建置句序、主拍与Associated Entities前列，只覆盖该拍建置已写入的可见角色与道具；该拍暂不可见的不写入这一 Pn，换衍生环境后建置重新写入的才从对应 P 提及；Video以运镜与动作流起笔（禁以ENV或全局风格起笔）；每一有人Pn（含同ENV的P2+）先照抄该拍入画角色/道具【建置】位置朝向全文，再写背景参考图为ENV:[…]，再写入戏/运镜/配音/音效；禁止把落位收成承上/同上/见上拍；同ENV的P2+照抄落位不计建置秒；相邻拍ENV:[…]名变时该Pn含该拍【建置】整句，并写「背景切换到参考图ENV:[新]」后再写入戏（含同主切角/换主/状态衍生；禁只写背景参考图为而不写切换到）；成稿顺序=运镜与动作流→两光影段→全局动态风格→品质收束；禁止另起物理文字段。"
            ...
            f"花字逐字上屏：引号内每个字单独成形。印章不压字：印=句外旁侧｜压字=禁｜替字=禁，印面不得盖住任一笔画，不得占一个字的格子。店号/品牌/热线走字卡专镜并写烧录=libass｜手写=禁：场景底只出画面，禁止视频模型描店号、品牌名、热线；这些字由后期libass按引号逐字烧录，印章另层不进字盒。字卡不是CHAR/PROP/ENV。有旁白的P不出花字。配套热线/英文字级=小。"
