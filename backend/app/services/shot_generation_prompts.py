    ...
    *,
    ...
    try:
        ...
        system_prompt = (
            ...
            f"花字逐字上屏：引号内每个字单独成形；含「X家」的店号「家」必须可见，禁止何家乐享收成何乐享或复写成何乐乐享；写逐字=…｜禁漏家｜禁复写邻字。印章不压字：印=句外旁侧｜压字=禁｜替字=禁，印面不得盖住任一笔画，不得占一个字的格子，不得把「家」画成印章。店号/品牌/热线走字卡专镜：企业场景底+字层先合成一张静帧，本P Static Hold、首帧=该静帧，按原样上屏，禁止改字重写，禁止把字卡图与场景图当两张参考图分喂；字卡不是CHAR/PROP/ENV。本P已有牌匾同核字号时花字仍出齐自己那句，禁止互吃或删家。有旁白的P不出花字。配套热线/英文字级=小。花字「家」不是第二个人物。"
