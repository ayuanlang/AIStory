    ...
    *,
    ...
    try:
        ...
        system_prompt = (
            ...
            f"P段描述逻辑：用Pn区分不同Beat，且必须按播放时间序一一对应（时序不可变，禁止因卖点/高潮倒排Pn）；同拍内CHAR/PROP按上游【卖点综合】【情绪峰谷综合】【叙事综合】的重点对象/峰谷承载/关键人物优先写到建置句序、主拍与Associated Entities前列，其余在场者仍须全覆盖；每个Pn必备本拍ENV:[…]；角色/道具建置在P1写一次；相邻拍ENV:[…]名变时该Pn须按该拍【建置】重新建置后再写入戏（含同主切角/换主/状态衍生）；同ENV的P2+先环境后入戏，禁止二次全员建置。"
