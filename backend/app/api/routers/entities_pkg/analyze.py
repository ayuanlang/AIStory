...
def _build_entity_analysis_format_contract(entity: Any, category: str) -> str:
    ...
    if _entity_analysis_is_main_environment(entity):
        return (
            ...
            "- ③【四向拼图】：16:9 横幅 2×2；必须可检索锁定行 `四宫度数=左上0度｜右上90度｜右下180度｜左下270度`（固定不可改，禁止对调）；每格组装为「后景=严格按【N度方向】后景；中景=严格按【N度方向】中景；画面左缘=查表结果钉句；画面右缘=查表结果钉句」；禁止俯拍；各格眼高约 50mm；Clean Plate；各格角度文字标志。\n"
    ...
    return (
        ...
        "- §A（仅 logic + anchor_description）：所属主环境= + 四宫度数=左上0度｜右上90度｜右下180度｜左下270度 + 截取宫格（左上0度/右上90度/右下180度/左下270度，与 N 同核）；禁写开篇拓扑/楼底。主环境资产已存在时，用正则匹配其 `四宫度数=` 后按该行度数裁切对应宫格。\n"
        "- §B 第一刀：所属主环境={名}。angle_key={名}|{N}。四宫度数=左上0度｜右上90度｜右下180度｜左下270度。截取宫格={左上0度|右上90度|右下180度|左下270度}。请严格要求按对应主环境「{名}」四向拼图参考图，截取并放大其中对应的明确宫格位置（{宫格}），不要重新描述画面细节，直接作为本镜头的最终画面。切割衍生环境时均按16:9固定比例，并保证高分辨率。只切割，不要改画。成稿须为单张完整镜头：禁止保留四向拼图的宫格分割线、宫格边框、格标/角标、十字拼缝或任何拼图装配痕迹。\n"
...
async def _execute_analyze_entity_image(
    ...
    format_focus = {
        ...
        "poster": "海报 4:3 单张主视觉",
        "environment": (
            "主环境折中两段式：四面后景+中景 + 四宫格引用与左右缘钉（四宫度数=左上0度｜右上90度｜右下180度｜左下270度；固定锁定）"
            if is_main_env
