...
def _build_entity_analysis_format_contract(entity: Any, category: str) -> str:
    ...
    if _entity_analysis_is_main_environment(entity):
        return (
            ...
            "- 各格机位正对输入该向内容（该向内容=背景正面面对的围合立面）；四方正交；禁止偏斜/墙角构图/荷兰角。\n"
            "- 成稿逐格写 [0度格-左上]/[90度格-右上]/[270度格-左下]/[180度格-右下]，每格按「背景立面→中景舞台净空→中景中央家具→前景/邻向立面斜切→天花地面→光照」。\n"
            "- 中区地面家具（茶几/矮几/中央桌/沙发配套几）四格守恒：凡几何可见必须具名；禁止只在 0 度格出现、90/270 蒸发。邻向贴边斜切只约束门窗墙柜等立面，不约束中区家具。背景=该向围合立面，沙发与矮几进中景-中央家具。\n"
            "- 门/窗/楼梯唯一落位：完整门扇或贴墙整跑梯只写在所属扇区那一格的背景；90/270 邻向格禁止再写「大门/玄关大门/楼梯」作第二件，只写邻向墙段贴边斜切。\n"
            "- 楼梯/台阶/坡道须声明上行朝向 U（走上时面朝哪一扇区，≠所在坐标）；U 当 F 查可见面：正面=迎面踏步，背面=梯底，侧面=梯帮；禁止四面都画成迎面拾级。\n"
            "- 歧义控制：一名一物；一物一位；坐标≠朝向；「朝玄关一侧」只定位家具，不在该格生成门/梯。\n"
