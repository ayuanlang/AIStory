...
def _build_entity_analysis_format_contract(entity: Any, category: str) -> str:
    ...
    if _entity_analysis_is_main_environment(entity):
        return (
            common
            + "主环境 generation_prompt_cn 格式（强制，两段式：先四面后四宫格引用）：\n"
            "- ①全局共享（全文一次，不得少）：整体风格定位 + 开篇拓扑 + 光影体系 + 色彩体系 + 中区共享实体。\n"
            "- ②【四面内容基准】：一次性写满【0度方向】【90度方向】【270度方向】【180度方向】（含细节·材质·可见面·左右缘·光照落点）；每向按「背景正对→中景舞台净空→中景中央家具→画面左右缘贴边窄条→天花地面→本向光照」。此为画面文字唯一真源。\n"
            "- ③【四向拼图】：仅引用四面生成 16:9 横幅 2×2 四宫格（左上=0度、右上=90度、左下=270度、右下=180度）；禁止俯拍/鸟瞰；各格眼高约 50mm；四格共享材质/光源；Clean Plate；各格角度文字标志；禁止在③重写陈设/材质/光色长文。\n"
            "- 各向机位正对输入该向内容（该向内容=背景正面面对的围合立面）；四方正交；禁止偏斜/墙角构图/荷兰角。\n"
            "- 中区地面家具（茶几/矮几/中央桌/沙发配套几）四向守恒：凡几何可见必须具名；禁止只在 0 度方向出现、90/270 蒸发。邻向贴边斜切只约束门窗墙柜等立面，不约束中区家具。背景=该向围合立面，沙发与矮几进中景-中央家具。\n"
            "- 门/窗/楼梯唯一落位：完整门扇或贴墙整跑梯只写在所属扇区那一向的背景正对；90/270 邻向禁止再写「大门/玄关大门/楼梯」作第二件，只写邻向墙段贴边斜切。\n"
            "- 楼梯/台阶/坡道须声明上行朝向 U（走上时面朝哪一扇区，≠所在坐标）；U 当 F 查可见面：正面=迎面踏步，背面=梯底，侧面=梯帮；禁止四面都画成迎面拾级。\n"
            "- 歧义控制：一名一物；一物一位；坐标≠朝向；「朝玄关一侧」只定位家具，不在该向生成门/梯。\n"
            ...
            "- 若图片本身已是四宫格，按四格可见内容回写进【四面内容基准】各向，【四向拼图】仅引用；若图片是单视角，仍须输出完整两段式（其余向据空间一致性合理补齐，并在 logic 标明推断向）。\n"
            "- 严禁跳过【四面内容基准】直接在四宫格堆细节；严禁改成单镜头可拍空镜、§A/§B/§C 衍生三段式、或角色/道具白底四视图。"
        )
    return (
        common
        ...
        "- §B（generation_prompt_cn 核心）：仅写截取放大指令——点名所属主环境「{名}」四向拼图参考图对应宫格；不要重新描述画面细节，不要重布门窗柜台楼梯；继承正对输入该向内容、四方正交、适度景深。\n"
...
"""
    ...
    prompt_placeholder = (
        "按上方主环境两段式原格式写满：全局共享→【四面内容基准】四向→【四向拼图】仅引用"
        if is_main_env
...
async def _execute_analyze_entity_image(
    ...
    format_focus = {
        ...
        "poster": "海报 4:3 单张主视觉",
        "environment": (
            "主环境两段式：全局+【四面内容基准】+【四向拼图】仅引用（左上0/右上90/左下270/右下180）"
            if is_main_env
            else "衍生环境 §B 截取放大（禁另起四面）"
        ),
