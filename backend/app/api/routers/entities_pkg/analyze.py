...
def _build_entity_analysis_format_contract(entity: Any, category: str) -> str:
    ...
    if _entity_analysis_is_main_environment(entity):
        return (
            ...
            "- ①全局共享（全文一次，不得少）：整体风格定位 + 开篇拓扑 + 光影体系 + 色彩体系 + 中区共享实体。开篇拓扑须可检索 `原点=中心` + `罗盘=正北=0｜正东=90｜正南=180｜正西=270`；具名主体写 `心点=`/`距原点={m}米≈{步}步`/`占地=`/`占场≈`。禁止只写度数不写东南西北。舱轴场写 `心点=舱轴{前|后|左|右}`，不写正北正东。\n"
            ...
            "- ②【四面内容基准】：每向只写【N度方向】后景（该向远锚+材质）+ 中景（舞台净空+沿用开篇主体入镜）；**禁止**按格重写桌椅朝向、椅面朝镜头、左右对调；**禁止在四面写左右缘长文**。\n"
            "- ③【四向拼图】：16:9 横幅 2×2；必须可检索锁定行 `四宫度数=左上0度｜右上90度｜左下180度｜右下270度`（行优先；固定不可改）；四格成稿书写序=阅读序 `0左上→90右上→180左下→270右下`（第三段=左下180，第四段=右下270）；每格组装为「机位=望向=正方位｜远锚=该向后景最远可见主体；后景=严格按【N度方向】后景；中景=沿用开篇世界锁；画面左缘=邻向主体点名；画面右缘=邻向主体点名」；禁止俯拍；禁止各格重写桌椅角度/左右对调；各格眼高约 50mm；Clean Plate；角标按格钉：左上必须标0度、右上必须标90度、左下必须标180度、右下必须标270度。\n"
            "- 左右缘必须是查表**结果钉句**（贴边窄条+端锚+可见面+NOT background）；楼梯邻缘另加单跑同核钉；**禁止**写「由邻面旋转/对照得到左右」过程词；**禁止**省略任一侧缘。\n"
            "- 各向机位正对输入该向内容；四方正交；禁止偏斜/墙角构图/荷兰角。\n"
            "- 中区地面家具四向守恒：凡几何可见必须在四面中景具名；禁止只在 0 度出现、90/270 蒸发。\n"
    ...
    return (
        ...
        "- §A（仅 logic + anchor_description）：所属主环境= + 四宫度数=左上0度｜右上90度｜左下180度｜右下270度 + 截取宫格（左上0度/右上90度/左下180度/右下270度，与 N 同核）；禁写开篇拓扑/楼底。主环境资产已存在时，用正则匹配其 `四宫度数=` 后按该行度数裁切对应宫格。\n"
...
"""
    ...
    prompt_placeholder = (
        "按上方主环境折中两段式写满：全局→【四面】后景+中景→【四向拼图】引用+左右缘钉"
        if is_main_env
...
async def _execute_analyze_entity_image(
    ...
    format_focus = {
        ...
        "poster": "海报 4:3 单张主视觉",
        "environment": (
            "主环境折中两段式：开篇极坐标（原点=中心｜正北=0）+ 四面后景+中景 + 四宫格引用与左右缘钉（四宫度数=左上0度｜右上90度｜左下180度｜右下270度；固定锁定）"
            if is_main_env
