...
def _build_entity_analysis_format_contract(entity: Any, category: str) -> str:
    ...
    if _entity_analysis_is_main_environment(entity):
        return (
            ...
            "- ③【四向拼图】：16:9 横幅 2×2；必须可检索锁定行 `四宫度数=左上0度｜右上90度｜左下180度｜右下270度`（行优先；固定不可改）；四格成稿书写序=阅读序 `0左上→90右上→180左下→270右下`（第三段=左下180，第四段=右下270）；每格组装为「后景=严格按【N度方向】后景；中景=严格按【N度方向】中景；画面左缘=查表结果钉句；画面右缘=查表结果钉句」；禁止俯拍；各格眼高约 50mm；Clean Plate；角标按格钉：左上必须标0度、右上必须标90度、左下必须标180度、右下必须标270度。\n"
    ...
    return (
        ...
        "- §B 第一刀：所属主环境={名}。angle_key={名}|{N}。四宫度数=左上0度｜右上90度｜左下180度｜右下270度。截取宫格={左上0度|右上90度|左下180度|右下270度}。请严格要求按对应主环境「{名}」四向拼图参考图，截取并放大其中对应的明确宫格位置（{宫格}），不要重新描述画面细节，直接作为本镜头的最终画面。切割衍生环境时均按16:9固定比例，并保证高分辨率。只切割，不要改画。成稿须为单张完整镜头：禁止保留四向拼图的宫格分割线、宫格边框、格标/角标、十字拼缝或任何拼图装配痕迹。\n"
...
async def _execute_analyze_entity_image(
    ...
    format_focus = {
        ...
        "poster": "海报 4:3 单张主视觉",
        "environment": (
            "主环境折中两段式：四面后景+中景 + 四宫格引用与左右缘钉（四宫度数=左上0度｜右上90度｜左下180度｜右下270度；固定锁定）"
            if is_main_env
