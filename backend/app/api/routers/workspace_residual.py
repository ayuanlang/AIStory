...
from app.core.queue_config import load_queue_config, save_queue_config
...
# Process-local mirror of queue config (updated on admin PUT).
from app.api import endpoints as _endpoints_mod  # noqa: E402

_q_conf = getattr(_endpoints_mod, "_q_conf", None)
if not isinstance(_q_conf, dict):
...
async def admin_update_queue_config(config: QueueConfigBase, current_user: User = Depends(get_current_user)):
    ...
    try:
        _endpoints_mod._q_conf = dict(saved_config)
    except Exception:
        pass
...
async def split_script(
    ...
    try:
        ...
        try:
            with open(
                os.path.join(os.path.dirname(__file__), "../core/prompts/script_split.md"),
                "r",
                encoding="utf-8",
            ) as f:
                sys_prompt = f.read()
            ...
            sys_prompt = (
                "Split script into episodes of ~1000 chars, output JSON with "
