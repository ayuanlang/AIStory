...
_SUPPORTED_FUNCTION_API_NAMES = [
    ...
    "generate_videos",
    "generate_entity_reference_audio",
    "generate_entity_reference_audio_audio",
    "generate_entity_reference_audio_video",
    "script_analysis",
    "ai_assistant",
    "ai_shot",
]
...
def create_provider_key_pool(
    ...
    record = ProviderKeyPool(
        provider=provider_name,
        provider_alias=str(payload.provider_alias or "").strip() or (
            "NukoAi" if provider_name == "nukoai" else ("ShiShiKeJi" if provider_name == "shishikeji" else None)
        ),
        api_keys=keys,
        strategy=strategy,
        weights=weights,
...
def update_provider_key_pool(
    ...
    if payload.provider is not None:
        ...
        if new_provider != record.provider:
            ...
            if new_provider == "shishikeji" and not str(getattr(record, "provider_alias", "") or "").strip():
