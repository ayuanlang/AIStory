...
start_str = "        # Check cache\n        skip_step1 = False"
end_str = "        llm_fallback_warnings = list(set(loop1_res[\"llm_fallback_warnings\"] + loop2_res[\"llm_fallback_warnings\"]))\n"

if start_str in text and end_str in text:
    start_idx = text.find(start_str)
    end_idx = text.find(end_str) + len(end_str)
    
    new_block = """        _release_db_connection(db, "analyze_scene_llm_call")
