...
new_section_2 = """  2. **按时间编排的运镜与动作流 (Chronological Camera & Action)**：此部分无需大写维度标签，直接以 (P1), (P2) 等动作段落切分，将运镜与动作在同一时间线上融合描写：
     ...
     - **精准主体与动作细节（强制）**：必须极度明确当前动作的执行主体，严禁使用独立代词。若是描写“全场震惊”等集体行为，绝不能让所有人做出完全一样的动作表情；坚决摒弃“逃跑”、“战斗”等抽象词，必须将动作拆解为具体的肢体发力方向、微表情变化和空间位移。
...
pattern = r'\n\s*2\..*?\[Camera Movement\].*?(?=\n\s*4\..*?\[Dynamic Atmosphere\])'
match = re.search(pattern, text, re.DOTALL)
if match:
    text = text[:match.start()] + "\n" + new_section_2 + text[match.end():]
