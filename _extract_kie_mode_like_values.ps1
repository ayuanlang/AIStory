...
$targets=@('mode','quality','model','duration')
$out=@()
...
function Normalize-Value([string]$field, [string]$value){
  ...
  switch($field){
    ...
    'quality' {
      if($v -in @('std','standard','pro','fast','turbo','master','quality','basic','high')){ return $v }
      return ''
    }
    'model' {
      # Keep model-like slugs only; drop status codes and generic words.
      if($v -match '^(?!http)(?!\d{3}$)[a-z0-9][a-z0-9\-\./]{2,80}$'){
        if($v -notin @(
          'type','string','boolean','number','array','object','required','description','default','enum','example',
          'assistant','user','system','tool','function','text','high','low','normal','auto','developer'
        ) -and
           $v -notmatch '^(\d{1,2}:\d{1,2}|\d{3,4}p|\d{1,2}k)$'){
          return $v
        }
      }
      return ''
...
foreach($u in $urls){
  ...
  foreach($f in $targets){
    ...
    foreach($idx in $idxs){
      ...
      $end=[Math]::Min($lines.Count-1,$idx+110)
...
$csv='C:\storyboard\AIStory\_kie_field_mode_like_values_catalog.csv'
$md='C:\storyboard\AIStory\_kie_field_mode_like_values_catalog.md'
...
$cleanCsv='C:\storyboard\AIStory\_kie_field_mode_like_values_catalog_clean.csv'
$cleanMd='C:\storyboard\AIStory\_kie_field_mode_like_values_catalog_clean.md'
$clean|Export-Csv -Path $cleanCsv -NoTypeInformation -Encoding UTF8

$lines=@()
$lines+='# KIE Mode-like Allowed Values Catalog'
$lines+="Generated at: $(Get-Date -Format s)"
$lines+="Rows: $($out.Count)"
$lines+=''
$g=$out|Group-Object field|Sort-Object Name
foreach($x in $g){ $lines+="- $($x.Name): $($x.Count) pages" }
$lines+=''
...
$lines2=@()
$lines2+='# KIE Mode-like Allowed Values Catalog (Cleaned)'
