...
from app.api.endpoints import parse_shots_markdown_table, _serialize_shot_rows_to_markdown, _parse_shot_markdown_or_raise
...
try:
    md = _serialize_shot_rows_to_markdown(rows)
    print("Serialized md:", repr(md[:100]))
    headers2, rows2, table_line_count = _parse_shot_markdown_or_raise(md, source_label="test")
    print("reparsed rows count:", len(rows2))
