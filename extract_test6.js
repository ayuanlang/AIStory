const text = '\n### Subject Index\nsubject_no | subject_type | subject_name_zh | subject_name_en | dependency_reference | entity_attributes | script_entity_coverage \n--- | --- | --- | --- | --- | --- | --- \nS001 | character | 布鲁克·海斯 | Brooke Hayes | None | Protagonist, Neutral faction, fake wife acting as twin sister, female, late 20s, Caucasian. Wearing Claire\'s expensive silk nightrobe. Cold, suppressed, isolated demean...';
...
const dashMatch = text.match(/-{4,}\s*\n([\s\S]*?)\n\s*-{4,}/);
if (dashMatch && looksLikeSubjectIndex(dashMatch[1])) {
    console.log('dashMatch');
} else {
    const match = text.match(/(?:^|\b|\s)#{0,6}\s*(?:\*\*)?\s*(?:Subject Index|Subjects? Index|角色索引|道具索引|场景索引|实体索引|设计资产索引|Entities Index|资产清单|实体清单|设计清单|Subject Extract|剧本实体分析|主要提取实体|实体|Entities|Subjects|Assets|资产|人物列表|提取实体)\s*(?:\*\*)?\s*\n[\s\S]*/i)
        || text.match(/#{1,6}\s*(?:\*\*)?\s*(?:Subject Index|Subjects? Index|角色索引|道具索引|场景索引|实体索引|设计资产索引|Entities Index|资产清单|实体清单|设计清单|Subject Extract|剧本实体分析|主要提取实体|实体|Entities|Subjects|Assets|资产|人物列表|提取实体)[\s\S]*/i)
        || text.match(/(?:^|\b|\s)(?:\*\*)?\s*(?:Subject Index|Subjects? Index|角色索引|道具索引|场景索引|实体索引|设计资产索引|Entities Index|资产清单|实体清单|设计清单|Subject Extract|剧本实体分析|主要提取实体|实体|Entities|Subjects|Assets|资产|人物列表|提取实体)[\s\S]*/i);
    if (match) {
        console.log('match!', trimSubjectIndexSection(match[0]));
    } else {
        const pipeMatch = text.match(/(?:^|\n)\s*(subject_no\s*=\s*[A-Za-z]?\d+[\s\S]*)/i);
        if (pipeMatch && String(pipeMatch[1] || '').trim()) {
            console.log('pipeMatch');
        } else {
             const subjectTypeLine = text.match(/(?:^|\n)(.*subject_type\s*=\s*(?:character|prop|environment|cover_poster).*(?:\|.*)+)/i);
             if (subjectTypeLine && String(subjectTypeLine[1] || '').trim()) {
                 console.log('subjectTypeLine');
             } else if (looksLikeSubjectIndex(text)) {
                 console.log('looksLike');
             } else {
                 console.log('NONE');
             }
        }
